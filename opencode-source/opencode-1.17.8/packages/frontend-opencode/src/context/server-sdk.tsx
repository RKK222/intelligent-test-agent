...
function createServerSdkContextBase(server: ServerConnection.Any, scope: ServerScope) {
  ...
  // 适配层事件注入：把后端 RunEvent 投影后的 opencode Event 喂给同一 emitter，
  // 由现有 reducer（applyDirectoryEvent）处理，无需改 reducer。
  const pushExternalEvent = (directory: string, event: Event) => {
    batch(() => emitter.emit(directory, event))
  }
  adapter.setDispatch(pushExternalEvent)
