...
- `DirectoryRows.vue`：递归渲染 22px 文件树行、目录展开状态、变更行数和加载状态；文件和目录双击后提供行内改名，行尾 `−` 或聚焦后按 Delete/Del 共用删除确认；普通文件支持 Ctrl/Cmd+C/X/V/Z、右键复制/剪切/粘贴/撤销和拖放到目录，目录 `+` 面板可按明确目标路径新建或上传；新建/上传与删除确认使用统一紧凑工作台弹框，最终操作均 emit 给 app 层。
