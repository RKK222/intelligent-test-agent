...
describe("Mermaid 单行紧凑 metadata", () => {
...
G --> E`);
    ...
    const legacyLength = [legacyLayout, legacyPorts, legacyRoutes]
      ...
      .join("\n").length;
    const markers = compactMarkers(serializeMermaidGraph(graph));
  ...
  it("Flow 与 Sequence 的紧凑单行注释均通过 Mermaid 官方 parser", async () => {
    const flow = serializeMermaidGraph(graphWithCompactMetadata());
    const sequence = parseMermaidSequence("sequenceDiagram\nactor U as 用户\nparticipant S as 服务\nU->>S: 请求");
    sequence.participants[0]!.position = { x: 80, y: 70 };
    sequence.participants[1]!.position = { x: 300, y: 70 };
    const serializedSequence = serializeMermaidSequence(sequence);
