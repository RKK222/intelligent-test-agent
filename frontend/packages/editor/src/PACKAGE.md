...
- `mermaid/visual-editor/`：通用可视化对话框和 Flowchart Vue Flow 画布；右侧节点图形库负责拖放或点击创建五种节点，判断节点使用水平真菱形，选中节点属性显示在图形库下方。每个节点按图方向提供六个通用 Handle，节点悬浮才显示点位；独立拖线控制器使用固定屏幕半径处理起线、目标激活、近点吸附、SmoothStep 临时路径和窗口级取消生命周期，Vue Flow 原生 connect 关闭。适配层优先投影新边保存的固定端口，旧边仍按入边、出边顺序确定性分配。Sequence 画布位于 `mermaid/sequence/visual-editor/`，消息使用按顺序错层的自定义边避免重叠。
