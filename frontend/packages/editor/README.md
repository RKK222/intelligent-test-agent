...
- Mermaid 可视化编辑：预览中的 Mermaid block 提供“可视化编辑”入口，点击后才懒加载 `@vue-flow/core`、Mermaid 官方 parser 和编辑对话框，不影响 Markdown 首屏。`flowchart`/`graph` 通过右侧图形库展示矩形、圆角、胶囊、水平真菱形判断和圆形节点，支持拖到画布落点或点击创建；选中节点后在图形库下方编辑名称、形状和删除操作。每个节点按图方向在入口侧和出口侧各布置 3 个通用连接点：空闲时隐藏、悬浮节点时显示，六点均可按住拖线；靠近目标节点后只显示该节点的点位，近点自动吸附，松开后创建固定端口连线。旧边继续按入边、出边顺序自动均匀分配端口，新边通过 Mermaid 可忽略的 `%% editor-edge-ports:` 注释保存所选起终点；编辑器继续支持方向与采用 ELK Layered 算法的工业级自动布局（基于 elkjs 进行计算，浏览器构建必须使用 `elkjs/lib/elk.bundled.js` 入口，不能引用默认 Node 入口）。`sequenceDiagram` 支持 participant/actor 增删拖拽、名称/类型，以及有序消息的增删、标签、方向、箭头类型和上移/下移。
...
- 布局、端口与并发保护：节点/参与者位置写入 Mermaid 可忽略的 `%% editor-layout:` JSON 注释，Flowchart 新建连线的固定端口写入 `%% editor-edge-ports:` JSON 注释；损坏或无法唯一匹配的端口注释原样保留，合法陈旧条目在下次保存时清理。应用时只替换当前 fence；若打开期间 Agent 或其他入口刷新了该 block，则拒绝覆盖并提示重新打开。
