...
- Model/Provider 目录仍读取 opencode 原生接口；运行配置存在非空 `enabled_providers` 时，client 按同一 Provider ID 白名单过滤两类目录，企业配置不会混入 OpenCode Zen。未配置白名单或 config 暂时读取失败时保持原生目录；并发目录查询复用同一轮 config 请求，请求结束即失效以支持配置热加载。
...
- 暴露 `getSessionRuntimeState()` 兼容读取 `/api/internal/platform/opencode-runtime/sessions/runtime-state`；缺少 `permissionCount` 的旧摘要按 `0` 或 `sessions[].attention=PERMISSION` 安全推导。新工作台以 `event-stream-client` 的用户级 runtime-state SSE 为主入口，不在流连接期间并行调用该 HTTP 接口。
...
- 暴露应用版本工作区和个人工作区 API 方法，统一走 `/api/internal/platform/workspace-management`，包括成员应用、模板、版本、个人空间、最近使用、diff、同步、版本工作区 `gitPullWorkspaceVersion(versionId)` 和选择前只读 `checkWorkspaceVersionGitAccess(versionId)`；预检结果透传版本库名称与 `REPOSITORY_PERMISSION_REQUIRED` / `SSH_KEY_MISSING` 原因，版本响应透传目标 commit 与服务器副本状态字段。
