...
- 暴露应用引用资产库方法 `listReferenceRepositories`、`initializeReferenceRepository`、`synchronizeReferenceRepository`、`getReferenceRepositoryStatus`、`listReferenceRepositoryTree`，统一走 `/api/internal/platform/workspace-management/applications/{appId}/reference-repositories`；初始化分支候选继续复用既有 `listRepositoryBranches(repositoryId)`。client 只透传总体/逐服务器状态和树节点 `highlighted/selectable`，不推断状态机，不访问引用资产磁盘；工作区 `.opencode/opencode.jsonc` 仍通过既有 `readFile/writeFile` 的 route + ticket + WebSocket RPC 修改。
...
`backend-api.test.ts` 覆盖 5 个引用资产端点的 app/repository/path 编码、初始化 body、无 body 同步和响应透传，以及组合视图 list/read 的 locator、稳定身份、只读来源和局部 warning 映射。
