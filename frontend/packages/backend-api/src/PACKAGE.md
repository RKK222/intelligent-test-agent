...
- `createXxlJobSsoTicket()`：调用平台票据 API并返回同源表单动作；只允许组件把 ticket 写入瞬时隐藏表单，禁止拼接 URL。原始 HTTP observer 会对 ticket/token/cookie/password/secret/sessionDigest 递归脱敏。
