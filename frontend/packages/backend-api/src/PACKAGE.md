...
- `index.ts` 同时定义引用资产总体/逐服务器状态和单层树类型，并封装 list/initialize/synchronize/status/tree 5 个内部 API；分支列表继续复用配置管理方法，引用配置文件继续复用 workspace 文件 WebSocket `readFile/writeFile`，工作台组合树使用同一连接的 `listWorkspaceView/readWorkspaceViewFile` 并透传稳定 locator、来源、只读能力和局部 warning，client 不直连 Git、磁盘或 opencode server。
