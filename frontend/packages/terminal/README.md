...
workspace 真实 PTY 浏览器到后端 WebSocket 的验收属于 `tools/dev-phase11-real-e2e.sh --start-services`；服务器终端可在本地 `test` profile 通过直连 WebSocket 验证，企业环境还必须使用运行于预期 systemd 用户下的 Java 和 HTTPS/WSS Nginx 验收权限边界与定向路由。
