...
import { createTerminalSession, TerminalPanel } from "../src";
...
describe("terminal client", () => {
  ...
  it("creates a ticket from the panel and renders terminal output", async () => {
    ...
    render(<TerminalPanel baseUrl="http://127.0.0.1:8080" createTicket={createTicket} WebSocketCtor={FakeWebSocket} />);

    fireEvent.click(screen.getByRole("button", { name: "连接终端" }));
    await waitFor(() => expect(createTicket).toHaveBeenCalledTimes(1));
    FakeWebSocket.instances[0]?.open();
    FakeWebSocket.instances[0]?.emit({ type: "output", data: "phase 11\n", seq: 1 });

    expect(await screen.findByText("phase 11")).toBeTruthy();
  });
});
