...
- 监听并按原样转发平台 RunEvent wire name，包括既有的 `session.updated`；标题等业务状态由上层应用按会话范围处理。
...
- 提供 `subscribeSessionRuntimeState()`，通过 fetch 订阅 `/api/internal/platform/opencode-runtime/sessions/runtime-state/events`，可携带 Bearer Token，解析 `session-runtime.snapshot` / `session-runtime.updated`。
...
已知事件包括旧 `run.*`、`assistant.message.delta`、`tool.*`、`diff.*`，以及 opencode Web App 运行态 `message.*`、`session.updated`、`session.status`、`todo.updated`、`permission.*`、`question.*`、`vcs.branch.updated`、`lsp.updated`、`mcp.tools.changed`。`session.updated` 沿用既有 wire name，旧消费方可忽略；客户端不新增 API、DTO 或 SDK 映射。transient live output 的 payload `seq=0` 且没有 SSE `id`，客户端必须依赖 payload `eventId` 去重。
