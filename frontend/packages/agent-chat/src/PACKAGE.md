...
- `opencode-like/components/rows/WorkStatusRow.vue`：主 Agent 工作状态块组合 reasoning、按需出现的事件图标行、可选 Todo 行、竖向流光和全宽悬浮工具详情；最新非成功轮投送到 composer Dock，成功完成态回到最后 assistant 输出下方并默认收为带通用 actions 插槽的可展开图标，历史轮仍收为单图标并可原位展开。
