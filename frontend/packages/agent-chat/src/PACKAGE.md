...
- `AssistantThread.vue`：对话线程视图，负责把消息、运行态、模型目录、Diff、分轮 Todo 快照和 Run Session scope 索引传入 `createOpencodeLikeState`，并在 composer 上方提供当前工作状态 Dock；子 Agent 视图隐藏 Dock、composer 和 runtime 控制，只展示返回主 Agent 的提示。
...
- `opencode-like/components/rows/WorkStatusRow.vue`：主 Agent 工作状态块组合 reasoning、按需出现的事件图标行、可选 Todo 行、竖向流光和全宽悬浮工具详情；最新轮投送到 composer Dock，成功完成态默认收为带通用 actions 插槽的可展开图标，历史轮仍收为单图标并可原位展开。
