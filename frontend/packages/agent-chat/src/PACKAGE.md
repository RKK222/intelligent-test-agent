...
- `ProcessDisclosure.vue`：存量折叠壳，仍被 `TaskBreakdown.vue` 等局部视图复用；不要用它恢复旧对话主路径。
- `ComposerArea.vue`、`RuntimeControls.vue`、`ChicPopover.vue`、`RuntimeDock.vue`、`SuggestionPanel.vue`、`TaskBreakdown.vue`：仍在当前 Agent 面板中使用的对话区子组件。
