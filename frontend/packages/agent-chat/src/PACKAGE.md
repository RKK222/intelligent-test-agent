...
- `opencode-like/`：opencode 风格时间线模块。`state/adapter.ts` 归并会话输入并按 `activeSubagentSessionId` 过滤主/子时间线，主视图会用保留的 subagent 索引补偿缺失的 task 入口；`state/projection.ts` 投影时间线行，task 子 Agent 卡片始终独立成行，不进入普通工具折叠组；`state/tool-registry.ts` 识别常见工具；`components/` 下按 row、part、tool、primitive 分层渲染，task 工具未绑定 child 时显示为“智能体 / 准备中”，绑定后渲染为可点击子 Agent 卡片，并包含输入框上方的 `TodoPanel.vue`；`styles/` 下集中维护 `.oc-*` token、布局、todo、markdown、diff 和动画样式。
