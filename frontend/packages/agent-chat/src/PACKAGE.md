...
- `runtime-reducer.ts`：归并 RunEvent 对话状态，将 opencode 重发的 user message/part 合并回当前 user 消息，并导出 `normalizeMessagePart` 供实时事件和历史 `partsJson` 共用同一套 opencode part 字段归一化规则。
