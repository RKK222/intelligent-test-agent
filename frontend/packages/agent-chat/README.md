...
- 用户消息已经进入时间线但尚无 assistant part 时，最新轮也会显示单行思考状态，但不会虚构 reasoning 正文或空事件行；状态依次使用“思考中 / 重试中 / 失败 / 已停止 / 已完成”。发送新消息后，上一轮在其最后一个 assistant 输出下方收为单图标，点击原位展开且同时只允许展开一个；新轮会关闭旧 reasoning、历史展开和工具气泡。最新运行或重试轮播放竖向 `ShimmerDivider`，完成轮显示静态渐变线。
