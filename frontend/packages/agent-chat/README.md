...
- 主 Agent 每个用户回合把真实 `reasoning` 与除 `task`、`question` 外的普通工具调用聚合为工作状态；最新轮固定投送到原 Todo 面板位置、输入框上方，初始只显示“思考状态”，首个事件出现后才增加事件图标行，存在 Todo 时再追加任务统计和展开列表。成功完成态自动收为可展开图标，并通过 `completed-status-actions` 插槽与业务操作同排；文件修改块始终紧随状态行或展开详情。`task` 子 Agent 卡片、`question` 问答卡片和进入后的子 Agent 时间线继续使用既有展示方式。
