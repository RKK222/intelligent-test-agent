...
- 主 Agent 每个用户回合把真实 `reasoning` 与除 `task`、`question` 外的普通工具调用聚合为工作状态；最新非成功轮固定投送到原 Todo 面板位置、输入框上方，初始只显示“思考状态”，首个事件出现后才增加事件图标行，存在 Todo 时再追加任务统计和展开列表。工作状态 dock 最高为 `min(360px, 50vh)`，长思考详情在 dock 内纵向滚动，不挤出输入区。每轮状态携带自己的 `runId/runStatus`，成功完成态移到最后一个 assistant 输出下方（无 assistant part 时紧跟用户消息）并自动收为可展开图标，通过带当前 `row` 参数的 `completed-status-actions` 插槽与该 Run 的反馈操作同排；所有成功历史 Run 保留入口。文件修改块始终固定在输入框上方，子 Agent 与问答展示不变。
...
- 工具视图统一使用 `.oc-*` primitives 和轻量折叠壳，工具详情默认折叠，过程行的标题、摘要、状态和展开箭头使用固定列对齐；最终文本直接以轻量气泡展示，不额外加“最终输出”标题，并保留复制按钮；复制统一复用 `@test-agent/ui-kit` 的兼容能力，在企业内 HTTP 与 Chromium 108 下不依赖安全上下文 Clipboard API；工作区内长绝对路径在列表中展示为面向用户的短路径，完整路径只保留在悬浮提示中，避免 `.testagent`/personal worktree 前缀撑开对话区域。
- 用户消息已经进入时间线但尚无 assistant part 时，最新轮也会显示单行思考状态，但不会虚构 reasoning 正文或空事件行；状态依次使用“思考中 / 重试中 / 失败 / 已停止 / 已完成”。发送新消息后，上一轮在其最后一个 assistant 输出下方收为单图标；点击后图标按钮保持在摘要行、状态块显示在其下方，再次点击同一按钮收起，且同时只允许展开一个历史状态。新轮会关闭旧 reasoning、历史展开和工具气泡。最新运行或重试轮播放竖向 `ShimmerDivider`，完成轮显示静态渐变线。
- 提供 Agent/Model/Mode selector、runtime status bar、slash command palette、`@` context picker、permission dock、question dock 和嵌入工作状态的 `TodoPanel`；question dock 只能由 RunEvent `question.asked` 归并出的 `QuestionRequest` 驱动。permission dock 与 Figma 权限卡共用展示转换：显式标题/说明优先，否则显示“需要权限”和 OpenCode 1.18.4 的中文权限说明；路径优先读取 `patterns[]`、回退 `pattern` 并按原顺序去空去重，未知权限不展示内部 type/requestId；卡片使用警告图标和“拒绝 / 始终允许 / 允许一次”按钮顺序。Todo 收起态展示各状态和总数，展开态展示任务、优先级与步骤；reducer 通过 Run ID 显式绑定用户轮次，root Todo 只更新所属轮次，child Todo 不进入 root 状态块；历史 `todowrite` part 只恢复对应用户轮次的快照。
...
- 已完成的 `text` 与 `reasoning` part 通过 `MarkdownView` 懒加载 markdown-it + DOMPurify + highlight.js 并注册 `@mermaid-js/layout-elk` 插件来渲染 Markdown、表格、链接和采用 ELK 自动布局的 Mermaid 流程图（首屏不打包）；running/pending `text` part 先以轻量纯文本 live preview 展示实际增长内容和"生成中"状态，完成后再切换为 Markdown 渲染。`MarkdownView` 在非空 source 重新渲染 pending 时保留已有 HTML，空白 `text` part 不进入主时间线，避免主/子 Agent 切换时挂载无内容的 Markdown 占位。
