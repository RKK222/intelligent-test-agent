...
- 提供 Agent/Model/Mode selector、runtime status bar、slash command palette、`@` context picker、permission dock、question dock 和嵌入工作状态的 `TodoPanel`；question dock 只能由 RunEvent `question.asked` 归并出的 `QuestionRequest` 驱动。permission dock 与 Figma 权限卡共用展示转换：显式标题/说明优先，否则显示“需要权限”和 OpenCode 1.17.8 的中文权限说明；路径优先读取 `patterns[]`、回退 `pattern` 并按原顺序去空去重，未知权限不展示内部 type/requestId；卡片使用警告图标和“拒绝 / 始终允许 / 允许一次”按钮顺序。Todo 收起态展示各状态和总数，展开态展示任务、优先级与步骤；reducer 通过 Run ID 显式绑定用户轮次，root Todo 只更新所属轮次，child Todo 不进入 root 状态块；历史 `todowrite` part 只恢复对应用户轮次的快照。
