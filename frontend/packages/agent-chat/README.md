...
- 提供纯 RunEvent reducer，把 `message.part.delta`、permission/question、todo、diff 等事件归并为对话展示状态；opencode 重发的远端 user message/part 会合并回当前乐观 user 消息，不会误建 assistant 消息或把提示词拼入回答；导出的 `normalizeMessagePart` 统一兼容 opencode `id/tool/state.input/metadata/status/time` 原始结构和平台规范化结构，供实时事件与历史 `partsJson` 恢复共同复用。
