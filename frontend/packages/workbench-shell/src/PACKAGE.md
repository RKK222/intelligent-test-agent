...
- `workbenchStore.ts`：Pinia 工作台状态，包括打开文件 tab、Agent public/workspace worktree、公共直接配置目录服务器记忆，以及 Git 变更面板的应用工作区与应用级 Agent/Skill mock 数据；文件 tab 保存加载三态、渐进预览字节进度/快照、错误、稳定快照身份和用户内容修订代次，`updateTab` 允许后台响应更新自己的 tab 而不改变当前活动文件或用户修订代次。
