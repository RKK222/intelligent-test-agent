...
- 登录进入工作台后独立查询当前用户 opencode 进程状态，并在页面可见时通过既有 `/processes/me` 自动重新探测：状态未 `READY` 时每 5 秒探测，`READY` 后降为每 30 秒探测，避免用户必须点击头像或刷新页面才能看到健康状态变化，同时降低常态 manager health 压力。状态非 `READY` 时禁用发送、新建对话和 busy follow-up 出队，并在右侧对话面板展示初始化按钮或不可用原因。右侧输入卡片被点击或输入框获得焦点时仍会立即触发一次主动重新探测，刷新中保持输入可编辑但暂时禁用发送和新建对话，避免旧的 `READY` 缓存放行已退出进程。Agent/Provider/Model/MCP/LSP/VCS 等运行态目录只在进程 `READY` 后查询，并在状态刚变为 `READY` 时主动刷新，避免早期健康检查失败后模型列表停留在空缓存。初始化动作只调用平台后端 `initializeMyOpencodeProcess()`，不直连 opencode server 或容器管理进程。
