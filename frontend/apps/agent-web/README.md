...
- 底部 footer 的 VCS 分支按钮改为两级菜单：一级菜单按"来源"分组（当前分支 / 默认分支 / 最近使用 / 其他分支），二级菜单展示该组下的分支名；分组键值由 `WorkbenchFooter` 的 `BranchGroupKey` 暴露（`current` / `default` / `recent` / `other`），数据由 `AgentWorkbench` 从 `vcs.status`（`branch` + `defaultBranch`）、`getRecentBranch` 偏好以及 `branches` prop 派生后通过 `defaultBranch` / `recentBranch` 注入；二级菜单的 hover 显隐走 `hoveredBranchGroup` 状态 + 200ms 关闭延迟，避免从一级菜单跨到子菜单时闪烁；hover 出的分支项支持键盘 `Esc` 收起，外部点击通过 `branchMenuRef.contains` 关闭。
