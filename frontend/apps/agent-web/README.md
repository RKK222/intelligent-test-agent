...
- 右上角应用菜单优先展示当前用户加入的应用；管理员未加入任何应用时回退展示启用应用，切换应用后优先切到应用最近使用工作区，没有最近工作区时清空旧文件树并提示选择工作空间；应用名旁边提供用户头像菜单，可退出登录并切换用户；未选择应用时不自动读取普通工作区文件树，避免应用与左侧工作区错位。
...
- 底部 footer 的 VCS 分支按钮改为两级菜单：一级菜单按"来源"分组（当前分支 / 默认分支 / 最近使用 / 其他分支），二级菜单展示该组下的分支名；分组键值由 `WorkbenchFooter` 的 `BranchGroupKey` 暴露（`current` / `default` / `recent` / `other`），数据由 `AgentWorkbench` 从 `vcs.status`（`branch` + `defaultBranch`）、`getRecentBranch` 偏好以及 `branches` prop 派生后通过 `defaultBranch` / `recentBranch` 注入；二级菜单的 hover 显隐走 `hoveredBranchGroup` 状态 + 200ms 关闭延迟，避免从一级菜单跨到子菜单时闪烁；hover 出的分支项支持键盘 `Esc` 收起。菜单本体用 `<Teleport to="body">` + `position: fixed` 挂到 body 上，坐标由 `updateBranchMenuPos()` 在打开前基于 `branchButtonRef.getBoundingClientRect()` 计算，并用 `requestAnimationFrame` 节流同步 scroll/resize 事件，避免被父级 dockview 面板的 `overflow: hidden` 裁切。
