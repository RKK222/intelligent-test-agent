...
- 右上角应用菜单优先展示当前用户加入的应用；管理员未加入任何应用时回退展示启用应用，切换应用后优先切到应用最近使用工作区，没有最近工作区时清空旧文件树并提示选择工作空间；应用名旁边提供用户头像菜单，可退出登录并切换用户，头像菜单每次打开都会实时查询 `/api/internal/agent/opencode/processes/me` 并用灰/绿/红展示“未分配 / 运行中(服务器ip:端口) / 未运行(服务器ip:端口)”；若状态查询失败且没有可用响应，头像菜单显示“状态未知”而不是误判为未分配。未选择应用时不自动读取普通工作区文件树，避免应用与左侧工作区错位；下拉菜单顶部以灰显行展示当前用户的角色中文名（来自 `/api/auth/me` 的 `roleLabels`，链路 `users` → `user_roles` → `dictionaries.dict_label`），缺失时整行不渲染。
- 应用目录加载完成且 `selectedAppId` 为空时，前端 `trySelectDefaultApp` 先等 `GET /recent-workspace`（`WorkspaceRuntimeResponse.appId` / `versionId` / `applicationWorkspaceId` 仅在最近工作区接口中填充）返回上次的全局最近工作区并解析出 `appId`，再以该 `appId` 调用 `handleSelectApp`，从而在重新登录 / 换电脑登录时自动还原「上次所在的应用 + 上次使用的工作空间版本」。应用级 recent 命中带 `versionId` 的工作区时，`pickDefaultWorkspaceForApp` 会先调用 `ensureDefaultPersonalWorkspace(versionId)`，统一切到用户 default 私人 worktree 后再加载文件树，避免首页直接展示或修改应用版本副本。`handleSelectApp → applyManagedWorkspace → switchWorkspace → syncCurrentVersionFromWorkspace` 链路上，`applyManagedWorkspace` 会把 `POST /workspaces/{workspaceId}/recent` 响应中的 `appId` / `versionId` / `applicationWorkspaceId` 通过 `mergeRecentRuntimeResponse` 合并到本次切到的工作区上（这是「切会话」「兜底选择首模板首版本」等路径上 `workspace.versionId` 缺失时的关键回填）；`syncCurrentVersionFromWorkspace` 优先用 `workspace.versionId` 直接设回 `currentVersionFromWorkspace`，并按 `workspace.applicationWorkspaceId` 触发对应模板 `versions` 的预加载，使 `WorkbenchFooter.selectedTemplate` 立即找到匹配、左下角"切换工作空间"按钮立刻显示 `模板 / 版本` 名；`versionId` 缺失时回退到原 `versionsByTemplateId` 精确匹配逻辑以兼容旧数据。`appId` 不在当前账号的应用目录（应用被禁用 / 切换租户）或接口失败时，降级到 `apps[0]`，由 per-app recent / 首模板首版本兜底；空目录则保持空态。
...
- 左侧文件区新增 `Agent` tab，与工作区/公共目录平级；固定展示“公共级”和“工作空间级”两个根目录，普通工作空间文件树过滤根级 `.opencode` 以避免重复。普通用户只读打开 agent 文件，`SUPER_ADMIN` 可更新公共配置、创建 worktree、切换公共 worktree、编辑文件、查看 Agent Git diff、双击文件 stage、提交和发布；公共仓库有未提交修改时仍加载文件树，更新弹窗要求明确勾选放弃本地修改后才传 `discardLocalChanges=true`。工作空间级 `+` 只生成符合 opencode `SKILL.md` frontmatter 约定的应用技能包：`skills/<name>/SKILL.md`、`rules/README.md`、`templates/README.md`，不自动生成 Agent。公共级切换先选择已初始化服务器，再选择“直接公共配置目录”或该服务器上的 `ACTIVE` worktree，切换后只清空公共文件树缓存并改变 `worktreeId/linuxServerId` 上下文。Agent 配置文件目录列表、读取和写入均通过 backend-api 的 agent-config 文件 WebSocket route/ticket/RPC，公共直接模式会选择已初始化公共配置服务器并把 `linuxServerId` 随打开的 tab 保存，worktree 模式按落库服务器路由；已打开 tab 不随切换关闭，保存时沿用 tab 内的上下文。创建公共 worktree 弹窗会实时读取远端分支和在线服务器公共配置仓库状态，后端和前端均按 `linuxServerId` 合并同 IP 在线 Java 快照，避免 Java 进程重启窗口内出现重复服务器行；只允许选择已初始化服务器并提交 `linuxServerId`，无已初始化服务器时禁用确认并引导到系统管理初始化。Git 长操作通过 `agent-config` ticket WebSocket 展示进度，不走 RunEvent SSE。Git 远端失败时优先展示后端 `gitFailureHint` 和 `traceId`，不把原始 stderr、完整命令或内部路径直接展示到 UI。
- `SUPER_ADMIN` 用户在文件区底部工作空间切换控件右侧可见服务器切换图标按钮；点击后先选择后端服务器 IP，再通过目标后端 WebSocket 从该 Java 进程运行目录开始浏览目录并添加工作空间。选择器顶部明确展示“将作为工作空间选择”的当前目录，完整展示目录名并换行展示完整路径；下面列表仅表示可进入的子目录，避免误把子目录列表理解为已选目录内容。选中服务器与当前 agent 不同服务器时，目录输入和确认按钮禁用，并提示“工作空间与 agent 不在同一服务器”。
- 点击历史会话时会按 `session.workspaceId` 切换运行态 Workspace；目标工作区不可用或无权限时保留 transcript 并禁用输入，作为只读会话展示。
- 组合运行态能力：Agent/Model/Mode 选择、按 Provider 分组的模型选择面板、slash command、`@` context、MCP/LSP/VCS 状态、Run/Session/VCS Diff 来源切换。VCS Diff 来源和左侧 Git 变更面板读取平台 `getWorkspaceGitDiff()`，基于当前 default 私人 worktree 的本地 Git 状态，不再依赖 opencode `/vcs/diff`；普通工作区文件保存成功后会静默刷新这份 Git diff，让文件树变更标记和 Diff 面板保持同步。
- 登录进入工作台后独立查询当前用户 opencode 进程状态，并在页面可见时通过既有 `/processes/me` 自动重新探测：状态未 `READY` 时每 5 秒探测，`READY` 后降为每 30 秒探测，避免用户必须点击头像或刷新页面才能看到健康状态变化，同时降低常态 manager health 压力。状态非 `READY` 时禁用发送、新建对话和 busy follow-up 出队，并在右侧对话面板展示初始化按钮或不可用原因。右侧输入卡片被点击或输入框获得焦点时仍会立即触发一次主动重新探测，刷新中保持输入可编辑但暂时禁用发送和新建对话，避免旧的 `READY` 缓存放行已退出进程。Agent/Provider/Model/MCP/LSP/VCS 等运行态目录只在进程 `READY` 后查询，并在状态刚转为 `READY` 时主动刷新，避免早期健康检查失败后模型列表停留在空缓存。初始化动作只调用平台后端 `initializeMyOpencodeProcess()`，不直连 opencode server 或容器管理进程；初始化请求若收到短暂失败，前端会立即复查 `/processes/me`，复查已 `READY` 时以可用状态为准，不继续展示旧的创建失败提示。
- 登录进入工作台后独立查询当前用户 opencode 进程状态；状态非 `READY` 时禁用发送、新建对话和 busy follow-up 出队，并在右侧对话面板展示初始化按钮或不可用原因。右侧对话面板状态卡与发送拦截反馈均按后端 `serviceStatus` 二级状态区分：`UNASSIGNED`（尚未分配）显示"尚未分配 opencode 专属进程"、按钮"分配专属进程"；`NOT_RUNNING`（已分配未运行或健康不可确认）显示"opencode 专属进程未运行"、按钮"启动进程"，并优先展示后端返回的 `serviceAddress`；与头像菜单的回退推断保持一致。两个按钮都调用 `initializeMyOpencodeProcess()`，由后端 `initialize()` 统一处理分配并启动或重建并启动。右侧输入卡片被点击或输入框获得焦点时会通过既有 `/processes/me` 查询重新探测进程状态，刷新中保持输入可编辑但暂时禁用发送和新建对话，避免旧的 `READY` 缓存放行已退出进程。初始化动作只调用平台后端 `initializeMyOpencodeProcess()`，不直连 opencode server 或容器管理进程。
...
- 左侧 Git 变更面板的“加载测试数据”只填充 mock：`应用工作空间` 分组展示应用源码/测试文件变更，`agents` 分组展示应用级 opencode `agents/*.md` 与 `skills/<skill>/SKILL.md` 变更；若真实刷新尚未结束即点击测试数据，旧请求结果不会覆盖 mock 列表。
