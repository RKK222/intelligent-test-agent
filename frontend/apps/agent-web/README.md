...
- 应用目录加载完成且 `selectedAppId` 为空时，前端 `trySelectDefaultApp` 先等 `GET /recent-workspace`（`WorkspaceRuntimeResponse.appId` 仅在全局接口填充）返回上次的全局最近工作区并解析出 `appId`，再以该 `appId` 调用 `handleSelectApp`，从而在重新登录 / 换电脑登录时自动还原「上次所在的应用 + 上次使用的工作空间版本」。该 `appId` 不在当前账号的应用目录（应用被禁用 / 切换租户）或接口失败时，降级到 `apps[0]`，由 per-app recent / 首模板首版本兜底；空目录则保持空态。
