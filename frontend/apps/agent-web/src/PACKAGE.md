...
- `components/AgentConfigPanel.vue`：展示公共级/应用级 Agent 配置树；具备应用配置写权限时，应用 Agent/Skill 文件节点复用普通文件树的双击行内改名交互并调用专用 Agent 配置重命名 RPC，成功后同步已打开标签和 Git Changes；公共级和无权限应用级保持只读。左侧根节点保留 Agent 配置更新按钮，公共/应用动作组统一布局且刷新图标固定在最右侧；宠物浮层仅在对话页提供同一能力，选择页不展示，均按权限显示并复用工作台运行态处理。
