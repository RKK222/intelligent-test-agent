...
- `components/AgentConfigPanel.vue`：展示公共级/应用级 Agent 配置树；公共/应用根和可写目录复用 `file-explorer` 共享新建面板，文件/目录行复用共享删除确认面板。文件夹以 `.gitkeep` 保证 Git 可追踪，应用级创建和删除受 `opencode.jsonc`、`agents/**`、`skills/**` Diff 白名单约束；具备应用配置写权限时，文件节点复用普通文件树的双击行内改名交互并调用专用 Agent 配置重命名 RPC，公共级和无权限应用级保持只读；创建、改名或删除成功只上报修订信号，由工作台复用既有 Git Changes 刷新，删除目录时同步清理子树缓存和相关 Agent 标签。左侧根节点保留 Agent 配置更新按钮，公共/应用动作组统一布局且刷新图标固定在最右侧；宠物浮层仅在对话页提供同一能力，选择页不展示，均按权限显示并复用工作台运行态处理。
