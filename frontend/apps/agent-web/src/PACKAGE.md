...
- `FigmaChatPanel` 会话列表交互：主对话不再显示“对话 / 待执行任务”页签；header 的“会话列表”通过 Teleport 在对话栏左侧展示最大 360px 的非模态浮层，窄屏退化为视口内覆盖。浮层内部提供“会话 / 待执行任务”页签，选择会话或任务后保持打开并高亮当前会话，只由关闭按钮、Esc 或 `panelVisible=false` 关闭。
