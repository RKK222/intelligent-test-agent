...
- `components/AgentWorkbench.vue`：组合 workspace、应用切换、用户头像退出、系统管理入口、应用版本/个人工作区切换与同步、文件树、编辑器、Agent、RunEvent SSE、Session History 搜索/置顶/删除、历史会话只读态、follow-up 队列、编辑器选区上下文、Diff 操作和底部 PTY terminal panel；普通消息和 slash 技能统一创建平台 Run，启动请求 pending、页面刷新或切换历史会话时通过 `getActiveRun(sessionId)` 接管 `PENDING/RUNNING/CANCELLING` Run 并订阅 SSE，模型/Provider 选择作为用户级偏好持久化，不随工作区切换清空；同时维护当前页面生命周期内的会话级原始报文内存缓存，只收集会话创建、Run 启动/取消、active-run、消息加载、permission/question 回复和 RunEvent SSE。
...
- `components/GitChangesPanel.vue`：应用工作区发布白名单、远端 push 确认和冲突编排；冲突内容由 backend-api 读取，复用 `diff-viewer/MergeConflictEditor` 完成三方编辑、标记解决和取消 merge。
- `components/FigmaChatPanel.vue`：Figma 风格右侧对话面板，按顺序渲染完整历史用户/助手消息；header 提供“原始输出”悬浮窗，使用 `<pre>` 展示前端捕获的请求体、响应体和 SSE `data` 原文，支持拖动、CSS resize、筛选和清空当前会话记录；输入框处理输入法组合输入，避免候选词确认 Enter 被误识别为发送；输入卡片交互会请求工作台重新探测 opencode 进程状态，刷新中保持输入但暂时阻止提交；上传附件按钮当前只打开前端样式弹窗，不接后台上传。
