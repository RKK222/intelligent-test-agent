...
directoryMapping:
  ...
  agentDesignRows:
    ...
    -
      scope: "个人过程资产"
      role: "spec/**"
      description: "物理文件位于应用 Git 的个人分支；所有应用成员都可本地提交，任何角色都不能发布。"
  ownershipRows:
    -
    ...
    -
      ...
      publisher: "仅个人 worktree 本地提交，禁止发布"
    -
...
| 应用 Git | 业务文件、`docs/**`、`archive/**`、`.opencode/**`，以及个人分支中的 `spec/**` | 普通文件和 `docs/**` 所有应用成员可维护；应用配置仅应用管理员及以上；`spec/**` 禁止发布 |
...
- `spec/**` 是个人过程资产，可以编辑、暂存和提交到个人分支，但任何角色都不能发布。生成结果按平台现有流程另行同步到系统。
...
如果一次选择同时包含 `spec/**` 和可发布文件，`spec/**` 只进入个人提交，其它文件继续发布；如果只选择 `spec/**`，流程停在个人提交。后端也会拒绝直接 API、`./spec/**` 路径别名或旧同步接口尝试发布 spec。
