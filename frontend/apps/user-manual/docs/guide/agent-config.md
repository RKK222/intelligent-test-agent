...
超级管理员可在公共级根目录或任意目录行点击“新建文件或文件夹”，也可通过文件/目录行尾删除按钮进入确认面板。两个入口都复用上方工作空间文件树的交互；新文件创建后会直接在编辑器打开，新文件夹会包含 `.gitkeep`，从而能出现在“公共Agent”Git 变更中。删除目录会递归删除其中全部内容，确认后相关已打开标签会关闭。
...
应用管理员也可在应用级根或 `agents/`、`skills/` 的任意可写目录行点击“新建文件或文件夹”，并通过文件/目录行尾删除按钮移除条目。应用级只允许创建或删除 `opencode.jsonc`、`agents/**` 和 `skills/**` 范围内的条目；空文件夹用 `.gitkeep` 保持可提交，删除文件夹会递归删除全部内容。操作成功后文件树和 Git 变更会自动刷新，不需要手工点击“变更”或刷新按钮。

应用管理员或超级管理员可以双击应用级 Agent/Skill 文件名直接改名，按 Enter 或移开焦点提交，按 Esc 取消；改名后文件树、已打开标签和 Git 变更会同步更新。普通成员的公共级与应用级配置均为只读，不会出现行内输入；公共级当前也不提供文件名改名。
