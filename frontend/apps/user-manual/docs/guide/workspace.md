...
| `spec/**` | 所有应用成员都可编辑、暂存和本地提交 | 普通成员和应用管理员只保留个人提交；超级管理员可投影并推送 |
...
普通成员或应用管理员同时选择 `spec/**` 和可发布文件时，平台会先把它们一起提交到个人 `HEAD`，再只发布非 `spec/**` 文件；仅选择 `spec/**` 时，操作停在个人提交，不会发起远端发布。超级管理员不受该限制。发布成功后平台会广播版本同步通知；其他在线用户需要手动刷新或同步，系统不会自动覆盖存在本地修改的工作树。
