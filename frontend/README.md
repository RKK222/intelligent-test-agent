...
应用管理员或超级管理员在已选择应用和个人运行态工作区时，可从工作区切换按钮后的“引用配置”入口初始化、同步或经二次确认切换应用资产库分支，并在双栏弹窗选择后端标记的橙色 SDD 根目录。页面以目标分支/HEAD 对照每台服务器实际 branch/HEAD、在线状态、匹配结果、最近同步和最近核验时间；后端缺少兼容字段时显示“在线状态未知/未核验”，不根据指针自行推断可信状态。“刷新 Git 指针”只发起只读核验。活动状态每 2 秒轮询；保存前重新读取当前个人工作区 `.opencode/opencode.jsonc`，再用 `jsonc-parser` 最小更新目标 `references` 节点，文件读写继续走 workspace 文件 WebSocket RPC。保存或分支切换完成后工作台刷新组合文件树：`merge=true` 按 `sdd-folder-name` 合并到工作区一级目录，纯引用节点显示蓝色、工作区已有同名目录保持普通颜色；`merge=false` 以参考别名显示只读一级目录。引用文件使用独立稳定身份和只读编辑 tab，不进入工作区搜索、Git 变更或 `requirements` 目录。已有用户 opencode 进程不会因保存或同步自动重启，引用目录环境在下次启动或受管重启生效。

引用配置的已选仓库标题会在“刷新 Git 指针”左侧展示当前服务器规范化仓库路径；旧后端、缺少引用根参数或历史非法英文名时显示“服务器路径暂不可用”。点击刷新后先打开“创建任务、逐服务器核验、汇总结果”三阶段弹层再提交核验请求，执行期间锁定父弹层和键盘焦点，逐服务器显示等待、处理、重试、失败或离线延后，终态保留到用户手动关闭；2 秒状态轮询临时失败会自动继续。
...
工作台左侧 Agent 配置树展示公共级 `opencode/` 和工作空间级 `.opencode/` 配置根。超级管理员进入公共级时，前端在所选已初始化服务器上自动挂载当前用户的长期本地分支/worktree；“更多操作”提供显式创建和切换入口，创建只确保当前用户稳定的 `public-{userId}` 分支/worktree。公共分支不包含应用版本，不能任意命名、切换他人 worktree，也不能回退到共享目录直接编辑。普通用户仍从共享运行时副本只读查看 `agents/` 与 `skills/`。公共文件与应用文件统一进入左侧 Git 变更面板，但变更区按“应用工作空间 / 应用 Agents/Skills / 公共 Agents/Skills”切换作用域：应用工作空间承载普通文件、docs、spec，两个 Agent 作用域只展示各自仓库的 `agents/` 与 `skills/`，提交和推送只作用于当前作用域；三个作用域仍共用暂存、回退、提交、提交并推送、进度弹框和冲突处理交互。应用和公共 Agent 的已跟踪、新增及未跟踪文件均可逐个或批量回退；冲突文件不显示普通回退，必须继续使用三方合并或取消合并。公共推送冲突保留在个人 worktree，可统一选择保留本地、保留远程、手工合并或取消。Agent 文件保存成功会立即刷新 Git Changes，不要求用户再点变更面板刷新按钮；后端确认提交成功后清空提交说明并更新本轮结果。刷新 Agent 配置时会使旧目录请求失效、重载展开目录，并从磁盘重读没有未保存修改的活动文件；显式 Git 回退后则同步刷新或关闭对应已打开 Agent tab。
