...
- Model/Provider 目录由 `packages/backend-api` 读取 opencode 实例级 `/config` 合并有效配置；`enabled_providers` 非空时按 Provider ID 同时过滤两类目录，企业包只展示企业白名单 Provider 下的全部模型，不再混入 OpenCode Zen。未声明白名单或配置暂时不可读时保持原生目录兼容。
