...
- 右侧会话 footer 最左侧在真实根 Session 建立后显示 16px 上下文使用率圆环，悬浮只展示使用率、当前模型总上下文和已使用量；点击后在右侧栏 header/footer 之间打开详情覆盖层，展示会话、供应商、模型、根消息数、限制及总/输入/输出 Token，并以系统、用户、助手、工具、其他五类堆叠条估算最近输入上下文。用量取最近一条有效根 assistant 快照的 input/output/reasoning/cache read/cache write 之和，限制始终跟随当前所选模型；子 Agent 视图隐藏该入口。该能力只复用模型目录、历史消息和 RunEvent 数据，不新增 API、事件或持久化字段。
