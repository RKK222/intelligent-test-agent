...
- 企业内浏览器基线按 Chromium 108 兼容，`agent-web` 生产构建显式使用 `build.target=chrome108` 和 `build.cssTarget=chrome108`；涉及 Web Crypto、Clipboard 等安全上下文 API 时必须先做能力检测，内网 HTTP 访问不能直接假设这些 API 可用。
