...
应用工作区暂存通过平台 API 操作真实 Git index，同时仍作为发布文件白名单；未暂存与已暂存分组对称提供批量入口：未暂存区可一次暂存全部普通文件，已暂存区可一次取消暂存全部普通文件，两个分组都可经二次确认后一次回退全部已暂存和未暂存普通文件。后端普通发布会隔离历史 index。冲突期间批量暂存和批量回退入口禁用，批量取消暂存及普通文件逐个 stage/unstage 仍可使用，但按 Git 原生规则禁止提交；冲突文件可在 Monaco 三方合并编辑器中可靠选择结果、保存或取消整次 merge。前端只有在后端明确返回 `remotePushed=true` 时才展示提交并推送成功。
