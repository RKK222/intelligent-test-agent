...
`packages/editor` 在 Markdown 预览中支持 Mermaid `flowchart`/`graph` 与 `sequenceDiagram` 可视化编辑。Flowchart 提供按“流程图 / 文档与显示”分组的 14 类共享 SVG 节点、轮廓分配的 8/12 个端口和不随画布缩放、可在视口边缘翻转的双列快捷建连菜单；节点无论是否选中都可直接从可见连接点拖出连线，选中节点的连接点外围继续用于移动节点，选中连线可拖动绿色端点更换起止锚点。读取兼容旧语法，应用时统一规范化为现代 `ID@{ shape: <短名>, label: "<文本>" }`。图模型、parser、serializer 与 Vue Flow 画布均留在 editor 包内；应用后只回写当前 Markdown fence，并继续复用工作台 dirty、Git Diff 与 workspace 文件保存链路。
...
工作台左侧 Agent 配置树展示公共级 `opencode/` 和工作空间级 `.opencode/` 配置根。超级管理员进入公共级时，前端在所选已初始化服务器上自动挂载当前用户的长期本地分支/worktree；公共分支不包含应用版本，不能切换他人 worktree，也不能回退到共享目录直接编辑。普通用户仍从共享运行时副本只读查看 `agents/` 与 `skills/`。公共文件与应用文件统一进入左侧 Git 变更面板，但变更区按“应用工作空间 / 应用 Agents/Skills / 公共 Agents/Skills”切换作用域：应用工作空间承载普通文件、docs、spec，两个 Agent 作用域只展示各自仓库的 `agents/` 与 `skills/`，提交和推送只作用于当前作用域；三个作用域仍共用暂存、提交、提交并推送、进度弹框和冲突处理交互。公共推送冲突保留在个人 worktree，可统一选择保留本地、保留远程、手工合并或取消。刷新 Agent 配置时会使旧目录请求失效、重载展开目录，并从磁盘重读没有未保存修改的活动文件。
