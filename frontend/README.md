...
工作台左侧 Agent 配置树展示公共级 `opencode/` 和工作空间级 `.opencode/` 配置根。超级管理员进入公共级时，前端在所选已初始化服务器上自动挂载当前用户的长期本地分支/worktree；公共分支不包含应用版本，不能切换他人 worktree，也不能回退到共享目录直接编辑。普通用户仍从共享运行时副本只读查看 `agents/` 与 `skills/`。公共文件与应用文件统一进入左侧 Git 变更面板，公共文件标记 `[公共]`，使用相同的暂存、提交、提交并推送、进度弹框和冲突处理交互；公共推送冲突保留在个人 worktree，可统一选择保留本地、保留远程、手工合并或取消。刷新 Agent 配置时会使旧目录请求失效、重载展开目录，并从磁盘重读没有未保存修改的活动文件。
