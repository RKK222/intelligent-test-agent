...
公共 Agent/Skill 更新或发布进入存量 Session 排空期时，现有 `/processes/me` 轮询按当前用户返回 `messageSendAllowed=false` 和阻断原因；该用户旧 opencode target dispose 后会立即恢复为 true，不等待其他用户。聊天面板禁用发送与新会话按钮、输入框展示排空提示，父级 `handleSend` 再次拦截；字段缺失按允许处理以兼容旧后端。前端状态只负责即时反馈，后端 Run 入口仍以同一持久化用户级门禁为准。
