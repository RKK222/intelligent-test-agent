...
- RunEvent SSE 只能通过 `packages/event-stream-client`；默认使用 `/api/internal/agent/opencode/runs/{runId}/events`；刷新或重进运行中会话时先通过 `backend-api.getActiveRun(sessionId)` 查询非终态 Run，再恢复 SSE 订阅；前端只把事件应用到当前订阅且仍为页面活动态的 Run，旧订阅关闭窗口内晚到的终态不会覆盖新一轮状态。
