...
工作台左侧 Agent 配置树展示公共级 `opencode/` 和工作空间级 `.opencode/` 配置根。超级管理员进入公共级时，前端在所选已初始化服务器上自动挂载当前用户的长期本地分支/worktree；“更多操作”提供显式创建和切换入口，创建只确保当前用户稳定的 `public-{userId}` 分支/worktree。公共分支不包含应用版本，不能任意命名、切换他人 worktree，也不能回退到共享目录直接编辑。普通用户仍从共享运行时副本只读查看 `agents/` 与 `skills/`。公共文件与应用文件统一进入左侧 Git 变更面板，但变更区按“应用工作空间 / 应用 Agents/Skills / 公共 Agents/Skills”切换作用域：应用工作空间承载普通文件、docs、spec；应用 Agent 作用域精确包含 `.opencode/opencode.jsonc`、`agents/**` 与 `skills/**`，公共 Agent 作用域包含公共仓库的 `agents/**` 与 `skills/**`，提交和推送只作用于当前作用域。三个作用域仍共用暂存、回退、提交、提交并推送、进度弹框和冲突处理交互。应用和公共 Agent 的已跟踪、新增及未跟踪文件均可逐个或批量回退；冲突文件不显示普通回退，必须继续使用三方合并或取消合并。公共推送冲突保留在个人 worktree，可统一选择保留本地、保留远程、手工合并或取消。Agent 文件保存成功会立即刷新 Git Changes；进入变更面板时再立即核验一次，并在面板可见期间每 5 秒复用同一查询持续感知磁盘变化，不要求用户点击刷新按钮。后端确认提交成功后清空提交说明并更新本轮结果。刷新 Agent 配置时会使旧目录请求失效、重载展开目录，并从磁盘重读没有未保存修改的活动文件；显式 Git 回退后则同步刷新或关闭对应已打开 Agent tab。
