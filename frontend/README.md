...
Agents 配置树的公共级、应用级根和可写目录复用工作空间 `FileEntryCreateDialog` 新建文件或文件夹，文件与目录行复用 `FileEntryDeleteDialog` 确认递归删除；新建文件夹写入 `.gitkeep`，让 Git Changes 能感知空目录。应用级创建和删除限制在 `opencode.jsonc`、`agents/**` 和 `skills/**` Diff 白名单内；成功后沿用 Agent 配置修订号刷新现有 Diff 查询，并在删除时关闭受影响的已打开标签。具备应用配置写权限的用户还可双击应用级文件名行内改名，复用既有文件重命名服务；公共级与无权限应用级保持只读。
