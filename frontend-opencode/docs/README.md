...
- `src/components/PromptComposer.vue`：复刻 opencode composer 的文本、附件、@ 上下文、Agent/Model 运行态选择、shell mode 和 slash command 入口；附件/@ 文件选择通过平台 fs catalog，context chip 可移除，slash 菜单通过平台命令目录写入 `/command` 文本。
