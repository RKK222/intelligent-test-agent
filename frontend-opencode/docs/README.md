...
- Playwright mock E2E 覆盖桌面/移动端 shell、首页会话列表、命令面板入口、会话详情加载、prompt 提交请求构造、toolbar abort、Session share publish/unpublish，以及通过 fake EventSource 注入 RunEvent 后的 timeline 流式渲染。
