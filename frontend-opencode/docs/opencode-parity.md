...
| Prompt parts：text/file/image/agent/reference/slash command | 已实现构造与单测；composer slash 菜单从平台 command catalog 选择命令并写入 `/command` 文本。 |
...
- Prompt composer slash command 已走平台命令目录；完整 opencode 参数表单、命令参数补全和键盘导航仍需继续补齐。
