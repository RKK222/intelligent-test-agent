...
| Provider auth/config/worktree/share/MCP auth API | 后端平台代理已补齐；Settings 已接入 provider auth 状态、API key 保存/移除、worktree 列表/创建/重置/删除，以及 MCP status/auth/remove auth 入口。 |
...
- 真实 provider OAuth、MCP auth、worktree、share 依赖可用 opencode runtime 环境；worktree UI 已走平台代理，仍需真实 Git runtime 验证创建/重置/删除结果。
