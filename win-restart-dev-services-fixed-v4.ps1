...
Notes:
  opencode-manager 编译失败（例如源码包含仅 Unix 的 syscall）时，脚本会打印警告但不会停止；
  后端与前端继续按原计划启动，跳过 opencode-manager。修复源码后重新执行即可。
"@ | Write-Host
}
...
function Load-EnvFile {
    ...
    foreach ($rawLine in Get-Content -LiteralPath $File) {
        ...
        if ($value.Length -ge 2) {
            ...
            $isDoubleQuoted = ($firstChar -eq [char]34 -and $lastChar -eq [char]34)
            $isSingleQuoted = ($firstChar -eq [char]39 -and $lastChar -eq [char]39)
...
& 'COMMAND_PLACEHOLDER' ARGUMENTS_PLACEHOLDER >> 'LOG_PATH_PLACEHOLDER' 2>> 'ERROR_LOG_PATH_PLACEHOLDER'
'@
    ...
    $script = $script.Replace("ARGUMENTS_PLACEHOLDER", $argumentText)
...
function Build-Backend {
    ...
    try {
        # 优先调用 Windows 上的真实 mvn.cmd，避免本机存在 mvn 函数/别名时吞掉 -D 参数。
        $mvnCommand = Get-Command "mvn.cmd" -ErrorAction SilentlyContinue
        if (-not $mvnCommand) {
            $mvnCommand = Get-Command "mvn" -ErrorAction Stop
        }

        # 用单引号字面量 + 参数数组，确保 Maven 收到完整的 -Dmaven.test.skip=true。
        $mvnExecutable = $mvnCommand.Source
        if ([string]::IsNullOrWhiteSpace($mvnExecutable)) {
            $mvnExecutable = $mvnCommand.Name
        }
        $mvnArgs = @('clean', 'package', '-Dmaven.test.skip=true')
        Write-Host ("Maven command: {0} ({1})" -f $mvnExecutable, $mvnCommand.CommandType)
...
function Start-OpencodeManager {
    ...
    if (-not $script:OpencodeManagerBuildSucceeded) {
        Write-Host "opencode-manager was not rebuilt this run (build failed earlier). Skipping start; backend and frontend remain up."
        return
    }
...
function Follow-ServiceLogs {
    # 把每个服务日志的最新若干行打印到当前窗口，然后阻塞在后台 tail，
    # 持续把新增内容合并输出，前缀带服务名；按 Ctrl+C 退出。
    $serviceNames = @("backend", "opencode-manager", "frontend")
    $logMap = [ordered]@{}
    ...
    if ($logMap.Count -eq 0) {
        Write-Host "No service log files found under $LogDir; nothing to follow."
        return
    }

    Write-Host ""
    Write-Host "=== Recent log tails ==="
    ...
    Write-Host ""
    Write-Host "=== Following service logs in this window (Ctrl+C to stop) ==="
    Write-Host ""

    # 每个日志一个后台 PowerShell 作业做 tail -f，主循环定期收取新增行。
    $jobs = @()
    foreach ($name in $logMap.Keys) {
        $logPath = $logMap[$name]
        ...
        } -ArgumentList $logPath, $name
    }

    # Ctrl+C 优雅退出：通过 Console.CancelKeyPress 不可用，改用 try/finally + Stop-Job 清理。
    try {
        while ($true) {
            $anyRunning = $false
            foreach ($job in @($jobs)) {
...
$resolvedTestAgentRoot = Get-EnvValue "TEST_AGENT_ROOT" ""
Write-Host ("TEST_AGENT_ROOT set to: {0}" -f $resolvedTestAgentRoot)
...
if (-not $script:OpencodeManagerBuildSucceeded) {
    Write-Host ""
    Write-Host "WARNING: opencode-manager did not build this run; it is NOT running." -ForegroundColor Yellow
