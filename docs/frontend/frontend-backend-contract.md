...
- 当前 UI 已提供 Agent/Provider/Model/Mode 选择、真实 Session history 切换、文件/图片附件、busy follow-up 本地 FIFO 队列、permission once/always/reject、question reply/reject、Todo 展示、slash command palette、`@` runtime context picker、Run/Session/VCS Diff 来源切换、MCP/LSP/VCS 状态摘要和 `/s/{sessionId}` 只读 transcript 页面；公开 share 授权、Diff 选区上下文、per-file/per-message 回滚和 PTY 仍按后续批次推进。
