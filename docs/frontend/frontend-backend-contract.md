...
3. 对重复事件做幂等保护。
4. 对缺失和未知事件给出可观测错误；重复事件必须优先按 `eventId` 幂等忽略，兼容旧事件时才回退 `runId + seq`。
...
5. 顶层 `frontend-opencode` 和 `opencode-source/opencode-1.17.8/packages/frontend-opencode` 不是平台前后端契约来源；除非后续正式纳入 workspace，否则不能替代 `frontend/`、`backend-api` 或 RunEvent SSE 契约。
