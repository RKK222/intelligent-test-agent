...
后端单独启动可用 `tools/dev-backend-run.sh [--profile test] [--env-file <path>]`；脚本只解析 `KEY=VALUE` 行，不执行 dotenv 内容。

## 8. 自检与提交

1. 完成前按 `docs/guides/self-checklist.md` 自检，并在回复中说明验证结果。
2. 自检通过后提交 git，commit message 使用中文并准确概括修改。
3. 只暂存本次任务相关文件，工作区无关改动保持原状。
