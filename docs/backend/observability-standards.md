...
当前 HTTP 入口使用请求/响应头 `X-Trace-Id`。合法 traceId 以 `trace_` 开头，只包含字母、数字、下划线和短横线；缺失或非法值由 `TraceIdSupport` 生成新值。`TraceIdWebFilter` 将 traceId 写入 WebExchange attribute、响应头、Reactor context 和 SLF4J MDC。
...
2. 后端运行态使用 Log4j2 作为 SLF4J 实际绑定，业务代码继续使用 SLF4J API。
3. 日志必须结构化，至少包含 traceId、模块、关键业务 ID 和结果。
4. `test-agent-app` 默认控制台日志使用 `key=value` 格式，包含 timestamp、level、traceId、module、logger、thread 和 message。
5. Log4j2 PatternLayout 必须对可变 message、thread 和 traceId 做 CRLF 编码，避免日志换行注入。
6. 不记录密钥、token、认证头、个人敏感信息和大段用户输入。
7. 热路径避免高频 info 日志，调试细节使用 debug。
