...
交互式 PTY 终端属于 Phase 11 P2 受控 WebSocket 例外，具体设计见 `docs/architecture/pty-websocket-design.md`。当前已落地后端 ticket、Origin、cwd workspace root 归一化、单次使用、每 session 单 active PTY、ticket 创建限流、input/resize 限速、审计、idle/hard timeout、输出截断和前端 terminal panel；真实三服务联调 E2E 尚无最新通过记录，仍需按该设计继续补齐。实现和后续扩展必须满足：
