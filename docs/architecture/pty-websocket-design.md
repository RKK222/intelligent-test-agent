...
本文档定义 Phase 11 P2 交互式 PTY 终端的架构和安全边界。未满足本文档的鉴权、限流、审计和隔离要求前，只允许展示 bash 工具输出卡片，不实现交互式 PTY。
...
- 后端 ticket controller、application service、WebSocket handler 和 PTY adapter 均有单元或集成测试。
- 安全测试覆盖 ticket 过期、重复使用、session/workspace 不匹配、cwd 越界、input 超限和 CORS/origin 拒绝。
- 前端测试覆盖 ticket 获取、连接失败、输出渲染、输入发送、resize 和 close。
