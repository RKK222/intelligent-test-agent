...
| `test-agent-api` | Controller、WebSocket 入口适配、请求/响应 DTO、统一异常、鉴权、限流、manager 控制面入口、超级管理员运行管理只读入口和 trace Web 入口。 |
...
| `packages/backend-api` | 访问平台后端服务的唯一前端 HTTP client，负责统一响应、错误、traceId、受控目录选择、用户 opencode 进程状态/初始化、运行管理 overview、配置管理、应用版本工作区 API 映射和默认 `opencode` 的 agent URL 前缀。 |
