...
| 包 | 职责 |
|---|---|
| `apps/agent-web` | 自研 Vue 3 + Vite 主应用，负责页面组合、Vue Query Provider、Pinia、工作空间选择、带超级管理员 root 终端视图的服务器工作空间选择、应用管理员引用配置双栏/2 秒状态轮询/JSONC 最小补丁、工作区与引用目录组合文件树（合并引用蓝色、冲突红色、只读 tab 和局部告警）、带上下文/路径请求代次和 dirty 修订保护的普通文件及公共级/应用级 Agent 文件加载编排、Agent 保存后的 Git Changes 修订刷新、用户 opencode 进程状态提示/初始化入口、Run 启动、SSE 订阅编排、后台运行会话历史计数/铃铛提醒（历史按钮数字只统计第一页 30 条中的未完成会话）、会话级前端原始报文内存查看器、五种 SVG 宠物的本地轮换/随机/固定选择和一次性旁路问答、设置模态（含版本库英文名、版本库类型、工作空间创建进度和用户管理页签，用户管理支持查询、创建测试用户和超管直接调角色）、超级管理员系统管理容器（定时任务管理 + 运行管理最新指标与 ECharts 趋势）和全局错误提示。 |
