...
| `test-agent-opencode-runtime` | Session、Run、RunEvent 编排、通过 `AgentRuntimeRegistry` 调用 agent、Diff/revert、terminal ticket/PTY 业务。 |
...
| 包 | 职责 |
|---|---|
| `apps/agent-web` | 自研 Vue 3 + Vite 主应用，负责页面组合、Vue Query Provider、Pinia、工作空间选择、Run 启动、SSE 订阅编排、设置模态和全局错误提示。 |
| `packages/backend-api` | 访问平台后端服务的唯一前端 HTTP client，负责统一响应、错误、traceId、受控目录选择、配置管理、应用版本工作区 API 映射和默认 `opencode` 的 agent URL 前缀。 |
