...
Phase 05 提供可重复的本地开发和联调入口，让后端、PostgreSQL、可选 Redis 和本地 opencode server 节点能按文档启动、检查和排障。仓库不保存真实密钥、token、数据库密码或生产地址。

## 配置分层

- `application.yml` 保存跨环境默认值：服务端口、Druid 默认池大小、Actuator 暴露范围、Runtime API 默认限制。
- `application-local.yml` 面向本地 Compose：PostgreSQL 默认连接本机 `15432/test_agent`，Redis 默认关闭，opencode 节点从 `TEST_AGENT_OPENCODE_BASE_URL` 读取。
- `application-test.yml` 继续使用 `TEST_AGENT_TEST_DB_*` 注入测试 PostgreSQL。
- `application-prod.yml` 不提供真实默认 secret；生产 token、CORS allow origins、数据库连接、opencode 节点都必须来自环境变量或配置中心。
...
- `deploy/local/docker-compose.yml` 提供 Postgres 和 Redis 服务。Redis 默认通过 compose profile 启用，避免强制依赖。
- `tools/dev-local-up.sh` 启动本地依赖，默认只启动 Postgres，可通过参数启用 Redis。
...
- 本地 profile 可连接 Compose Postgres 并 seed opencode node。
