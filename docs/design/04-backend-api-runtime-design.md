...
- `POST /api/runs` 接收 `sessionId` 和 `prompt`，创建 Run、追加用户消息、追加 `run.created`、路由执行节点、保存 routing decision、调用 `OpencodeClientFacade.startRun`，成功后 Run 转为 `RUNNING` 并追加 `run.started`。
