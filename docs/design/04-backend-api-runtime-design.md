...
Phase 04 原设计在 `test-agent-app` 中暴露 Runtime API；当前工程边界已调整为 `test-agent-api` 定义 HTTP/SSE/WebSocket 入口，并由 `test-agent-app` 装配运行。入口层只负责协议转换、参数校验、traceId 和统一响应；业务模块负责编排 domain、persistence、event 和 opencode facade。Controller 不直接访问 Repository，不直接调用 generated SDK。
...
- CORS、基础安全响应头统一在 API 入口配置中处理，并由 app 装配生效；生产环境允许来源只能来自配置。
