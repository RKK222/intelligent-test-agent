...
2. 原主 Run 流继续负责回答。root `run.succeeded` 到达后，如果 token 仍有效且平台标题仍为首条消息临时标题，终态 payload 标注 `platformSessionTitlePending=true`，流切换到标题等待阶段：只通过该 root session 的 `session.updated`，丢弃消息、工具、child 和其它下一轮事件。
...
5. 终态前原主流已经同步有效标题时立即注销 token，不标记 pending。成功同步、条件更新失败（用户已改名）、新 Run 开始、会话归档/删除时，原子注销 token；新 Run 使旧 token 失效后，旧流立即完成，避免同一 root session 的下一轮事件落入旧 Run。
6. 标题等待阶段的远端连接暂时断开时，只有 token 仍有效才按既有运行时路由重连同一条 Run 事件订阅；不依据生成时长结束等待。进程停止或用户显式关闭会话时按既有连接生命周期释放。

## 错误与兼容

- 原生 title 不产生有效更新时，平台保留首条消息临时标题，不显示失败，也不生成猜测标题；等待不以固定时长断开。
...
- 单元测试：首轮注册、终态后有效 root `session.updated`、默认标题忽略、child 忽略、过期 token 忽略、下一轮注销、归档/删除注销、条件更新失败不发布事件、终态前已同步时不进入等待。
