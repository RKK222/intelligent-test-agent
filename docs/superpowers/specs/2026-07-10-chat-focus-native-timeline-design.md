...
2. 运行中的子 Agent；
3. 运行中的任务；
4. 当前 active Run 的失败摘要；只读展示既有状态文本和错误摘要，不增加重试入口。
...
- 当前 active Run 的失败摘要在会话/Run 切换前保持可见；切换后立即由新 active Run 状态替换，不长期展示历史失败。待处理 Ask / Permission 不受 Run 终态影响，直到既有原生状态将其清除。
