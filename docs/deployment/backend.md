...
| `TEST_AGENT_SCHEDULER_ENABLED` | `false` / 需要后台任务时 `true` | 控制 scheduler 后台扫描和 pending run 执行；任务定义在应用启动时仍会同步到 `scheduled_tasks`，管理页可展示。运行记录保留清理、opencode stale active Run 收敛和运营分析汇总任务复用该框架。 |
