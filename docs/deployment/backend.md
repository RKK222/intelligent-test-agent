...
多数通用参数仍按需直读数据库；显式 JVM 内存参数在每个 Java 启动时严格查库加载。当前 `NIGHT_EXECUTION_SLOT_CAPACITY` 缺失、空白、非正整数或溢出会阻止该 Java 启动，旧 `TEST_AGENT_NIGHT_EXECUTION_SLOT_CAPACITY` 环境变量和 yaml 配置不再生效。超级管理员可在通用参数页按需查询全部在线 Java 的实际内存值，并按数据库当前值刷新全部或指定 `backendProcessId`；集群操作最多处理 500 个 Java、并发 8、单进程 10 秒，同服务器多个 Java 不合并。部署排障日志只记录 trace、进程身份和状态，不记录参数值或原始异常。
