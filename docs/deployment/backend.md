...
公共配置 push 成功后会建立持久化排空任务，并把持久化 Linux 服务器清单纳入同步范围；各目标 Java 使用发起管理员数据库中已存的 SSH key，将本服务器共享运行副本 checkout/reset 到目标 commit。发起管理员自己的稳定个人 worktree 在显式拉取时先合并远端，但其他管理员的个人 worktree 不会被覆盖；运行时生效来源是每台服务器的共享副本。目标服务器取得本机 manager 进程清单后才确认同步，随后固定延迟任务只处理本 `linuxServerId` 的目标，通过本机 opencode 检查该进程历史绑定的所有 Workspace Session 状态；全部空闲后每个用户专属进程只调用一次 `/global/dispose`。该用户目标完成即单独开闸，下一次消息会重新创建 Instance，因此新 `opencode.jsonc`、Agent 和 Skill 在该用户下一次请求生效；忙碌、非法响应、服务器离线或 manager 清单缺失均持续重试，不设最大次数。
