...
当前企业单后端升级以 `122.233.30.114` 为准：交付 zip 放到 `/data/0709/internal.zip` 后，在 114 执行 `deploy-internal-release.sh --backend-host 122.233.30.114 --skip-frontend`；前端机统一在 `122.233.30.2` 本地执行 `deploy-internal-frontend.sh --archive /data/0709/internal.zip`。历史双后端流程仅见 `deploy/internal/README-two-backend-122-233-30-114.md`，不作为当前部署依据。
...
| `TEST_AGENT_SCHEDULER_ENABLED` | `false` / 需要后台任务时 `true` | 控制 scheduler 后台扫描和 pending run 执行；任务定义在应用启动时仍会同步到 `scheduled_tasks`，管理页可展示。opencode stale active Run 收敛和运营分析汇总任务复用该框架。 |
