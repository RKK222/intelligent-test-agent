...
- `TEST_AGENT_XXL_JOB_ADMIN_ADDRESSES` 列出 executor 注册所用的全部 Admin 地址；多地址按 XXL 约定用逗号分隔。
...
Admin/MySQL 故障只让 `xxlJobAdmin` health 为 `DOWN`，生命周期组件按 5～60 秒指数退避恢复，不停止平台 WebFlux 主服务，也不进入平台 readiness group。运维必须同时监控平台 readiness 与 XXL health，避免只看到主服务 UP 而遗漏周期任务不可用。
...
| `TEST_AGENT_XXL_JOB_ENABLED` | `true` | 启动独立 Admin 子上下文与 executor；MySQL 故障不改变平台 readiness。 |
...
应用启动时，旧 `ScheduledTaskRegistry` 只同步 USER_PLAN 专用 handler；`ScheduledTaskRunner` 只扫描服务器亲和 `USER_PLAN`。Admin 子上下文 Flyway 负责初始化六条周期任务，所有 Java executor 自动注册且不带 Linux 亲和。超级管理员在同源 XXL 页面查看/修改/触发/停止周期任务和查看日志；清理任务由 XXL 在北京时间 08:00 触发并删除 PostgreSQL 中超过 7 天的旧 scheduler 已结束记录。
