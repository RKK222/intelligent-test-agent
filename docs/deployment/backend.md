...
| `TEST_AGENT_LEGACY_RUN_WITHOUT_CONTEXT_ENABLED` | `true` | 绑定 `test-agent.redis-summary.legacy-run-without-context-enabled`；兼容期允许旧客户端不携带 `contextToken`。 |
...
| Run 活跃运行态 TTL | `3h` 滑动 | manifest/input/Stream/snapshot/scope 在 append、投影和 touch 时续期；现有 2 小时无活动收敛仍负责更早结束异常 Run。 |
...
| 单 Run Redis 详情上限 | `20,000` 条/项或 `32 MiB` | durable 事件、runtime 事件、snapshot 投影项任一超过 20,000，或 input + scope + 双 Stream + snapshot 规范化详情超过 32 MiB 时，Lua 显式删除旧 Stream、递增 reset generation 并保留当前物化状态；超大 payload 先规范化，再优先移除低价值投影，禁止依赖 Redis eviction 静默丢数据。 |
...
- 所有 Java 实例必须连接同一受控 Redis 集群/主从服务。单 Run manifest/input/durable `events` Stream/全事件 `runtime-events` Stream/snapshot Hash + order ZSET/动态 key registry/scope/dedup/pending key 使用 `test-agent:run:{runId}:...` hash tag，保证 Lua 和 Stream 操作同 slot；active 用户/Session/服务器索引按各自业务 ID hash tag 分布，并在回读时校验 manifest。
