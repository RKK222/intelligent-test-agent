...
每个 Java 进程均启动一个 Admin 子端口和一个 executor 端口：

- 同一主机多个 Java 的 `TEST_AGENT_XXL_JOB_ADMIN_PORT` 与 `TEST_AGENT_XXL_JOB_EXECUTOR_PORT` 必须各自唯一。
- `TEST_AGENT_XXL_JOB_ADMIN_ADDRESSES` 列出 executor 注册及启动 readiness 探测所用的全部 Admin 地址；多地址按 XXL 约定用逗号分隔。
- `TEST_AGENT_XXL_JOB_EXECUTOR_ADDRESS` 必须是所有 Admin 节点可访问的当前 Java executor URL；未显式提供时可用 `IP + port`，生产建议显式配置。
- 防火墙只向受信 Admin 节点开放 executor 端口，不向浏览器或公网开放。
- executor 组固定为 `test-agent-backend`，不携带 `linuxServerId`，不使用稳定 Linux 服务器亲和；夜间一次性 `USER_PLAN` 的原 affinity 不受影响。

Admin/MySQL 故障只让 `xxlJobAdmin` health 为 `DOWN`，Admin 生命周期按 5～60 秒指数退避恢复，不停止平台 WebFlux 主服务，也不进入平台 readiness group。executor 不会在 Spring 初始化阶段抢先启动，而是以 250 毫秒～5 秒退避等待任意配置 Admin 的 readiness 返回 HTTP 200；全部 Admin 不可用期间 executor 端口不监听，恢复后在当前 Java 内只启动一次。运维必须同时监控平台 readiness、XXL health 和 executor 注册状态，避免只看到主服务 UP 而遗漏周期任务不可用。

推荐生产部署顺序：

1. 创建共享 MySQL 库和账号并验证字符集/时区。
2. 为所有 Java 配置相同 MySQL 凭据和强随机 `TEST_AGENT_XXL_JOB_ACCESS_TOKEN`，配置每实例唯一端口与可达地址。
3. 滚动部署 Java，确认 Flyway 到 V3、六条任务和各 executor 自动注册。
4. 开放 Admin 到所有 executor 的受控网络。
5. 配置 Nginx `/xxl-job-admin/` 同源代理并执行 iframe SSO 验收。
...
| `TEST_AGENT_SCHEDULER_ENABLED` | `true` | 只控制旧 scheduler 的服务器亲和 `USER_PLAN` 扫描；显式关闭会使夜间一次性计划不可用，不影响 XXL Admin/executor 启动。 |
...
| 夜间任务补偿 XXL cron | `0 0/5 * * * ? *` | 任务 key 为 `opencode-runtime.night-execution-reconcile`，恢复超过 5 分钟的认领、窗口内顺延、07:00 最终失败，并清理 30 天前终态任务/容量占位。 |
...
TEST_AGENT_XXL_JOB_ADMIN_ADDRESSES=http://127.0.0.1:18080/xxl-job-admin
TEST_AGENT_XXL_JOB_EXECUTOR_PORT=9999
TEST_AGENT_XXL_JOB_EXECUTOR_ADDRESS=http://127.0.0.1:9999
...
TEST_AGENT_XXL_JOB_ADMIN_PORT=<unique-admin-port>
TEST_AGENT_XXL_JOB_ADMIN_ADDRESSES=http://<admin-1>:18080/xxl-job-admin,http://<admin-2>:18080/xxl-job-admin
TEST_AGENT_XXL_JOB_EXECUTOR_PORT=<unique-executor-port>
TEST_AGENT_XXL_JOB_EXECUTOR_ADDRESS=http://<this-java-reachable-host>:<executor-port>
```
...
docker run --rm -p 8080:8080 \
  ...
  -e TEST_AGENT_XXL_JOB_EXECUTOR_ADDRESS=http://backend-1.example.internal:9999 \
...
应用启动时，旧 `ScheduledTaskRegistry` 只同步 USER_PLAN 专用 handler；`ScheduledTaskRunner` 只扫描服务器亲和 `USER_PLAN`。Admin 子上下文 Flyway 负责初始化六条周期任务；每个 Java 在任意配置 Admin readiness 就绪后才启动并自动注册 executor，注册不带 Linux 亲和。超级管理员在同源 XXL 页面查看/修改/触发/停止周期任务和查看日志；清理任务由 XXL 在北京时间 08:00 触发并删除 PostgreSQL 中超过 7 天的旧 scheduler 已结束记录。
