...
应用启动时，旧 `ScheduledTaskRegistry` 只同步 USER_PLAN 专用 handler；`ScheduledTaskRunner` 只扫描服务器亲和 `USER_PLAN`。Admin 子上下文 Flyway 负责初始化六条周期任务；每个 Java 在本机 Admin readiness 就绪后才启动 executor，并使用平台 advertised host 自动注册地址，注册不带 Linux 亲和。超级管理员在同源 XXL 页面查看/修改/触发/停止周期任务和查看日志；清理任务由 XXL 在北京时间 08:00 触发并删除 PostgreSQL 中超过 7 天的旧 scheduler 已结束记录。
