...
| 用户初始化返回 `OPENCODE_BAD_GATEWAY` 且包含 `already managed but unhealthy` | 目标端口已有 manager 本地 state，但 PID 或 HTTP 健康检查失败 | 先查看 `{stateDir}/processes/{port}.json`，再按其中的 `unifiedAuthId`、`startedAt` 和端口定位 `{stateDir}/logs/{safeUnifiedAuthId}-*.log`；历史 state 则回查 `{stateDir}/logs/{port}.log`。确认无业务流量后用 manager `restart` 或 `stop` 清理该端口。健康的已托管端口会被幂等复用，不会再因 `already managed` 初始化失败。 |
