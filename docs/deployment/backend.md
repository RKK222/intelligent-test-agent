...
| `opencode --version` 返回 `Trace/breakpoint trap`、退出码 `133`，`dmesg` 出现 `trap int3` | 对比宿主机与容器 `uname -r`，确认 CPU 指令集；当前 Linux OpenCode CLI 嵌入 Bun，Bun 官方要求 Linux 内核至少 `5.1`，推荐 `5.6+` | Docker 与宿主机直接运行共享同一内核约束，换基础镜像、移出容器或延长健康检查超时无效；改用满足要求的完整虚拟机/执行节点，或先单独研发并全量验证非 Bun 的 Node server 交付物，当前发布包不包含该替代物。 |
