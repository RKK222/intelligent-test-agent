...
企业内三机升级时，如果已把交付 zip 放到 `122.233.30.4:/data/0709/internal.zip`，优先在后端服务器执行 `/data/testagent/deploy/internal/deploy-internal-release.sh --archive /data/0709/internal.zip`。脚本会先通过前端服务器更新静态资源并 reload Nginx，再替换本机 Java jar、解压外挂程序、导入 worker 镜像，最后按“启动 Java 并校验 `.serverid/.serverhost` → 重启 worker”的顺序完成升级。
