...
每台 Linux 只运行一个 Java，每个 Java 均启动一个 Admin 子端口和一个 executor 端口：

- executor 固定使用同 JVM 的 `http://127.0.0.1:${TEST_AGENT_XXL_JOB_ADMIN_PORT}/xxl-job-admin` 注册和启动 readiness 门控，不维护跨节点 Admin 地址列表。
- executor 注册地址复用平台 `BackendInstanceIdentity.listenUrl()` 的 advertised host 并拼接 `TEST_AGENT_XXL_JOB_EXECUTOR_PORT`。`TEST_AGENT_SERVER_ADVERTISED_HOST` 未配置时由现有 `LinuxServerIpResolver` 自动探测内网 IPv4。
- 所有 Admin 共享同一个 XXL MySQL，因而任一 Admin 都能看到全部 executor；每个 Admin 必须能访问所有节点派生出的 `advertisedHost:executorPort`。
- 防火墙只向受信 Admin 节点开放 executor 端口，不向浏览器或公网开放。
- executor 组固定为 `test-agent-backend`，注册和 ROUND 路由不携带 `linuxServerId`；夜间扫描后的业务分发读取任务固化的目标服务器，并通过公共 Java 路由调用目标进程。

Admin/MySQL 故障只让 `xxlJobAdmin` health 为 `DOWN`，Admin 生命周期按 5～60 秒指数退避恢复，不停止平台 WebFlux 主服务，也不进入平台 readiness group。executor 不会在 Spring 初始化阶段抢先启动，而是以 250 毫秒～5 秒退避等待本机 Admin readiness 返回 HTTP 200；本机 Admin 不可用期间 executor 端口不监听，恢复后在当前 Java 内只启动一次。运维必须同时监控平台 readiness、XXL health 和 executor 注册状态，避免只看到主服务 UP 而遗漏周期任务不可用。
...
基础 `application.yml` 中的 XXL access token 默认值仅用于本地开发。生产模板必须显式设置 `TEST_AGENT_XXL_JOB_ACCESS_TOKEN`；漏配会继承本地默认值，部署检查必须把这种情况判定为失败。

架构和任务 SQL 规则见 `docs/architecture/xxl-job-integration.md`。
...
```text
/data/.testagent/agent-opencode/.session/            # 用户 HOME/XDG/TMP 根目录，按统一认证号分目录
...
其中 `sessionPath=/data/.testagent/agent-opencode/.session/users/{unifiedAuthId}`，按统一认证号隔离，不对应任何应用 worktree；`current-public-config` 是 Java 维护的固定软链接，而不是复制出来的配置目录。本仓库本地 test profile 的默认测试用户可对应为：

```text
HOME=<repo>/.testagent/agent-opencode/.session/users/DEV_888888888
XDG_DATA_HOME=<repo>/.testagent/agent-opencode/.session/users/DEV_888888888
XDG_CACHE_HOME=<repo>/.testagent/agent-opencode/.session/users/DEV_888888888/.cache
XDG_STATE_HOME=<repo>/.testagent/agent-opencode/.session/users/DEV_888888888/.local/state
TMPDIR=<repo>/.testagent/agent-opencode/.session/users/DEV_888888888/.tmp
...
用户运行目录的根和公共配置源不再由 manager 环境变量传入，而是 manager 通过 WebSocket `configRequest` 从 Java 后端获取通用参数 `OPENCODE_SESSION_DIR`、`OPENCODE_PUBLIC_CONFIG_DIR`；Java 再按用户生成 session 与固定 `current-public-config` 软链接，通过 `start.sessionPath/start.configPath` 显式下发。manager 先合并 `start.environment`，再覆盖 `HOME`、全部 XDG 变量、`TMPDIR` 和 `OPENCODE_CONFIG_DIR`，调用方不能绕过隔离。fork 前，manager 把 session、`.cache`、`.local/state`、`.tmp` 创建或校正为 `0755` 普通物理目录；任何目录无法创建或被文件/软链接占用时不启动进程、不写成功 state。目录停止和重启时不清理。

链接默认指向共享公共目录，公共个人保存时只切到本人 worktree，公共发布排空时恢复共享目录；不复制配置。最大进程数同样来自 `OPENCODE_MANAGER_MAX_PROCESSES`（`platform=all`）。OpenCode 会合并用户全局配置、`OPENCODE_CONFIG_DIR` 自定义目录和请求工作区 `.opencode`，企业部署必须保证运行 `opencode serve` 的系统用户 `~/.config/opencode/config.json`、`opencode.json`、`opencode.jsonc` 不维护模型或供应商配置，最多保留只含 `$schema` 的空配置；模型和供应商只写入公共配置 Git。收到完整 `configUpdate` 前，manager 会拒绝 `start`/`restart` 命令，不会用容器内默认路径启动用户进程；成功应用 `configUpdate` 后，manager 会立即补发 heartbeat，把端口池裁剪后的生效容量写入运行管理 Redis 快照。后续前端只允许修改最大进程数，Java 只向本服务器 manager 热推 max-only `configUpdate`，路径参数不做运行中刷新。通用参数值不会经过 shell；`$NAME` 直接按 Java 后端进程环境变量展开，`${NAME}` 先按通用参数引用解析、未命中时再按环境变量展开，路径开头的 `$HOME` 和 `~/` 会解析为当前用户主目录后再下发给 manager。实际 `configPath` 的存在性和非空检查发生在目标 manager 执行 `start` 时；软链接不受支持或权限不足时 Java 明确失败，不降级为配置复制。运行目录隔离只承诺 Linux 生产环境，同一 manager 系统账号下仍是目录级隔离，不引入独立 OS 用户；存量 PID 不会自动中断，必须经过平台公共停止/启动程序受管重启后才取得新变量。
...
数据库状态是引用副本任务的事实源。generation 和服务器目标建档后，发起 Java 会立即把本机任务提交给有界异步调度器，同时发布 Redis `reference-repository.sync-requested` 唤醒其它 Java；广播消费者只把任务排入同一调度器并立即返回，不在 HTTP 或 Redis listener 线程运行 Git。调度器按 `repositoryId + generation` 合并重复唤醒，默认 `worker-count=2`、`max-pending-tasks=256`，可通过 `test-agent.reference-repository.replica-worker.*` 调整；容量满时拒绝新 key 并由补偿扫描恢复，禁止 caller-runs。每台服务器的 worker 仍须先按 `repositoryId + generation + linuxServerId` 取得两分钟数据库租约，并在 clone/fetch/reset/原子移动等共享目录写步骤前续租；generation、lease token 或有效期任一不匹配时旧 worker 立即停止写入。同服务器多 Java 还使用引用根下的本机文件锁避免租约到期边界并发写同一仓库。
...
| `/data/.testagent/agent-opencode/.session/users/{unifiedAuthId}` | Linux 服务器本地盘并挂载到容器 | 用户进程 `HOME`、`XDG_DATA_HOME` 及其 `.cache`、`.local/state`、`.tmp` | 普通目录固定为 `0755`，不能跨 Linux 服务器共享；停止/重启不清理，备份或清理必须按统一认证号和用户绑定关系执行。旧命令缺少显式 session 时仍兼容 `{OPENCODE_SESSION_DIR}/{port}`。 |
...
| `/data/.testagent/agent-opencode/manager/logs/{safeUnifiedAuthId}-{yyyyMMddTHHmmss.nnnnnnnnnZ}-{port}.log` | 容器挂载目录 | 单次 opencode server 启动的 stdout/stderr | 日志文件名含统一认证号的安全编码，访问和对外传输都必须按身份数据保护；日志内容不得输出 token、Authorization、Cookie 或完整 prompt。 |

### 应用成员撤权后的个人 worktree 磁盘回收

“应用人员管理”移除成员只撤销平台访问权限，不自动删除个人 worktree。个人目录可能包含未提交文件，数据库记录还承担历史 Session 归属和重新加入后的 worktree 修复，因此不能在撤权请求内直接级联删除。系统管理的整个平台用户删除也会在存在 `personal_workspaces` 等业务引用时拒绝执行。

撤权后的正常产品语义是“磁盘与历史记录保留、当前工作区不可见”：受影响用户的工作台在成员应用目录刷新后会清空该应用的文件树、编辑器、Diff 和当前运行上下文；全局最近工作区接口也不会返回已撤权应用的保留路径。文件 WebSocket route、ticket、每条 RPC 以及会话运行上下文仍在后端实时复核成员关系，不能用旧页面或旧 ticket 继续访问。历史 Session 记录保留用于追溯，但重新进入工作区失败时只读展示。

确需回收服务器磁盘时，按下列顺序逐个个人 worktree 操作：

1. 在“系统管理 → 运行管理”按用户查询并停止该用户的 opencode server，确认进程已停止。多服务器环境必须在 `workspaces.linux_server_id` 指向的服务器执行后续命令。
2. 使用只读 SQL 确认范围，不直接修改数据库：

```sql
select pw.personal_workspace_id,
       pw.app_id,
       pw.app_workspace_version_id,
       pw.branch,
       pw.repo_root_path,
...
where pw.user_id = '<目标 userId>'
  and pw.app_id = '<目标 appId>'
order by w.linux_server_id, pw.app_workspace_version_id, pw.workspace_name;
```

新记录的 `repo_root_path` 可能是 `personalworktree:` 逻辑值。将该前缀后的相对路径拼到当前服务器 `common_parameters.OPENCODE_PERSONAL_WORKTREE_ROOT` 的实际值下，得到唯一物理目录；不要用模糊 `find`、通配符或用户根目录作为删除目标。

3. 对查询得到的每一个**精确物理目录**先检查状态，并记录 Git 公共目录。下面的路径必须替换为上一步得到且人工核对过的绝对路径：

```bash
git -C /exact/personal/worktree status --short --branch --untracked-files=all
git -C /exact/personal/worktree rev-parse --path-format=absolute --git-common-dir
```

状态非空时先备份并由负责人确认去留，不得继续删除。状态为空时，把第二条命令返回的精确公共 Git 目录代入：

```bash
git --git-dir=/exact/application/repository/.git worktree list --porcelain
git --git-dir=/exact/application/repository/.git worktree remove /exact/personal/worktree
git --git-dir=/exact/application/repository/.git worktree prune
```

不要添加 `--force`，也不要用 `rm -rf` 代替 `git worktree remove`；前者可能丢失未提交内容，后者会留下应用版本仓库的 worktree 元数据。磁盘回收后保留数据库记录和个人分支；用户以后重新加入应用并显式选择该版本时，既有 `ensure-default-personal-workspace` 会从保留分支重建缺失目录。若要求连同历史归属、运行态 Workspace、个人分支和数据库记录永久销毁，应另行设计带预检、备份、多服务器路由和审计的退役流程，不能通过现场 SQL 直接删除。
...
- 受管用户 opencode server 日志在 `OPENCODE_MANAGER_STATE_DIR/logs/{safeUnifiedAuthId}-{yyyyMMddTHHmmss.nnnnnnnnnZ}-{port}.log`；企业内纯 Docker worker 示例为 `/data/testagent/data/agent-opencode/manager/worker/logs/DEV_888888888-20260721T081530.123456789Z-4096.log`。启动时间使用 UTC，每次启动独立成档，stdout/stderr 写入同一文件；重启不会覆盖旧文件。
- 常规 ASCII 统一认证号可用 `ls -1t /data/testagent/data/agent-opencode/manager/worker/logs/DEV_888888888-*.log` 查找最近启动日志。身份含特殊字符时，文件名会按 UTF-8 字节使用大写 `%HH` 编码；过长时使用有界前缀加完整 SHA-256，应结合 `processes/{port}.json` 中的 `unifiedAuthId`、`startedAt` 和端口定位，不要把完整文件名复制到未脱敏的外部工单。
- 升级不会迁移或删除历史 `{port}.log`；旧 Java/本地 CLI 无法提供或派生统一认证号时也继续使用该兼容文件名，因此历史排障仍需按端口检查旧文件。
...
| `TEST_AGENT_XXL_JOB_ENABLED` | `true` | 启动独立 Admin 子上下文，并在任一 Admin readiness 就绪后启动 executor；MySQL 故障不改变平台 readiness。 |
| `TEST_AGENT_XXL_JOB_ADMIN_PORT` / `TEST_AGENT_XXL_JOB_EXECUTOR_PORT` | `18080` / `9999` | 每个 Java 进程的独立端口；同机多进程必须唯一。 |
| `TEST_AGENT_XXL_JOB_COOKIE_SECURE` | `true` | XXL SSO Cookie 的 `Secure` 属性。生产 HTTPS 保持默认；只有受控内网固定 HTTP 入口才显式设为 `false`，并仍保留 HttpOnly、SameSite 与 Path 限制。 |
| 定时任务分发 XXL cron | `0 0/1 * * * ? *` | `opencode-runtime.night-execution-dispatch` 每分钟扫描最多 500 条已到期且未越过各自 `windowEnd` 的 `NIGHT_WINDOW/ADMIN_CUSTOM` 任务，固定目标分组、50 条一批、最多并发 8 台服务器；任务级失败等待下轮。 |
| 定时任务补偿 XXL cron | `0 0/5 * * * ? *` | `opencode-runtime.night-execution-reconcile` 先查稳定 Run 锚点，再按 5 分钟租约、精确 owner Java 心跳和 attempt fencing 修复/恢复；越过各自 `windowEnd` 且无锚点时最终失败。 |
...
已有环境升级本版本时，推荐顺序为“opencode-manager → Java 后端 → 前端”。先逐台替换并重启 worker 内 manager，确认运行管理 capability 包含 `stopOwned`，心跳明细可选出现 `unifiedAuthId/managerStatus`；再滚动升级各 Java，最后一次性部署前端静态资源。新 manager 仍接受旧 Java 的 `start/health/stop/restart`，旧 Java 会忽略新增心跳字段和 `bindingRecovery` 语义；若新 Java 暂时连接旧 manager，已有 binding 在 manager 容量已满时可能只报容量错误，但必须保留原 binding，不得扫描新端口。其它未知命令或未知启动错误同样 fail closed，不得回退端口 stop 或迁移端口。滚动期间不要同时对同一用户执行人工重启和初始化。

本次升级不新增 SSE 事件、数据库表/列、Flyway migration、环境变量或 generated SDK；不需要修改 `backend.env`、`docker.env` 或执行数据迁移。存量重复进程和无主进程不会自动认领、停止、删除或改绑；若 `IDENTITY_ALREADY_MANAGED` 阻止启动，由 `SUPER_ADMIN` 在运行管理页核对 UCID、manager PID 状态、服务器、容器、端口和启动时间后手工处理。回滚时先回滚前端和 Java，新 manager 可继续兼容旧 Java；确认 Java 已回滚后再决定是否回滚 manager，期间不得清理 manager state 或用户 session 目录。

后端 Java 扩容流程：

1. 新增一台 Linux，并为其配置共享 PostgreSQL/Redis 范围内唯一且长期稳定的 `TEST_AGENT_LINUX_SERVER_ID`；每台 Linux 只运行一个 Java。
...
6. 确认新 executor 以同一 advertised host 和 executor 端口注册；将新节点同时加入中央 Nginx 的普通 backend 与 XXL Admin upstream 并无停机 reload。旧 Java 不修改环境、不重启。
...
| 公共个人保存提示仍使用旧共享路径 | 运行管理页查看该用户进程 `configPath` 是否为 `{sessionPath}/.testagent-runtime/current-public-config` | 通过平台停止并重新启动该用户进程一次；不要手工复制公共配置。 |
| 公共个人保存或初始化提示无法创建受管软链接 | 检查 session 挂载是否同时对 Java/manager 可见、Java 运行用户是否有目录与软链接权限、固定路径是否被普通文件/目录占用 | 修正共享挂载/权限或移走冲突项后受管重启；Windows/受限文件系统不允许静默降级复制。 |
...
- 回滚前不要清理 `/data/.testagent/agent-opencode/.session/users/{unifiedAuthId}` 或兼容的 `{OPENCODE_SESSION_DIR}/{port}`，否则再恢复用户进程模型时会丢失远端 session、缓存和状态数据。
...
4. 原端口复用：记录当前用户的 `processId/linuxServerId/containerId/port`，让 4104 的 HTTP health 失败后重新初始化；确认公共停止服务先停止旧 PID，再在同一 4104 启动，且没有调用空闲端口选择。`STALE`、超时、配置错误和容量不足时上述 binding 坐标均不变化。
5. 明确冲突迁移：仅让 manager 对 4104 返回 `PORT_CONFLICT` 或 `PORT_OUT_OF_RANGE`，确认才按原服务器既有规则选择 4105，并原子更新同一 processId 和 binding；其它错误不得迁移。
6. 后端扩容：新增后端实例后，Redis 出现新 Java 快照，本服务器 manager 连接该 Java，运行管理页出现新的 manager-backend `CONNECTED` 连接；其它 Java 能通过 `listenUrl` 把 remote binding 请求转发到该后端。
7. 管理页展示：`SUPER_ADMIN` 能看到 Linux 服务器、后端进程、容器、manager、连接、opencode 进程和绑定状态；无主进程展示 UCID、`PID_ALIVE`、“平台未登记”、“未执行 HTTP 健康检查”和正确 baseUrl 列，拓扑缺新字段时安全回退。`APP_ADMIN` 和普通用户菜单不可见，直接访问 API 返回 `FORBIDDEN`。
8. 存量无主进程：确认运行管理只展示、不自动认领/停止/改绑；管理员按 UCID/PID/端口核对后手工处理。
9. 固定节点兼容：使用无用户主体的 static-token 兼容调用时，旧 `execution_nodes` fallback 仍可用于本地探测或旧集成。
10. 用户目录隔离：受管重启两个不同统一认证号的进程，确认 `startCommand` 中 HOME/XDG/TMP 路径分别落在各自 `sessionPath`，`.cache/.local/state/.tmp` 是 `0755` 普通目录且公共配置仍为软链接；确认重启后缓存和状态目录未清理。
11. 日志脱敏：后端日志、manager 日志和 opencode server 日志不出现 `Authorization`、token、Cookie、普通运行管理 UCID 或完整 prompt。
...
```bash
tools/dev-local-up.sh            # 启动备用 Postgres + MySQL 8.4；--redis 额外启动 Redis
...
TEST_AGENT_DB_URL=jdbc:postgresql://127.0.0.1:15432/test_agent
TEST_AGENT_DB_USERNAME=test_agent
TEST_AGENT_DB_PASSWORD=<local-pg-password>
...
TEST_AGENT_XXL_JOB_MYSQL_URL=jdbc:mysql://127.0.0.1:13306/xxl_job?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai
TEST_AGENT_XXL_JOB_MYSQL_USERNAME=xxl_job
TEST_AGENT_XXL_JOB_MYSQL_PASSWORD=<local-xxl-mysql-password>
TEST_AGENT_XXL_JOB_ACCESS_TOKEN=<local-shared-access-token>
TEST_AGENT_XXL_JOB_ADMIN_PORT=18080
TEST_AGENT_XXL_JOB_EXECUTOR_PORT=9999
TEST_AGENT_XXL_JOB_COOKIE_SECURE=true
...
TEST_AGENT_XXL_JOB_ENABLED=true
TEST_AGENT_XXL_JOB_MYSQL_URL=jdbc:mysql://<mysql-host>:3306/xxl_job?useUnicode=true&characterEncoding=UTF-8&serverTimezone=Asia/Shanghai
TEST_AGENT_XXL_JOB_MYSQL_USERNAME=<xxl-job-username>
TEST_AGENT_XXL_JOB_MYSQL_PASSWORD=<xxl-job-password>
TEST_AGENT_XXL_JOB_ACCESS_TOKEN=<shared-xxl-access-token>
TEST_AGENT_XXL_JOB_ADMIN_PORT=18080
TEST_AGENT_XXL_JOB_EXECUTOR_PORT=9999
TEST_AGENT_XXL_JOB_COOKIE_SECURE=true
```
...
应用内已删除旧 scheduler runner、`USER_PLAN` 服务和 `test-agent.scheduler.*` 配置；周期任务统一由 XXL 调用 handler adapter。`GLOBAL_MUTEX` 继续使用 Redis `test-agent:scheduler:lock:{taskKey}` 和 token 续租/释放，不降级为本机锁或数据库锁。旧 PostgreSQL scheduler 表只保留审计和 7 天历史清理。

夜间任务窗口固定为北京时间 21:00 至次日 07:00，按 15 分钟时段使用数据库全局容量占位。容量唯一来源是 `common_parameters.NIGHT_EXECUTION_SLOT_CAPACITY`（`platform=all`、`editable=true`），Flyway 初始化为 `20`；超级管理员可在“系统管理 → 通用参数管理”在线修改。每个 Java 实例启动时把已校验的正整数加载到 `NightExecutionCapacityRegistry` 内存快照；缺失或非法时实例启动失败。修改成功后沿用 `CommonParameterUpdatedEvent → common-parameter.refresh-requested → CommonParameterReloadedEvent` 链路通知各服务器查库并原子刷新；运行中刷新失败保留上一有效值。调低容量不会取消已有预约，当已预约数达到或超过新上限时停止新增；调高后立即可用。多后端生产必须启用并连通既有服务器广播配置。

任务提交时固化用户 opencode binding 所属服务器，之后不随 binding 变化迁移；提交、时段查询和改期不检查 XXL 是否在线。XXL 每分钟扫描已到期且未越过各自 `windowEnd` 的 `SCHEDULED`，通过公共 Java resolver 先选出固定服务器上的精确 backendProcessId，再由公共 forwarder 分发任务 ID；目标 Java 从共享数据库重读输入，使用 attemptId、精确 backendProcessId 和 5 分钟租约认领，只复用公共进程初始化/启动、会话上下文与普通 `startScheduledRun`，不建立专属队列。内部 HTTP 只传目标服务器和最多 50 个 taskId，使用 traceId、标准 XXL token、防循环 header 和 120 秒超时。默认 legacy Scheduled Run 与 Redis 摘要 Run 共用数据库 clientRequestId 唯一锚点，恢复还会校验 task/owner/Session/Workspace 来源；普通 Run 返回已受理 runId 后定时任务状态即为 `DISPATCHED` 并解锁，仅 `NIGHT_WINDOW` 同时释放夜间容量，`ADMIN_CUSTOM` 从不读写夜间容量，接口不等待 Run 终态。5 分钟补偿以 Run 锚点优先并按 attempt fencing 收敛，Run 副作用前执行一次窗口续租 CAS；`NIGHT_WINDOW` 在 07:00 窗口结束、`ADMIN_CUSTOM` 在计划时间后 15 分钟窗口结束时，若无锚点且没有仍存活的 owner/handle 则收敛为失败。
...
docker run --rm -p 8080:8080 \
  ...
  -e TEST_AGENT_XXL_JOB_MYSQL_URL='jdbc:mysql://mysql.example.internal:3306/xxl_job?serverTimezone=Asia/Shanghai' \
  -e TEST_AGENT_XXL_JOB_MYSQL_USERNAME=xxl_job \
  -e TEST_AGENT_XXL_JOB_MYSQL_PASSWORD=change-me \
  -e TEST_AGENT_XXL_JOB_ACCESS_TOKEN=change-me-xxl-token \
