...
| 夜间任务分发 XXL cron | `0 0/15 * * * ? *` | `opencode-runtime.night-execution-dispatch` 扫描最多 500 条，固定目标分组、50 条一批、最多并发 8 台服务器；任务级失败等待下轮。 |
| 夜间任务补偿 XXL cron | `0 0/5 * * * ? *` | `opencode-runtime.night-execution-reconcile` 先查稳定 Run 锚点，再按 5 分钟租约、精确 owner Java 心跳和 attempt fencing 修复/恢复；07:00 无锚点时最终失败。 |
