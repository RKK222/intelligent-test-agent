...
```bash
XDG_DATA_HOME=/data/.testagent/agent-opencode/.session/users/{unifiedAuthId} \
OPENCODE_CONFIG_DIR=/data/.testagent/agent-opencode/.session/users/{unifiedAuthId}/.testagent-runtime/current-public-config \
...
10. 日志脱敏：后端日志、manager 日志和 opencode server 日志不出现 `Authorization`、token、Cookie、普通运行管理 UCID 或完整 prompt。
