...
- `opencode-manager run` 的自身日志走容器 stdout，可用 `docker logs -f <worker-container>` 或 Compose logs 查看；重点搜索 `config update applied`、`websocket disconnected`、`.serverhost`、`.serverid` 和 `OPENCODE_UNAVAILABLE`。
- 用户 opencode server 日志在 `OPENCODE_MANAGER_STATE_DIR/logs/{port}.log`；企业内 Compose 当前为 `/data/testagent/data/agent-opencode/manager/worker/logs/{port}.log`。
...
| manager 日志持续连接旧 IP 或旧 Java | 对比 Java 服务器 `cat ${SYS_DATA_ROOT_DIR}/.serverhost`、worker 容器内 `cat ${SYS_DATA_ROOT_DIR}/.serverhost`、Docker Compose 的 `SYS_DATA_ROOT_DIR` 和后端 `SYS_DATA_ROOT_DIR` 是否指向同一挂载目录 | 先启动或重启 Java 让它写入正确 `.serverhost`，确认 worker 读到同一文件后再重启 worker；不要用 `OPENCODE_MANAGER_SERVER_IP_FILE` 或手工环境变量绕过。 |
