...
当前企业交付使用同一份离线 zip：单后台按 `deploy/internal/SINGLE-BACKEND.md` 执行；两个或更多后台的普通 HTTP、RunEvent SSE、用户 OpenCode 路由和内部模型代理按 `deploy/internal/MULTI-BACKEND.md` 执行。多后台每个节点都使用本机 `--backend-host <advertised-host> --skip-frontend` 部署，共享 PostgreSQL/Redis，但分别维护稳定 `linuxServerId`、本机数据目录、worker、公共配置和 9070 出站链路；前端只在 `122.233.30.2` 部署一次并通过 Nginx upstream 访问全部后台。PTY、Workspace 文件和 Agent 配置进度的一次性 WebSocket ticket 当前仍是单 JVM 内存，完整多后台上线前必须按多后台手册处理该限制，不能仅依赖 `least_conn` 或 sticky session。
