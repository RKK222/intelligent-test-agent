...
2. migration 文件命名必须稳定、递增、可重复执行，必须能从空库执行到最新版本。V17 及以前保留既有数字版本；V17 之后新增 migration 统一使用 `VyyyyMMddHHmmss__description.sql`，时间戳按开发者创建迁移时的本地时间确定，禁止继续使用顺序数字版本以免多人并行开发冲突；已应用到任何共享或本地库的 migration 禁止重命名。
...
4. 新字段优先允许空值或提供默认值；删除字段必须先完成读取兼容和数据迁移，再分阶段删除；枚举值、状态值、唯一约束和索引变更必须评估历史数据。
