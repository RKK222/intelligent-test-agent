...
日志配置必须对可变 message、thread 和 traceId 做 CRLF 编码，避免换行注入伪造日志记录。opencode 节点 health、Redis optional health 和 opencode-manager 控制面日志必须避免输出 token、完整 Authorization header、Cookie、用户输入或完整 prompt。
