...
| `workspace.upload` | `workspaceId`, `path`, `contentBase64` | `null`；上传 Base64 二进制新文件，沿用单文件大小上限，目标已存在返回 `CONFLICT` |
...
| `agent-config.upload` | `scope`, `workspaceId?`, `worktreeId?`, `path`, `contentBase64` | `null`；上传二进制新文件且不覆盖同名内容；公共 scope 仅 `SUPER_ADMIN`，工作空间 scope 仅 `APP_ADMIN`（`SUPER_ADMIN` 继承）并限制在 Agent 配置 Diff 白名单 |
