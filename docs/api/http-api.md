...
| `workspace-management` | `/api/internal/platform/workspace-management/applications/{appId}/reference-repositories` | 无旧 URL |
...
| `opencode-runtime` | `/api/internal/platform/opencode-runtime/management/linux-servers/{linuxServerId}/terminal/tickets` | `SUPER_ADMIN` 服务器终端；权限与目标 Java 系统用户一致，应用级默认关闭，企业交付模板显式启用，正式环境 WebSocket 必须走 WSS 定向网关。 |
...
7. 激活持久化 rollout、广播 `agent-config.public-sync-requested` 事件并返回最新 commit hash；各服务器后台同步与进程排空不计入该 HTTP 请求耗时。

该接口要求 `SUPER_ADMIN`，所有 Git 操作使用当前登录用户唯一 SSH key；commit 和可能产生 commit 的 merge 同时使用当前登录用户的命令级 Git 作者/提交者身份，不写入公共仓库或服务器全局 Git 配置；平台用户没有邮箱字段时按统一认证号生成企业 SCM 已登记的 `mails.icbc` Git email。提交说明为空返回 `VALIDATION_ERROR`，push 被远端拒绝（如 non-fast-forward）时返回 `GIT_UNAVAILABLE`，错误 details 携带 `gitFailureType=REMOTE_REJECTED` 与可展示的 `gitFailureHint`，不修改仓库外的状态。
...
成员删除只更新 `application_members.deleted_at`，不影响平台用户、其他应用关系或后续运行态数据。已有 `personal_workspaces`、运行态 `workspaces`、历史 Session 和服务器物理 worktree 均保留：成员撤权后所有托管工作区入口仍实时返回 `FORBIDDEN`，`GET /workspace-management/applications` 不再返回该应用，若全局 recent 指向该应用则 `GET /workspace-management/recent-workspace` 返回 `null`。工作台在窗口重新聚焦和最长 30 秒的前台轮询刷新成员应用目录；发现当前应用已撤权时清空旧文件树、编辑器、Diff、当前 Session/Run 与工作区选择，再切到仍有权限的第一个应用或进入空态。历史 Session 记录继续展示，但无法重新进入已撤权工作区，只能按既有只读态查看。保留记录用于历史归属和以后重新加入时修复 worktree。磁盘回收必须先停止该用户进程、核对目标服务器和未提交内容，再通过 Git worktree 命令清理；不得在成员删除请求中静默 `--force` 删除，也不得手工删除关系型记录。服务器操作见 `docs/deployment/backend.md` 的“应用成员撤权后的个人 worktree 磁盘回收”。
...
多数通用参数继续按需直读数据库；以下接口只展示实现了显式内存参数 SPI 的注册项。当前首个注册项为 `NIGHT_EXECUTION_SLOT_CAPACITY/all`。四个接口均仅允许 `SUPER_ADMIN`，不会把未注册参数推断为已加载，也不会创建 Redis 参数快照。

Base URL：`/api/internal/platform/configuration-management/common-parameters`

| 方法 | 路径 | 用途 |
|---|---|---|
| `GET` | `/memory-values` | 并发聚合全部在线 Java 的内存参数实际值。 |
| `GET` | `/memory-values/{backendProcessId}` | 按稳定进程 ID 精确查询一个在线 Java。 |
| `POST` | `/memory-values/refresh` | 让全部在线 Java 从数据库重载本机全部注册项。 |
| `POST` | `/memory-values/{backendProcessId}/refresh` | 让指定 Java 从数据库重载本机全部注册项。 |

集群查询与刷新最多处理 500 个在线 Java，单进程超时 10 秒、转发并发度 8；同一 `linuxServerId` 下的多个 Java 按各自 `backendProcessId` 独立返回，不按服务器合并。结果按 `linuxServerId`、`backendProcessId` 稳定排序。跨 Java 请求复用公共 `BackendJavaRouteResolver` 和 `BackendHttpForwarder`，内部路由头到达目标 Java 后禁止二次转发。
...
```json
{
  ...
  "processes": [
    ...
    {
      "backendProcessId": "bjp_backend_b",
      "linuxServerId": "server-b",
      "listenUrl": "http://server-b:8080",
      "instanceId": null,
      "capturedAt": "2026-07-19T12:00:00Z",
      "status": "UNAVAILABLE",
      "errorCode": "PROCESS_TIMEOUT",
      "errorMessage": "Java 进程响应超时",
      "parameters": []
    }
  ]
}
...
进程状态为 `SUCCESS/PARTIAL/FAILED/UNAVAILABLE`；`failedProcesses` 同时统计 `FAILED` 与 `UNAVAILABLE`。单个注册项返回参数键、上次成功加载的数据库源值、当前内存生效值、成功加载时间、最近刷新尝试时间、`SUCCESS/FAILED` 状态和安全错误。运行期刷新失败时保留上一有效 `sourceValue/memoryValue/loadedAt`，只更新最近尝试时间和失败状态。

集群内部分进程失败仍返回 HTTP 200，并保留所有成功结果。未知、已离线、超时或错误路由的单进程请求统一返回 HTTP 503 `OPENCODE_UNAVAILABLE`；非法 `backendProcessId` 返回 `VALIDATION_ERROR`。手工刷新只读数据库并更新目标 JVM，不写参数修改历史、不再次发布 `common-parameter.refresh-requested` 广播。日志只记录脱敏 `traceId`、进程身份和结果状态，不记录源值、内存值或原始异常。本能力不产生 RunEvent/SSE。

### manager 运行公共参数

通用参数表中的 `OPENCODE_MANAGER_MAX_PROCESSES`（`platform=all`，全局单值）是 opencode manager 的最大并发进程数权威来源；`OPENCODE_SESSION_DIR` 和 `OPENCODE_PUBLIC_CONFIG_DIR` 是 manager 启动用户 opencode server 时的 session/config 路径权威来源。manager 通过 WebSocket 控制面连入本服务器后端时，后端在注册成功后只返回 `registered`；manager 收到后主动发送 `configRequest` 拉取完整配置。连接异常断开后，manager 按重连间隔无限重连，重连成功后重新发送 `configRequest`。前端仅允许修改 `editable=true` 的通用参数（`OPENCODE_MANAGER_MAX_PROCESSES`、`OPENCODE_PUBLIC_AGENT_GIT_URL`、`NIGHT_EXECUTION_SLOT_CAPACITY`）；其余为只读部署/初始化参数，修改将影响系统正常运行，接口对只读参数的更新返回 `VALIDATION_ERROR`。对公共 Git 地址参数的修改同样发布 `common-parameter.refresh-requested` 跨实例广播以保证 DB 一致，但不触发 manager 热刷新（URL 属部署参数，由 AgentConfig 在下次公共配置操作时按当前部署模式直读 DB 生效）。最大进程数 `PATCH` 成功后发布进程内 `CommonParameterReloadedEvent`，后端经控制面 WebSocket 把只包含 `maxProcesses` 的 `configUpdate` 广播给当前 Java 实例持有的本服务器 manager 连接；多实例部署下，广播器同时发布 `common-parameter.refresh-requested` 跨实例广播，每台实例收到事件后直接从数据库读取最新最大进程数并只向本服务器 manager 下发。夜间容量修改使用同一跨实例广播，各 Java 实例收到事件后从数据库重新加载本机内存快照；运行中读取失败保留上一有效值。响应中的 `editable` 字段标识是否允许前端修改：`true` 可改，`false` 只读。

manager 收到后按自身端口池容量 `PortEnd-PortStart+1` 做 clamp（超上限 clamp 到容量、`<1` 拒绝）并热更新，成功应用 `configUpdate` 后立即补发一次 `managerHeartbeat` 上报生效值，后续周期 heartbeat 继续同步，`opencode_containers.max_processes` 随之更新。首次或重连后完整 `configUpdate` 中，`OPENCODE_SESSION_DIR` 和 `OPENCODE_PUBLIC_CONFIG_DIR` 是 Java 生成用户 session 与公共配置源的权威参数；Java 启动用户进程时把 `sessionPath` 固定为 `{sessionRoot}/users/{unifiedAuthId}`，把 `OPENCODE_CONFIG_DIR` 固定为该 session 下 `.testagent-runtime/current-public-config` 软链接，并通过 `command.sessionPath/configPath` 显式下发。manager 从 `sessionPath` 固定派生 `HOME=sessionPath`、`XDG_DATA_HOME=sessionPath`、`XDG_CACHE_HOME=sessionPath/.cache`、`XDG_STATE_HOME=sessionPath/.local/state`、`TMPDIR=sessionPath/.tmp`；这些变量和 `OPENCODE_CONFIG_DIR` 会在合并 `command.environment` 后强制覆盖。Linux fork 前，session、cache、state、tmp 必须是 `0755` 普通物理目录，文件/软链接冲突或创建失败时启动失败且不写成功 state。软链接默认指向公共共享目录，公共个人保存只切换本人，公共发布排空时恢复共享目录。OpenCode 原生合并用户全局配置、`OPENCODE_CONFIG_DIR` 和请求工作区 `.opencode`；企业部署必须保证运行用户 `~/.config/opencode` 不维护模型或供应商，模型和供应商只维护在公共配置 Git。任一必需参数缺失、空白、最大进程数非正整数或软链接无法安全创建时，后端/manager 拒绝启动，不回退环境变量、代码默认路径或配置复制。新增目录停止/重启时不清理，存量 PID 需经过受管重启后生效；该目录级隔离不创建独立 OS 用户。该通道不新增 HTTP、WebSocket、DTO 或 RunEvent 字段。
...
| `POST` | `/workspaces/{workspaceId}/git-conflict/complete` | 全部 unmerged 路径解决后提交完整 merge index；合并结果包含 `.opencode/**` 时要求 `APP_ADMIN`。 |
...
`resolution` 支持 `CURRENT`、`INCOMING`、`BOTH`、`MANUAL`、`DELETE`。非冲突路径返回 `CONFLICT`。批量接口请求体为 `{"resolution":"CURRENT"}` 或 `{"resolution":"INCOMING"}`，分别将全部冲突采用个人侧 stage 2 或远程侧 stage 3；目标侧不存在的文件按删除处理，不要求逐个解决。`POST /workspaces/{workspaceId}/git-conflict/abort` 无请求体，仅在存在未完成 merge 时成功。全部冲突解决后仍需调用 `POST /workspaces/{workspaceId}/git-conflict/complete` 提交完整 merge index；响应为 `{"status":"MERGED","headCommit":"...","applicationTargetCommit":"..."}`。该专用入口不会像普通所选文件提交那样重置 index。
...
1. 应用目录加载完成且 `selectedAppId` 为空时，先等待 `GET /recent-workspace` 返回当前仍可见的「全局最近 Workspace」及其 `appId`；该偏好关联的托管应用已撤权、停用或删除时接口返回 `null`，偏好和物理工作区本身不删除。
...
3. 若成员目录与 recent 两个请求之间发生权限变化，导致已返回的 `appId` 不再位于当前账号应用目录：降级到 `apps[0]`，但不会因此自动加载旧工作区。
...
- `WorkspaceRuntimeResponse.appId` / `versionId` / `applicationWorkspaceId` 字段：
  - `GET /recent-workspace`：关联托管应用仍启用且当前用户仍是有效成员时填充；已撤权、停用或删除时整体返回 `null`，不暴露保留工作区的路径和版本信息。非托管兼容工作区仍可返回且三者均为 `null`。
  - `GET /applications/{appId}/recent-workspace`：成员校验通过后填充（工作区不属于任何应用版本时三者均为 `null`），用于前端还原应用 + 模板 + 版本。
  ...
  "managers": [
    {
      ...
      "capabilities": {
        "commands": ["start", "health", "stop", "restart", "stopOwned"]
      },
      "lastHeartbeatAt": "2026-06-24T08:00:00Z",
      "createdAt": "2026-06-24T07:00:00Z",
      "updatedAt": "2026-06-24T08:00:00Z",
      ...
      "managedProcesses": [
        {
          ...
          "sessionPath": "/data/.testagent/agent-opencode/.session/users/A",
          "configPath": "/data/.testagent/agent-opencode/.session/users/A/.testagent-runtime/current-public-config",
          "startedAt": "2026-06-24T07:30:00Z",
          "startCommand": "HOME=/data/.testagent/agent-opencode/.session/users/A XDG_DATA_HOME=/data/.testagent/agent-opencode/.session/users/A XDG_CACHE_HOME=/data/.testagent/agent-opencode/.session/users/A/.cache XDG_STATE_HOME=/data/.testagent/agent-opencode/.session/users/A/.local/state TMPDIR=/data/.testagent/agent-opencode/.session/users/A/.tmp OPENCODE_CONFIG_DIR=/data/.testagent/agent-opencode/.session/users/A/.testagent-runtime/current-public-config opencode serve --hostname 0.0.0.0 --port 4096 --print-logs",
          "traceId": "trace_...",
          "unifiedAuthId": "A",
...
```json
{
  "success": true,
  "data": {
    "ticket": "43-char-url-safe-random-value",
    "expiresAt": "2026-07-20T08:01:00Z",
...
- `ticket` 由 32 字节安全随机数生成，等价于 256 位随机值；有效期取 60 秒与当前平台 Token 剩余时间的较小值。
- Redis 使用兼容 Redis 5+ 的 Lua 脚本原子读删消费，不依赖 Redis 6.2 的 `GETDEL`。票据无效、过期或重复消费时，Admin 表单入口返回 403 状态页；Redis、JIT 或 XXL 登录运行异常返回 503 平台状态页。
- 前端必须用隐藏表单 `POST` 到 `formAction` 的同源 iframe；禁止把 ticket 拼入 URL、query、hash、router state、浏览器存储或原始 HTTP 日志。
- 首次消费按平台稳定用户 ID JIT upsert XXL 管理员；同一用户重复登录幂等，平台改名会同步显示名，重名追加稳定短 hash。
- XXL 会话每次请求校验平台 SHA-256 session marker；平台登出、刷新 Token 或 Token 到期后返回会话失效状态页。
- 此 API 只签发 SSO 票据，不返回 XXL Cookie、不代理 XXL HTML，也不新增 RunEvent/SSE 类型。
- 对应测试：`XxlJobSsoTicketControllerTest`、`RedisXxlJobSsoTicketServiceTest`、`PlatformXxlJobUserProvisionerTest`、`DefaultXxlJobAdminContextLauncherTest`。

### 已作废的 scheduler-management 兼容入口

`/api/internal/platform/scheduler-management/**` 的所有方法和任意后缀均统一返回 HTTP `410` 与平台错误码 `API_GONE`。旧客户端不得继续用这些入口查询、修改、触发或停止周期任务；周期任务的启停、Cron 修改、手动触发、停止和日志查看统一在 XXL iframe 完成。

下面的字段和操作只记录旧客户端契约，供历史排查，不代表当前可调用能力。旧 PostgreSQL 任务定义和运行历史继续保留，但应用内已没有 runner，不再扫描 `CRON`、`MANUAL` 或 `USER_PLAN`。
...
| `triggerType` | 历史可选值：`CRON`、`MANUAL`、`USER_PLAN`；当前入口不再创建任何 scheduler 运行记录。 |
...
- `scheduled_task_plans`、`scheduled_tasks`、`scheduled_task_runs` 只保留历史和兼容审计；短暂停机升级时旧夜间 `PENDING/RUNNING/STOPPING USER_PLAN` 全部标记为 `SKIPPED`，历史数据不删除。
- `opencode-runtime.night-execution` 旧 USER_PLAN handler、管理服务和 runner 已删除；夜间任务改由 XXL 的 `opencode-runtime.night-execution-dispatch` 扫描业务表。
- 上述修改、触发、停止语义均只描述旧客户端，不再有生产实现。
- `TaskResponse`、`RunResponse` 的中文 label 由后端按字典表查询，字典缺失时回退为原 code；不新增通用字典查询 API。
- 分布式互斥只使用 Redis 锁；Redis 不可用时 scheduler 不降级为本机锁。
- 对应兼容测试：`SchedulerManagementControllerTest`。
...
`POST /api/internal/platform/opencode-runtime/night-execution/internal-dispatch` 只供平台 Java 间调用。该精确路径不使用普通用户/静态 API token，必须携带 `XXL-JOB-ACCESS-TOKEN`；Controller 使用常量时间比较并返回统一 `401 UNAUTHENTICATED`。系统转发复用公共 Java resolver/forwarder，先解析目标服务器上选中的精确 backendProcessId，再按进程身份决定本机或转发；携带 traceId 与 `X-Test-Agent-Backend-Routed` 防循环头，超时为 120 秒。

请求只包含固定目标和 1～50 个任务 ID：

```json
{
  "linuxServerId": "linux-night-a",
  "taskIds": ["net_...", "net_..."]
}
```

有效批次逐任务返回 `taskId/status/runId/errorCode`。`STARTED` 与 `ALREADY_STARTED` 为成功并返回稳定 runId；其它状态为 `IN_PROGRESS`、`NOT_FOUND`、`NOT_DUE`、`TARGET_MISMATCH`、`TERMINAL`、`RETRYABLE_FAILURE`、`PERMANENT_FAILURE`。接口只等待普通 Run 返回已受理 runId，不等待 Run 最终结果；请求和响应都不包含 prompt、parts、附件、用户或 attemptId。

对应测试：`NightExecutionControllerTest`、`NightExecutionDtosTest`、`NightExecutionInternalDispatchControllerTest`、`NightExecutionCapacityRegistryTest`、`NightExecutionTaskApplicationServiceTest`、`NightExecutionDispatchCoordinatorTest`、`NightExecutionDispatchServiceTest`、`NightExecutionDispatchLeaseGuardTest`、`NightExecutionRunLifecycleServiceTest`、`NightExecutionReconcileServiceTest`、`UserOpencodeBackendRoutingWebFilterTest`、`BackendHttpForwarderTest`。
...
- `POST /api/internal/platform/opencode-runtime/management/linux-servers/{linuxServerId}/terminal/tickets`：仅 `SUPER_ADMIN` 可创建目标服务器终端 ticket；请求会通过公共 Java 路由器转发到目标服务器。
- `GET /api/internal/platform/opencode-runtime/management/linux-servers/{linuxServerId}/terminal/ws?ticket=...`：服务器终端 WebSocket upgrade；正式环境必须通过按 `linuxServerId` 定向的 WSS 网关访问。
...
当前已覆盖后端 ticket、Origin、workspace/server-shell 目标、单次使用、ticket 创建限流、JVM 内 active 租约、真实 PTY resize、output 截断、结构化审计、idle/hard timeout 和 xterm 前端接入。已有 active PTY 时，新 WebSocket 会返回 `error` envelope，`code=CONFLICT`，并关闭连接；非法 client envelope 返回 `error` envelope，`code=VALIDATION_ERROR`；input/resize 超限返回 `error` envelope，`code=RATE_LIMITED`；idle/hard timeout 返回 `error` envelope，`code=PTY_TIMEOUT`。服务器终端未开启、工作目录不可用或正式环境 WSS 网关未配置时返回 `TERMINAL_UNAVAILABLE`。未新增 SSE 事件或数据库字段。
