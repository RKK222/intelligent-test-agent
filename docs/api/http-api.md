...
成员删除只更新 `application_members.deleted_at`，不影响平台用户、其他应用关系或后续运行态数据。已有 `personal_workspaces`、运行态 `workspaces`、历史 Session 和服务器物理 worktree 均保留：成员撤权后所有托管工作区入口仍实时返回 `FORBIDDEN`，保留记录用于历史归属和以后重新加入时修复 worktree。磁盘回收必须先停止该用户进程、核对目标服务器和未提交内容，再通过 Git worktree 命令清理；不得在成员删除请求中静默 `--force` 删除，也不得手工删除关系型记录。服务器操作见 `docs/deployment/backend.md` 的“应用成员撤权后的个人 worktree 磁盘回收”。
