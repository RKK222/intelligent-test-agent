...
工作空间级接口把同名能力挂在 `/workspaces/{workspaceId}/...`，其中 `files/content/diff/stage/unstage/commit/publish/worktrees/status` 的语义与公共级一致；HTTP `files/content` 同样仅兼容保留，前端文件读写必须通过文件 WebSocket。物理目录为当前运行态 Workspace 或指定 worktree 下的 `.opencode/`，工作空间新增配置包时会初始化 `agents/<name>.md`、`skills/<name>/SKILL.md`、`skills/<name>/rules/` 和 `skills/<name>/templates/`。
