...
`GET /processes/me` 还返回公共配置发布闸门字段：`messageSendAllowed`、`messageSendBlockedReason`、`publicConfigRolloutId`。兼容旧前端时字段缺失等同允许；新前端在 `messageSendAllowed=false` 时禁用新消息和新会话按钮并显示原因。闸门按当前登录用户判断：所有服务器尚未同步完成时全部用户禁发；服务器同步完成后，只阻止当前用户仍未 dispose 的旧 opencode 实例，该实例 dispose 成功后该用户立即恢复，不等待其他用户。该字段只用于提前反馈，`POST /runs` 在 `RunApplicationService` 仍会读取同一持久化闸门强制拒绝 opencode 新 Run，避免绕过前端或轮询延迟产生竞态。
...
| `POST` | `/public/publish` | 在当前用户公共 worktree 中 fetch 并 merge `origin/{公共分支}`，然后以非强制 refspec 把个人分支推送到公共分支；push 成功后建立持久化禁发任务，各服务器同步共享副本并登记本机 manager 进程，旧 Session 空闲后逐实例 dispose；某用户旧实例 dispose 后该用户立即恢复发送，个人 worktree 保持 `ACTIVE`。 |
...
公共 `update`、`update-and-push`、`publish` 的同步广播携带内部 `rolloutId`。发布端在远端 push 成功后先写 `DRAINING` 任务，形成后端禁发硬闸门；每台服务器完成 Git checkout/reset 后，从 Redis manager 心跳筛选本 `linuxServerId` 的已有 opencode 进程写入目标表，再确认服务器同步完成。集群 worker 仅在所有服务器确认后认领目标，并强制只认领 `target.linuxServerId=当前 Java 所在服务器` 的行，由本机 Java 调用本机 opencode；禁止发布端或其他服务器跨机直连代为 dispose。`GET /session/status` 发现 `busy/retry` 时跳过并累计 `retryCount`，空闲时调用一次 `POST /global/dispose`；该接口清理用户 opencode server 内全部 Instance/配置缓存，明确返回 `true` 后，该用户立即恢复发送并在下一次请求重新加载公共配置，不等待其他用户。失败或仍忙按递增退避无限重试；全部目标结束后 rollout 原子改为 `COMPLETED`。发布初始化失败会将任务标记 `FAILED` 并解除闸门；同一时刻只允许一个 `DRAINING` 任务。
...
- 内部服务器广播事件 `agent-config.public-sync-requested` 的 payload 只包含 `branch`、`commitHash`、`reason`、`rolloutId`，`agent-config.operation-progress` 只包含 operation/status/step/command/error/commit 等安全进度字段；envelope 继续携带 `traceId`，不携带文件内容、私钥、token、Authorization 或 Cookie。
