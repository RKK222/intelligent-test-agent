...
| `POST` | `/public/publish` | 在当前用户公共 worktree 中 fetch 并 merge `origin/{公共分支}`，远端 push 前建立持久化禁发任务，再以非强制 refspec 把个人分支推送到公共分支；push 与 rollout 激活确认后即返回发布成功，请求线程不认领本机同步。各服务器由广播或默认 5 秒补偿任务异步同步共享副本并登记本机 manager 进程，旧 Session 空闲后逐实例 dispose；某用户旧实例 dispose 后该用户立即恢复发送，个人 worktree 保持 `ACTIVE`。 |
