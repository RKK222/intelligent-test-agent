...
- 初始化成功必须同时满足 manager 已管理该端口、PID 存活、opencode server HTTP health healthy；仅 manager 返回 `STARTED` 不算成功。启动后 health 为 not-running 时进程快照回写 `STOPPED`，HTTP 不健康时回写 `UNHEALTHY`，异常时回写 `FAILED`，接口按统一错误格式返回，不返回 READY。
...
| `POST` | `/api/internal/platform/opencode-runtime/management/containers/{containerId}/processes/{port}/restart` | 重启指定容器端口上的 opencode server；平台已有用户进程记录时，manager `STARTED` 后还必须 health healthy 才视为成功。 |
