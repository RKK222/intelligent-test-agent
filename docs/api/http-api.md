...
`value` 为空或空白返回 `VALIDATION_ERROR`；`parameterId` 不存在返回 `NOT_FOUND`。成功返回更新后的 `CommonParameterResponse`（单条，非分页）。响应中的 `parameterId` 为业务 ID（如 `param_opencode_app_workspace_root_linux`），不暴露数据库代理主键。
