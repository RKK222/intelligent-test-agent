...
- Agent 配置文件的目录列表、读取、写入和应用文件重命名前端必须走平台文件 WebSocket；应用文件使用 `agent-config.rename`，仅 `APP_ADMIN`（含继承权限的 `SUPER_ADMIN`）可用，公共配置暂不支持重命名；旧 HTTP `public/files*` 和 `workspaces/{workspaceId}/files*` 入口已作废，返回 `410 API_GONE`。
