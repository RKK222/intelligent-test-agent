...
| `GET` | `/public/git-conflicts?worktreeId=&linuxServerId=` | 轻量查询公共配置直接目录或公共 worktree 中未解决冲突文件路径，不返回 patch 内容，用于左侧 Agents 树快速标识冲突。 |
| `GET` | `/public/git-conflict?path=&worktreeId=&linuxServerId=` | 读取公共配置直接目录或公共 worktree 中冲突文件的 Git base/current/incoming stage 与工作树结果。 |
...
工作空间级接口把同名能力挂在 `/workspaces/{workspaceId}/...`，其中 `diff/stage/unstage/commit/publish/worktrees/status` 的语义与公共级一致；文件读写必须通过文件 WebSocket。物理目录为当前运行态 Workspace 或指定 worktree 下的 `.opencode/`，但普通工作空间文件树不重复展示根级 `.opencode`。工作空间级 `diff` 只返回 `.opencode/agents` 与 `.opencode/skills` 下的变更，响应 path 会去掉 `.opencode/` 前缀。工作空间新增配置包时只初始化应用技能包 `skills/<name>/SKILL.md`、`skills/<name>/rules/README.md` 和 `skills/<name>/templates/README.md`，不自动创建 Agent 入口。

公共级和工作空间级 worktree publish 发生 merge 冲突时返回 HTTP 409、错误码 `CONFLICT`，`details.conflictFiles` 携带可展示的冲突文件列表；后端会在返回前执行 `git merge --abort`，不 push、不把 worktree 标记为已发布。`/public/update-and-push` 合并远端分支时发生冲突也返回 `CONFLICT` 和 `details.conflictFiles`，但会保留公共配置仓库中的 Git 原生 merge 现场，供超级管理员通过公共冲突接口解决后再次提交并推送。
...
`POST /public/repositories/{linuxServerId}/pull` 请求体同 `/public/update`，用于超级管理员在“系统管理 → 配置管理 → opencode 公共配置管理”中对指定服务器的公共配置 Git 仓库执行显式拉取。该接口按 `linuxServerId` 路由到目标 Java 后端，使用当前登录管理员唯一 SSH key，在目标服务器本地执行 `fetch/checkout/pull --ff-only`；`discardLocalChanges` 可选且默认 `false`，为 `true` 时只恢复已跟踪本地修改，不删除未跟踪文件。响应为最新的 `PublicRepositoryStatusResponse`。该接口只负责拉取远端最新提交，不提交、不 push。
...
执行顺序为：可选 `reset --hard HEAD`、`fetch`、`stage all`、按需 `commit`、`merge origin/{branch}`、`push`、广播公共配置同步。若本地没有新的可提交文件，仍会继续 merge 和 push，确保已有未推送本地提交不会被 UI 误报成功。若远端有新提交且合并冲突，响应为 HTTP 409 / `CONFLICT`，`details.conflictFiles` 返回冲突文件；前端应刷新公共 Git diff，把 `status=conflict/rawStatus` 的文件作为冲突处理，用户解决全部冲突后再次调用本接口会提交已解决的 merge index 并 push。
...
该接口要求 `SUPER_ADMIN`，所有 Git 操作使用当前登录用户唯一 SSH key；提交说明为空返回 `VALIDATION_ERROR`，push 被远端拒绝（如 non-fast-forward）时返回 `GIT_UNAVAILABLE`，错误 details 携带 `gitFailureType=REMOTE_REJECTED` 与可展示的 `gitFailureHint`，不修改仓库外的状态。
...
| `GET` | `/api/internal/platform/opencode-runtime/sessions/{sessionId}/active-run` | 查询会话最近的非终态 Run；没有时 `data=null`。 |
...
| `POST` | `/api/internal/platform/opencode-runtime/sessions/{sessionId}/questions/{requestId}/reply` | 回复 question，body 为 `{ "answers": [[...], ...], "remoteSessionId": "ses_..." }`；`answers` 为 `List<List<String>>`，外层按子问题顺序排列，内层是该问题的选中 label，一次回复覆盖同一请求下的全部子问题。平台也兼容扁平 `string[]`，按单问题整体包成单个内层数组。`remoteSessionId` 可选，task 子会话 ask 必须使用 `question.asked.sessionId/sessionID` 透传；该字段只覆盖 opencode question path 的远端 session 段，平台 `sessionId` 仍用于鉴权和定位原绑定。缺失时后端使用平台 Session 绑定的 root remote session。 |
| `POST` | `/api/internal/platform/opencode-runtime/sessions/{sessionId}/questions/{requestId}/reject` | 拒绝 question，body 可选 `{ "remoteSessionId": "ses_..." }`，语义同 reply。 |
| `GET` | `/api/internal/platform/opencode-runtime/sessions/runtime-state` | 查询当前登录用户历史会话运行态摘要，用于前端派生历史入口运行计数和 ask 提醒。 |
...
用户级会话运行态摘要只面向已登录用户，统计范围与当前用户可见历史会话一致：会话创建人、Run 触发人或消息发送人为当前用户，且会话仍为 ACTIVE。`runningCount` 只统计 `PENDING/RUNNING/CANCELLING` Run；同一会话只返回最近一个非终态 Run。`questionCount` 只统计最新 question 状态仍为 `question.asked` 的运行中会话；收到 `question.replied`、`question.rejected` 或 Run 终态后，该会话会从待关注集合移除。历史按钮展示数字不直接使用全量 `runningCount`，而是由前端按历史第一页 30 条会话与 `sessions[]` 交集派生。
...
- `sessionId` 为平台 session id。无用户主体时，后端通过 `agent_session_bindings` 中的 `(sessionId, agentId)` 定位远端 session；`opencode` 会兼容读取旧 `sessions.opencode_*` 字段并回填 binding，未绑定远端 session 时返回 `CONFLICT`。有用户主体且 agent 为 `opencode` 时，普通 session runtime 操作在缺失或不匹配绑定时会自动在当前用户进程上重建。
- `permission`/`question` 的平台路径和请求体保持兼容，后端实际映射到 opencode v2 session-scoped API：`GET /api/session/{remoteSessionId}/permission`、`POST /api/session/{remoteSessionId}/permission/{requestId}/reply`、`GET /api/session/{remoteSessionId}/question`、`POST /api/session/{remoteSessionId}/question/{requestId}/reply`、`POST /api/session/{remoteSessionId}/question/{requestId}/reject`。pending permission/question list/reply/reject 使用平台 Session 当前 `AgentSessionBinding` 的 remoteSessionId 和 execution node，不因认证用户当前 opencode 进程变化而调用 `sessionExists`、`createSession` 或切换绑定节点。list 会在 root session-scoped 列表外合并同 workspace/directory 的全局 pending request，覆盖 task 子会话 ask；reply/reject 首次遇到 404 时会按全局 pending request 的 `requestId -> sessionID` 反查真实 owner session，并改用该远端 session 重试一次。question reply/reject 仍可通过请求体 `remoteSessionId` 覆盖 opencode question path 的远端会话段，用于 task 子会话 ask；平台 `sessionId` 仍用于鉴权、workspace directory 和原 binding 定位。permission/question list/reply/reject 重试后仍遇到 opencode 404 会转为平台 `CONFLICT`，消息为“权限请求已失效，请重新运行任务”或“提问请求已失效，请重新运行任务”，`details.reason=STALE_RUNTIME_REQUEST`；其它 opencode 404 映射不变。前端收到 question stale 409 时保留当前提问卡片并刷新 pending question 列表，不能直接按 `question.replied` 清理卡片。
...
- `OpencodeRuntimeApplicationServiceTest`：验证 workspace directory、用户进程节点路由、固定节点 fallback、远端 session id、binding mismatch 自动重建、permission/question v2 session-scoped path、全局 pending request owner 兜底重试、question child remote session override、reply body 兼容、过期请求 `CONFLICT` 映射、MCP resources/tools、config/provider OAuth/worktree/share/MCP auth 映射。
...
`GET /api/internal/platform/opencode-runtime/sessions/{sessionId}/active-run` 返回最近的 `PENDING`、`RUNNING` 或 `CANCELLING` Run，供前端刷新后恢复 RunEvent SSE；没有非终态 Run 时响应仍为 `success=true` 且 `data=null`。
...
```json
{
  ...
  "webSocketUrl": "/api/internal/platform/opencode-runtime/sessions/ses_.../terminal/ws?ticket=tty_..."
}
```

`webSocketUrl` 固定返回 `/api/internal/platform/opencode-runtime/sessions/{sessionId}/terminal/ws?ticket=...`。
