...
| `POST` | `/public/publish` | 在当前用户公共 worktree 中 fetch 并 merge `origin/{公共分支}`，然后以非强制 refspec 把个人分支推送到公共分支；成功后更新共享运行时副本并广播同步，个人 worktree 保持 `ACTIVE`。 |
...
- 新建公共/工作空间 Agent worktree 均返回并保存 `linuxServerId`。公共 worktree 后续文件目录列表、读取、写入按落库服务器归属通过文件 WebSocket route/ticket/RPC 执行；diff/stage/unstage/commit/publish 仍按现有 HTTP 后端代理执行，并在目标后端再次校验创建人。
- 浏览器选择已初始化公共仓库服务器后，自动调用 `GET /public/worktrees` 并在缺失时调用 `POST /public/worktrees`，随后始终使用当前用户 worktree；不再提供切换他人 worktree 或回退共享直接目录编辑的入口。
