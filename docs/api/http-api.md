...
Base URL：`/api/internal/platform/workspace-management/agent-config`。该能力管理工作台左侧 Agent 栏目中的“公共级”和“工作空间级”配置文件。公共级 Git 根目录来自通用参数 `OPENCODE_PUBLIC_CONFIG_GIT_ROOT`，opencode 运行时配置目录来自 `OPENCODE_PUBLIC_CONFIG_DIR`，标准 agent 目录为 Git 根目录下 `opencode/agents/`，读取兼容 legacy `opencode/agent/`；工作空间级标准目录为 `{workspace.rootPath}/.opencode/agents/`，读取兼容 `.opencode/agent/`。写入始终落到标准复数目录。
...
| `GET` | `/public/repositories` | 查询 Redis 中当前仍在线的后端服务器及当前后端的公共配置仓库初始化状态；响应不包含目标后端 `listenUrl`。 |
...
| `GET` | `/public/files?path=&worktreeId=` | 兼容保留：列出本机公共 agent 目录一层子项，前端不再使用。 |
| `GET` | `/public/files/content?path=&worktreeId=` | 兼容保留：读取本机公共 agent 文件，前端不再使用。 |
| `PUT` | `/public/files/content` | 兼容保留：写入本机公共 agent 文件，前端不再使用。 |
...
工作空间级接口把同名能力挂在 `/workspaces/{workspaceId}/...`，其中 `files/content/diff/stage/unstage/commit/publish/worktrees/status` 的语义与公共级一致；HTTP `files/content` 同样仅兼容保留，前端文件读写必须通过文件 WebSocket。物理目录为当前运行态 Workspace 或指定 worktree 下的 `.opencode/agents/`。
...
| 参数 | 说明 |
|---|---|
| `platform` | 可选平台过滤值：`windows`、`linux`、`all`；缺失表示不过滤。非法值返回 `VALIDATION_ERROR`。 |
| `page` | 页码，默认 `1`。 |
...
- Java 后端不检查本机 `OPENCODE_PUBLIC_CONFIG_DIR` 目录；初始化先按当前候选中进程数最少且有空闲端口的容器选择目标 manager，再下发 `start`。目标 manager 在所在服务器检查 `OPENCODE_PUBLIC_CONFIG_DIR` 必须是已存在且非空的目录；缺失、为空、非目录或不可读时返回 `FAILED + errorCode=OPENCODE_UNAVAILABLE`，`message` 为“公共配置未初始化，请联系管理员。”，不会创建 session、不会启动 opencode server，Java 将该结果映射为同码平台错误。
