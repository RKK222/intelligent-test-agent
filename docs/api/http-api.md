...
`GET /processes/me` 还返回公共配置发布闸门字段：`messageSendAllowed`、`messageSendBlockedReason`、`publicConfigRolloutId`。兼容旧前端时字段缺失等同允许；新前端在 `messageSendAllowed=false` 时禁用新消息和新会话按钮并显示原因。该字段只用于提前反馈，`POST /runs` 在 `RunApplicationService` 仍会读取同一持久化闸门强制拒绝 opencode 新 Run，避免绕过前端或轮询延迟产生竞态。
...
| `POST` | `/public/publish` | 在当前用户公共 worktree 中 fetch 并 merge `origin/{公共分支}`，然后以非强制 refspec 把个人分支推送到公共分支；push 成功后建立持久化禁发任务，各服务器同步共享副本并登记本机 manager 进程，旧 Session 空闲后逐实例 dispose，全部完成才恢复发送；个人 worktree 保持 `ACTIVE`。 |
