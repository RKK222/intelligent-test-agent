...
应用 Agent `publish` 仍先提交当前个人分支，再把所选 `.opencode` 白名单投影并推送应用 feature。远端确认后，每台服务器把 feature 固定提交中的 `.opencode/opencode.json(c)`、`agents/**`、`skills/**` 反向投影到该版本干净的个人 worktree，并用 `git commit --only` 生成同步提交，个人分支中其它 staged docs 保持原状。某用户任一相关个人 worktree 的白名单路径存在本地修改时，该用户整组跳过、写入 `APPLICATION_TO_PERSONAL/FAILED` 审计且不进入 dispose 目标；普通 docs/spec 的 dirty 或 staged 状态不阻塞。OpenCode 不增加应用共享配置目录或覆盖层。
...
5. 在 feature worktree 提交投影结果并 `git push origin {appVersionBranch}`；个人分支不 push。普通 docs 发布到此结束，其他用户通过 `workspace.version.sync-requested` 感知新版本并由本人手动更新个人 worktree。应用 Agent 使用同一前半段流程，只把选中的 `.opencode/**` 从个人 `HEAD` 投影到 feature；push 后再由 rollout 把固定提交的 Agent 白名单反向投影到各服务器干净的相关个人 worktree，并只 dispose 同步成功用户。服务端在准备 feature worktree 前对所有角色强制拒绝 `spec/**`，`SUPER_ADMIN` 也不能绕过目录规则，其它未选文件仍不会泄漏。
