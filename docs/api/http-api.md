...
`GET /processes/me` 还返回公共配置发布闸门字段：`messageSendAllowed`、`messageSendBlockedReason`、`publicConfigRolloutId`。兼容旧前端时字段缺失等同允许；新前端在 `messageSendAllowed=false` 时禁用新消息、新会话和宠物旁路问答并显示原因，此期间每 5 秒复查强状态，当前用户旧实例 dispose 后立即恢复并停止额外轮询，不等待其他用户。闸门按当前登录用户判断：所有服务器尚未同步完成时全部用户禁发；服务器同步完成后，只阻止当前用户仍未 dispose 的旧 opencode 实例。该字段只用于提前反馈；`POST /runs`、旁路问答以及 opencode command/shell Session 的后端入口都会读取同一持久化闸门强制拒绝新消息，避免绕过前端或轮询延迟产生竞态。
...
公共 `update`、`update-and-push`、`publish` 的同步广播携带内部 `rolloutId`。发布端在远端 push 成功后先写 `DRAINING` 任务、发起人用户 ID 以及持久化服务器清单（包含发布瞬间离线的已登记服务器），形成后端禁发硬闸门；一旦该任务建立，广播失败、服务器离线或 Java 重启都只会保留 `PENDING/DRAINING` 并由定时补偿继续处理，不允许以失败状态提前开闸。每台服务器使用发起人的已存 SSH 凭据把本机共享运行仓库 checkout/reset 到目标 commit；只有取得本服务器 manager 的实时进程清单、把已有 opencode 进程及其用户快照写入目标表后，才确认该服务器同步完成。凭据只在目标 Java 从数据库读取并解密，不进入广播 payload。
