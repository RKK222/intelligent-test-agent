...
Agent 配置文件上传使用 `agent-config.upload`，请求携带 `scope`、可选 `workspaceId`、可选 `worktreeId`、`path` 和 `contentBase64`；公共 scope 仅 `SUPER_ADMIN`，工作空间 scope 仅 `APP_ADMIN`（`SUPER_ADMIN` 继承）。上传复用工作空间文件服务的 Base64、大小、重名和路径安全校验，不覆盖同名条目；应用级额外限制在 `opencode.jsonc`、`agents/**`、`skills/**` 白名单内。
...
WebSocket 协议版本固定为 `opencode-manager.v1`。文本帧是 JSON envelope，稳定 `type` 包括 `register`、`registered`、`configRequest`、`configUpdate`、`managerHeartbeat`、`command`、`commandResult`、`error`；`backendListRequest`、`backendListResponse` 和旧 `heartbeat` 只作为兼容枚举保留，不作为新运行路径主入口，Java 收到 `backendListRequest` 会忽略。manager 发出的 `register` 与 `managerHeartbeat` 可带 `buildVersion?: string`，值为二进制构建时按北京时间注入的 `VyyyyMMdd.HHmmss`；旧 manager 缺字段时 Java 保持 `null`，不会阻断连接。`configUpdate` 首次由 manager 的 `configRequest` 拉取，完整帧返回 `maxProcesses`、`sessionRoot`、`configDir`，其中 `sessionRoot/configDir` 分别来自 `common_parameters.OPENCODE_SESSION_DIR` 与 `OPENCODE_PUBLIC_CONFIG_DIR`；后续最大进程数刷新帧只携带 `maxProcesses`，`sessionRoot/configDir` 为空表示路径不变。`start` command 在 `sessionPath` 外显式携带可选 `configPath`；新版 Java 固定传用户受管软链接，新版 manager 使用它设置 `OPENCODE_CONFIG_DIR`，旧命令缺失时才兼容公共共享 `configDir`。企业部署要求运行用户的 `~/.config/opencode` 不维护模型或供应商。`managerHeartbeat` 每 5 秒由 manager 通过本服务器 Java socket 发送一次，后端写入 Redis manager 快照，TTL 为 10 秒，同时把容器资源指标追加到 Redis 48 小时历史 ZSET；资源指标可带 `metricsSource=cgroup|process|unavailable`。`managerHeartbeat.managedProcesses[]` 可选携带本 manager 管理的 opencode server 明细：`port`、`pid`、`baseUrl`、`sessionPath`、`configPath`、`startedAt`、`startCommand`、`traceId`；`startCommand` 是 manager 生成的安全展示命令，不包含 manager token、用户 JWT、Cookie、prompt 或 API key。manager 生成心跳前会清理 PID 已不存在的本地 stale state。后端命令使用 `commandId=mcmd_...`，manager 回包必须带同一 `commandId` 和 `traceId`；目标配置不可加载或健康旧进程路径与显式路径不一致时 `start` 返回失败，不会复用错误运行态。当前命令集合为 `start`、`health`、`stop`、`restart`；收到完整 `configUpdate` 前，`start`/`restart` 返回 `FAILED`。运行管理命令继续由统一 Java 路由转发到本机 manager；`stop` 对 state 存在但 OS 进程已结束的端口幂等清理，成功的配置应用和进程命令都会立即补发 heartbeat。
