...
`POST /public/repositories/{linuxServerId}/pull` 请求体同 `/public/update`，用于超级管理员在“系统管理 → 配置管理 → opencode 公共配置管理”中对指定服务器的公共配置 Git 仓库执行显式拉取。该接口按 `linuxServerId` 路由到目标 Java 后端，使用当前登录管理员唯一 SSH key，在目标服务器本地执行 `fetch/checkout/pull --ff-only`；内部部署公共 Git 地址同样按当前用户统一认证号拼接实际 SSH URL。`discardLocalChanges` 可选且默认 `false`，为 `true` 时只恢复已跟踪本地修改，不删除未跟踪文件。响应为最新的 `PublicRepositoryStatusResponse`。该接口只负责拉取远端最新提交，不提交、不 push。`configDirPath` 必须由公共配置 Git 仓库初始化后产生且非空，后端不会在 manager 启动时创建空配置目录。
...
- 公共 Git 远端分支和远端目录只读查询允许最长 60 秒；后端日志会记录 `event=git_command_timeout|git_command_failed|git_command_slow` 以及公共配置入口的 `event=agent_config_public_branches_*` / `event=agent_config_public_repository_initialize_*`，用于按 traceId 追踪具体 Git 阶段、目标服务器、脱敏后的 Git URL 和耗时。
