...
- `test-agent.model-catalog.source=opencode` 时，`/api/models` 和 `/api/providers` 保持原生行为，直接代理 opencode。
- `source=external` 时，`/api/models` 由后端请求外部 OpenAI-compatible `/models` 获取；请求失败时返回配置内置外网模型，Provider 默认为 `external-openai`。
