...
| `POST` | `/public/update-and-push` | 公共配置"更新+提交并推送"复合操作：按分支 fetch/reset/pull 后用 commitMessage stage + commit 当前工作区并 push 到远端，再广播同步；`discardLocalChanges` 语义与 `/public/update` 一致。 |
...
`commitMessage` 必填且不能为空字符串。流程为：先按 `branch` 完成与 `/public/update` 相同的 fetch/reset/pull（受 `discardLocalChanges` 控制），再对工作区执行 `git add --all`，若产生新的本地 commit 则用 `commitMessage` 生成提交并 `git push` 到远端；最后广播同步事件并返回最新 commit hash。该接口要求 `SUPER_ADMIN`，所有 Git 操作使用当前登录用户唯一 SSH key；提交说明为空或 push 失败时返回 `VALIDATION_ERROR`/`INTERNAL_ERROR`，不修改仓库外的状态。

`GET /public/repositories` 响应元素字段：

响应按 `linuxServerId` 唯一返回服务器行；同一 IP 上 Java 进程重启导致 Redis TTL 窗口内同时存在多个 `backendProcessId` 快照时，后端会合并为一条公共配置仓库状态，前端也以 IP 作为去重 key。
...
- Agent 配置进度不走 RunEvent SSE，也不写入 `run_events`；浏览器只通过 ticket WebSocket 或 `GET /operations/{operationId}` 查看。多实例部署开启服务器广播后，执行实例会通过 `agent-config.operation-progress` 广播安全进度字段，让连接到非执行实例的页面也能收到进度；未启用广播时仍可通过 `GET /operations/{operationId}` 查看最终状态。
- 内部服务器广播事件 `agent-config.public-sync-requested` 的 payload 只包含 `branch`、`commitHash`、`reason`，`agent-config.operation-progress` 只包含 operation/status/step/error/commit 等安全进度字段；envelope 继续携带 `traceId`，不携带文件内容、私钥、token、Authorization 或 Cookie。
...
| `PATCH` | `/repositories/{repoId}` | 编辑中文名称、英文名称和是否测试智能体标准代码库。 |
...
- `englishName` 为必填英文名称，仅允许 1 到 29 位英文字母，后端统一按小写保存；非空值唯一。历史数据可能为 `null`，但缺少英文名称的历史代码库不能再创建新的应用版本工作区。
...
通用参数（`common_parameters`）保存跨模块共享的稳定运行参数（如 `OPENCODE_APP_WORKSPACE_ROOT`、`OPENCODE_PUBLIC_AGENT_GIT_URL` 等路径与 Git 参数）。本接口只允许已认证用户且角色包含 `SUPER_ADMIN` 访问：未认证返回 `UNAUTHENTICATED`，非超级管理员返回 `FORBIDDEN`，非法平台值或空参数值返回 `VALIDATION_ERROR`，参数不存在返回 `NOT_FOUND`。**接口仅提供列表查询与「仅修改 value」的更新，不提供新增/删除**，保证通用参数集合稳定。

参数值支持变量引用：值中可使用 `${englishName}` 引用其他通用参数的值，读取时按目标参数自身平台展开（`platform=all` 的参数只能引用同为 `all` 的参数；`linux`/`windows` 参数按"先当前平台、再回退 all"解析被引用参数）。循环引用、缺失引用或超深度嵌套（上限 16 层）时保留字面 `${...}` 占位符不展开，不抛异常；管理端通过加载值接口可查看每个进程的展开结果与解析状态。所有通用参数在启动时全量加载到内存，消费方走内存读取；数据库修改后通过 `common-parameter.refresh-requested` 广播通知其他后端实例刷新内存。
...
| `GET` | `/load-snapshots` | 查询每个后端 Java 进程内存中加载的通用参数原始值与展开值。 |
...
`value` 为空或空白返回 `VALIDATION_ERROR`；`parameterId` 不存在返回 `NOT_FOUND`。成功返回更新后的 `CommonParameterResponse`（单条，非分页）。响应中的 `parameterId` 为业务 ID（如 `param_opencode_app_workspace_root_linux`），不暴露数据库代理主键。修改成功后自动记录修改日志，包含修改前后的值、修改用户和修改时间。
...
`GET /load-snapshots` 返回所有存活后端 Java 进程的加载快照（响应体 `List<LoadSnapshotResponse>`）：

```json
[
  {
    "backendProcessId": "bjp_xxx",
    "linuxServerId": "10.8.0.12",
    "listenUrl": "http://10.8.0.12:8080",
    "instanceId": "instance-a",
    "loadedAt": "2026-06-28T00:00:00Z",
    "parameters": [
      {
        "englishName": "OPENCODE_APP_WORKSPACE_ROOT",
        "platform": "linux",
        "rawValue": "${OPENCODE_DATA_ROOT}/appworkspace",
        "resolvedValue": "/data/test-agent/appworkspace",
        "hasReference": true,
...
`rawValue` 为数据库原始值，`resolvedValue` 为展开后的值；`hasReference` 标识原始值是否含 `${...}`、`$NAME`、路径开头 `$HOME` 或 `~/`。`${NAME}` 先按通用参数引用解析，未命中时再读取 Java 后端进程环境变量；`$NAME` 直接读取环境变量。`resolutionError` 非 null 表示展开后仍残留未解析占位符（循环、缺失、环境变量不存在或 `all` 引用了平台参数）。快照来源为每台后端实例刷新内存缓存后写入 Redis 的加载快照，TTL 30 秒，进程崩溃后短期自动消失。该接口仅用于核对各进程内存加载是否一致，不产生 RunEvent/SSE。
...
`POST /api/workspaces/{workspaceId}/file-ws-route` 使用当前登录用户的 `opencode` 进程服务器归属定位同服务器后端，返回浏览器应直连的目标后端地址。该路由查询只读取 ACTIVE binding 和可恢复进程记录，不下发 opencode-manager `health` 或 `start` 命令；工作区服务器归属、用户 opencode 进程服务器和目标后端服务器不一致时返回统一 `CONFLICT`。本地换 IP 或切换测试库后，若历史 workspace 仍绑定旧 `linuxServerId`，且旧服务器没有在线后端快照、当前 opencode 进程在本后端、workspace 根目录在本机可访问，后端会在路由时把 workspace 回绑到当前服务器；多机环境中旧服务器仍在线或目录不可访问时不会自动迁移。Run 启动、用户进程初始化和 `/processes/me` 状态查询仍按用户进程 API 执行强健康检查。
...
| `GET` | `/api/internal/platform/opencode-runtime/management/linux-servers/{linuxServerId}/backend-metrics` | 主接口：按服务器 IP 查询后端 Java 服务 Redis 中近 48 小时运行指标历史。 |
| `GET` | `/api/internal/platform/opencode-runtime/management/backend-processes/{backendProcessId}/metrics` | 兼容入口：先解析当前后端进程所属 `linuxServerId` 后委托 IP-based 查询，无法解析时才回退旧样本读取。 |
| `POST` | `/api/internal/platform/opencode-runtime/management/containers/{containerId}/processes/{port}/restart` | 重启指定容器端口上的 opencode server，manager 返回 `STARTED` 视为成功。 |
| `POST` | `/api/internal/platform/opencode-runtime/management/containers/{containerId}/processes/{port}/stop` | 停止指定容器端口上的 opencode server，manager 返回 `STOPPED` 视为成功。 |
