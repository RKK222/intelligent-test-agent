...
- `permission`/`question` 的平台路径和请求体保持兼容，后端实际映射到 opencode v2 session-scoped API：`GET /api/session/{remoteSessionId}/permission`、`POST /api/session/{remoteSessionId}/permission/{requestId}/reply`、`GET /api/session/{remoteSessionId}/question`、`POST /api/session/{remoteSessionId}/question/{requestId}/reply`、`POST /api/session/{remoteSessionId}/question/{requestId}/reject`。question reply/reject 可通过请求体 `remoteSessionId` 覆盖远端会话段，用于 task 子会话 ask；平台 `sessionId` 仍用于定位当前用户进程、workspace directory 和鉴权归属。仅在 permission/question 回复或拒绝时，opencode 404 会转为平台 `CONFLICT`，消息为“权限请求已失效，请重新运行任务”或“提问请求已失效，请重新运行任务”，`details.reason=STALE_RUNTIME_REQUEST`，用于前端清理已过期卡片；其它 opencode 404 映射不变。
