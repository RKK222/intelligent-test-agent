...
**排序优化**：分支列表按以下规则排序：
- 符合格式的分支排在前面（可选择），**按日期倒序排序（最新的在前）**
- 不符合格式的分支排在后面（置灰不可选），按字母顺序排序
...
**预期结果**：
- ✅ 符合格式的分支排在前面，可以点击选择，**按日期倒序排列（最新的在前）**
- ❌ 不符合格式的分支排在后面，置灰，无法选择，按字母顺序排列
- **默认选中第一条符合格式的分支（即最新的分支）**
- **如果没有任何符合格式的分支，选中空**
...
1. **用户体验**：
   - 符合格式的分支优先展示，提升操作效率
   - **最新的分支排在最前面，便于用户选择最新版本**
   - 置灰而非隐藏分支，让用户看到所有可用的分支
   - 明确的错误提示，告知用户正确格式
   - 实时校验，不需要等到提交时才发现错误
   - **默认选中可用分支（最新的），避免用户看到已选中但无法操作的尴尬**
