...
- Spring 单例初始化不得直接启动 executor；配置中的任意 Admin readiness 返回 HTTP 200 前，executor 端口和注册线程保持关闭，Admin 恢复后只启动一次。多 Admin 探测必须接受“前一节点非 200、后一节点 200”，并拒绝非法地址、重定向和其它非 200 响应。
...
单节点启动日志还必须确认：平台 Netty 可以先于 Admin 就绪；Admin Tomcat/readiness 完成后才出现 executor 启动和 `ExecutorRegistryThread` 启动；同一次启动区间不得出现指向尚未监听 Admin 的 `registry error` / `Connection refused`。
