...
| 公共 Agent 发布排空旧 Session | 1. 超管在个人 worktree 提交并推送；2. 检查各服务器共享 Git commit；3. 检查 rollout/server/target 表；4. 模拟用户 A 的实例 busy、用户 B 的实例 idle；5. 等待重试 | `rollout.status=DRAINING`；A、B 初始 `messageSendAllowed=false` | push 后立即禁发；每台服务器同步后登记本机进程且只能处理本服务器目标；B 的 `/global/dispose` 明确成功后 B 立即变为 `messageSendAllowed=true`，A 仍为 false 且 `retry_count+1`；A 后续 idle 并 dispose 后也恢复，最后 rollout 为 `COMPLETED` |
