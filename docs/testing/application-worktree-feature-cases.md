...
| 应用 feature 工作区普通文件写操作 | 只读，前端不展示写入口 | 只读，前端不展示写入口 | 只读；超管通过个人 HEAD 投影发布全部路径，避免形成第二套直提协议 |
| `docs/**` 从个人 HEAD 投影并推送 feature | 允许 | 允许 | 允许 |
| `spec/**` 从个人 HEAD 投影并推送 feature | 禁止，仅本地提交 | 禁止，仅本地提交 | 允许 |
...
```mermaid
flowchart TD
  A[选中应用文件并暂存] --> B{当前角色是 SUPER_ADMIN}
  B -- 是 --> C[提交到个人 HEAD]
  C --> D[全部选中文件投影到 feature]
  B -- 否 --> E{路径是否位于 spec}
  E -- 是 --> F[只提交个人 HEAD]
  E -- 否 --> G[提交个人 HEAD]
  G --> H[允许文件投影到 feature]
...
| 个人 HEAD 已提交 | 提交并推送 | 允许路径投影到 feature 并广播 | 普通成员/应用负责人选择 `spec/**` 时只保留本地提交 |
...
| 普通成员只选择 spec | 1. 暂存 `spec`；2. 点击提交并推送；3. 检查请求 | `path=spec/payment/design.md`；`roles=[USER]` | 只执行个人 worktree 本地提交；不调用 feature 发布；页面明确提示未推送 |
...
| 超管拥有应用与公共配置写权限 | 1. 以超级管理员进入同一应用；2. 检查 feature 文件、个人 worktree、应用配置和公共配置操作入口 | `roles=[SUPER_ADMIN]`；`workspaceId=wrk_feature_readonly_20260715`；`personalWorkspaceId=psw_member_default_20260715` | feature 发布副本仍无普通文件直写入口；个人 worktree 可写且所有路径均可发布；应用配置初始化/创建 worktree 可用；公共 Git 操作可用 |
| 超管发布 spec | 1. 在个人 worktree 提交 spec；2. 点击提交并推送；3. 检查 feature 投影和 push | `roles=[SUPER_ADMIN]`；`path=spec/payment/design.md`；`message=spec: 超管发布设计` | 前端不剔除 spec；后端允许投影、提交和推送 feature；返回 `PUBLISHED` 且 `remotePushed=true` |
...
| UI 自动化固定数据回归 | 1. 执行组件测试；2. 执行 Playwright 权限用例；3. 检查失败截图和 trace | `corepack pnpm vitest run apps/agent-web/tests/git-changes-panel.test.ts packages/file-explorer/tests/DirectoryRows.test.ts`；`corepack pnpm exec playwright test apps/agent-web/tests/workbench.spec.ts --grep "application workspace mutation entries"` | 组件测试验证 DOM 读写入口与 API 调用；浏览器用例验证普通成员隐藏应用配置写入口、超管显示入口；全部通过且无控制台错误 |

# 3. 案例审核结果

| 审核项 | 审核结果 | 说明 |
| --- | --- | --- |
| 需求/设计覆盖 | 补充后通过 | 已覆盖个人 worktree、feature、docs、spec、应用配置、公共 Git、广播和超管豁免 |
