...
| 公共 Agent 发布排空旧 Session | 1. 超管在个人 worktree 提交并推送；2. 检查各服务器共享 Git commit；3. 检查 rollout/server/target 表；4. 模拟不同服务器各有 busy/idle 实例；5. 等待重试 | `rollout.status=DRAINING`；linux-1 实例 A `/session/status=busy`，linux-2 实例 B `idle`；前端进程响应 `messageSendAllowed=false` | push 后立即禁发；每台服务器同步后登记本机进程；linux-1 只认领 A、linux-2 只认领 B，禁止跨服务器直连；B 由 linux-2 调用 `/global/dispose`，A 由 linux-1 跳过且 `retry_count+1`；A 后续 idle 时由 linux-1 dispose；全部完成后 rollout 为 `COMPLETED` 并恢复发送 |
