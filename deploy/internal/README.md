...
平台与 MySQL 固定分为两个 U 盘交付包。平台代码更新时只重建、传输平台包；MySQL 镜像或 `.147`
配置变化时才重建 MySQL 包：
...
deploy/internal/package-release.sh --mysql-only \
  --output-dir /path/to/mysql-image-output
deploy/internal/package-mysql-offline.sh \
  --image-tar /path/to/mysql-image-output/mysql_8.4-linux-amd64.tar \
  --nodes-dir /path/to/prepared-node-packages \
  --output-dir /path/to/usb-output
```
...
三个应用节点包及后台/前端入口；MySQL ZIP 顶层固定为 `test-agent-mysql-offline/`，只包含
`mysql:8.4` 离线镜像、`.147` 节点包和 `deploy-mysql-node.sh`。入口从本机网卡取 IP，连续执行节点包
校验、预校验、正式部署和部署后校验，完整输出保存在交付目录上一层的 `deploy-<本机IP>.log`。
企业内部中转机首次部署接收两对文件；以后普通平台更新只接收平台 ZIP 和 SHA，不再重复传 MySQL 大包。
...
MySQL 镜像只存在于独立 MySQL 包。企业服务器只执行校验、解压、`docker load` 和服务启停，不执行
...
企业内所有目标服务器统一使用 `/data/0709/`：`.4/.114/.2` 只上传平台 ZIP 与 SHA，`.147` 只上传
MySQL ZIP 与 SHA。文件名保持不变：

```text
/data/0709/test-agent-two-backend-complete.zip
/data/0709/test-agent-two-backend-complete.zip.sha256
/data/0709/test-agent-mysql-offline.zip
/data/0709/test-agent-mysql-offline.zip.sha256
```
...
1. 首次部署或 MySQL 交付物变化时，在 `122.233.30.147` 解压独立 MySQL 包并执行 `deploy-mysql-node.sh`，离线导入 MySQL 8.4 linux/amd64 镜像，初始化 `xxl_job` 库和最小权限账号；普通平台升级不执行本步。
...
- 分包一键入口：平台包使用 [deploy-backend-node.sh](deploy-backend-node.sh)、
  [deploy-frontend-node.sh](deploy-frontend-node.sh)，MySQL 包使用 [deploy-mysql-node.sh](deploy-mysql-node.sh)；本机 IP 自动识别且校验、部署、复验输出统一落盘
