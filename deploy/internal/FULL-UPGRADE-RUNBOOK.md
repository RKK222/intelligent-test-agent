...
### 12.1 Docker 18.09 千端口池前置条件

`.4/.114` 每台 worker 固定发布 `14096-15095` 共 1000 个端口。现场 Docker
18.09.7 在默认 `userland-proxy` 下实测到第 536 个端口时报
`iptables ... fork/exec ... resource temporarily unavailable`；Docker service、父 cgroup、内核
PID 和内存均未达上限，不能通过反复执行 `docker run` 绕过。两台后台都必须先在
`/etc/docker/daemon.json` 合并配置 `"userland-proxy": false` 并按批准的维护窗口重启
Docker。`opencode-worker-docker.sh` 会在删除现有 worker 容器前拒绝“Docker 18.09 +
千端口 + userland proxy”组合，不修改 manager state。

`.4` 上还运行 `postgres` 容器，修改 daemon 前必须先核对其 restart policy 和 mounts，
用 `docker stop -t 60 postgres` 优雅停止；Docker 重启后显式 `docker start postgres`，确认
...
docker ps -a --filter name=^/test-agent-opencode-worker$ \
  --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}'
docker port test-agent-opencode-worker | grep -E '^(14096|15095)/tcp'
...
成功条件：Java/XXL readiness 通过；`.serverid=test-agent-backend-122-233-30-114`；`.serverhost=122.233.30.114`；worker 端口首尾为 14096/15095，映射数为 1000；`PidsLimit=8192`；无 Redis 认证错误。
...
4. 使用 `ss -lntp` 检查 14096 和 15095 的真实占用者，不直接删除 state。Docker 18.09 如报 `resource temporarily unavailable`，先核对 `userland-proxy=false`，不提高 manager 进程上限或反复重试。
