...
docker ps -a --filter name=^/test-agent-opencode-worker$ \
  --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}\t{{.Ports}}'
docker port test-agent-opencode-worker | grep -E '^(14096|16095)/tcp'
...
成功条件：Java/XXL readiness 通过；`.serverid=test-agent-backend-122-233-30-4`；`.serverhost=122.233.30.4`；worker 端口首尾为 14096/16095；`PidsLimit=8192`；无 Redis 认证错误。
...
成功条件：Java/XXL readiness 通过；`.serverid=test-agent-backend-122-233-30-114`；`.serverhost=122.233.30.114`；worker 端口首尾为 14096/16095；`PidsLimit=8192`；无 Redis 认证错误。
...
3. 保存后进入运行管理，两台 manager 都必须显示 `portStart=14096`、`portEnd=16095`、`maxProcesses=1000`。
...
5. 两台 manager 都显示 `14096/16095/1000`。
...
4. 使用 `ss -lntp` 检查 14096 和 16095 的真实占用者，不直接删除 state。
...
- 每台 worker 映射 14096-16095 共 2000 个端口坐标，但 `OPENCODE_MANAGER_MAX_PROCESSES=1000` 仍把每台实际进程数限制为 1000；CPU、内存、`PidsLimit=8192`、`nproc=8192`、文件句柄和模型并发都要另行压测。
- Redis 离线包不含 TLS 证书；防火墙只允许 `.4`、`.114` 和授权运维源访问 6379。
- 14096-16095 只对受信内网开放，浏览器不直连这些端口。
