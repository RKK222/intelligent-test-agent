...
> 本文仅保留用于追溯曾经的 `.4 + .114` 双后端方案。当前生产是 `122.233.30.114` 单后端，部署请以 `deploy/internal/README.md` 和 `deploy/internal/OPERATION-MANUAL.md` 为准。

本文档记录历史企业内拓扑：前端入口 `122.233.30.2`，后端/worker 节点 `122.233.30.4` 和 `122.233.30.114`，Redis `122.233.30.20`，PostgreSQL `122.42.203.103:8000/testagent`。
