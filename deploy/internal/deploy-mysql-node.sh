...
IMAGE_TAR="${SCRIPT_DIR}/mysql_8.4-linux-amd64.tar"
DEPLOY_LOG="$(cd "${SCRIPT_DIR}/.." && pwd)/deploy-${LOCAL_IP}-mysql.log"

[[ -f "${IMAGE_TAR}" ]] || { echo "Required file not found: ${IMAGE_TAR}" >&2; exit 1; }
verify_checksum_pair "${IMAGE_TAR}"
...
{
  bash "${DEPLOY_SCRIPT}" --env-file "${MYSQL_ENV}" --image-tar "${IMAGE_TAR}" validate
  bash "${DEPLOY_SCRIPT}" --env-file "${MYSQL_ENV}" --image-tar "${IMAGE_TAR}" deploy
