...
- `CommonParameterMemoryRegistry` / `CommonParameterMemoryApplicationService`：收集显式 `CommonParameterMemoryEntry`，校验 `englishName + platform` 唯一并稳定排序；Spring 单例装配完成时严格加载全部条目，任一失败阻止应用进入可用状态。数据库更新广播只刷新匹配项，手工操作刷新本机全部注册项；运行期单项失败保留上一有效源值、内存值与加载时间，只更新失败状态和安全错误。应用服务只返回本 Java 进程身份和诊断快照，不访问跨 Java 路由。
