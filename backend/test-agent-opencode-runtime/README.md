...
- Run Diff 查询、接受和拒绝。
- agent runtime 能力映射，包括 catalog/fs/vcs/lsp/mcp、config、provider auth/OAuth、worktree、session share、permission/question 和 MCP auth；permission/question 代理使用 opencode v2 `/api/session/{remoteSessionId}/permission|question` 路由，平台外部 API 和请求体保持兼容。pending permission/question list/reply/reject 只读取平台 Session 当前 `AgentSessionBinding` 和绑定节点，不触发当前用户进程校验、远端 `sessionExists` 探测或 `createSession` 重建，避免把 ask 回复发到非产生 ask 的 opencode 进程。list 会合并 root session-scoped 结果和同 workspace/directory 的 opencode 全局 pending request；reply/reject 首次 404 时会按全局 pending request 的 `requestId -> sessionID` 反查真实 owner session 并重试一次，覆盖 task 子会话或前端未透传远端 session 的场景。question reply/reject 可消费请求体中的 `remoteSessionId`，该字段只覆盖 opencode question path 的远端 session 段，用于把 task 子会话 ask 发回对应远端 session；平台 Session 仍用于鉴权、workspace directory 和原 binding 定位。重试后仍 404 的 pending ask 继续映射为 `CONFLICT + STALE_RUNTIME_REQUEST`，日志只记录 platformSessionId、targetRemoteSessionId、requestId、nodeId、directoryPresent 和 hasRemoteSessionOverride 等排障字段。
...
- `OpencodeRuntimeApplicationServiceTest` 覆盖 agent/provider/MCP runtime path、config/provider OAuth/worktree/share/MCP auth、workspace directory 透传、permission/question v2 session-scoped path、全局 pending request owner 兜底重试、question child remote session override、permission reply body 兼容和过期请求 `CONFLICT + STALE_RUNTIME_REQUEST` 映射。
...
- `OpencodeRuntimeApplicationServiceTest` 覆盖 agent/provider/MCP runtime path、用户进程节点路由、固定节点 fallback、普通 session binding 节点迁移自动重建、远端 session 404 缺失时自动重建绑定、远端校验非 404 错误透出、pending permission/question 保留原 binding 且不触发用户进程解析/sessionExists/createSession、config/provider OAuth/worktree/share/MCP auth、workspace directory 透传、permission/question v2 session-scoped path、全局 pending request owner 兜底重试、question child remote session override、permission reply body 兼容和过期请求 `CONFLICT + STALE_RUNTIME_REQUEST` 映射。
