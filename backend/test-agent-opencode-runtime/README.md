...
- OpenCode 内置 title agent 产生的既有 `session.updated`：仅当路由已确认当前 Run 的 root scope、远端 root ID 与平台 Session 绑定一致，且 `info.title` 有效且不是 `New session - <timestamp>` / `Child session - <timestamp>` 默认标题时，才回写对应平台 Session 标题；同时兼容 `rawPayload.properties.info.title` 包装。只有标题成功持久化，才在原 RunEvent payload 增加 `platformSessionTitleSynchronized: true` 和 `platformSessionTitle`；child、未知归属、绑定不一致、空标题、默认标题或标题仓储失败均不改平台标题，原事件仍照常持久化和 SSE 发布但没有标记。若首轮 root Run 成功时仍未收到原生确认标题，`RunSessionTitleFallbackService` 会以当前用户、当前 workspace 创建并清理一个临时远端 session，调用同一个原生 `title` agent；标题写回使用 MyBatis 条件更新，只有当前标题仍是首条消息临时标题才成功，避免覆盖后到的原生标题或用户手动改名。成功后仍用既有 `session.updated` 标记同步，额外带 `platformSessionTitleFallback: true`。`test-agent.opencode.session-title` 可配置开关、等待超时与轮询间隔。本行为不新增 API、DTO、事件 wire name、数据库/Flyway 或 SDK。
