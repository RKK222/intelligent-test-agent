...
- `OpencodeRuntimeApplicationService.sideQuestion` 复用已有 session message 读取、目标路由和 runtime HTTP 代理：按 40 条消息/约 48000 字符预算从消息边界 fork，超预算时只对临时 fork 调用 summarize，随后以 `tools: {"*": false}` 发送一条问题并在 finally 删除临时会话；主 session 不写入旁路问题。
