...
- `RunSessionScopeRouterTest` 覆盖 root/child idle/error 终态派生、task metadata/session parentID/session.children child discovery、原生 pending task part 到 child session.created 的 FIFO 绑定、未知 child pending drain、嵌套 session 防串流、raw event dedup 和全局 unknown 噪声过滤。
