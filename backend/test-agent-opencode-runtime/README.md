...
- `OpencodeProcessStartupServiceTest` 覆盖公共启动服务的 start、候选进程保存、启动后公共状态查询、短暂 HTTP health 不可达时等待恢复、持续健康失败超时、RUNNING/binding/heartbeat/ExecutionNode 回写、复用旧进程/绑定时间和健康失败状态映射。
