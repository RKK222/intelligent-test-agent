...
- OpenCode 内置 title agent 产生的既有 `session.updated`：仅当路由已确认当前 Run 的 root scope、远端 root ID 与平台 Session 绑定一致，且 `info.title` 有效时，才回写对应平台 Session 标题；同时兼容 `rawPayload.properties.info.title` 包装。只有标题成功持久化，才在原 RunEvent payload 增加 `platformSessionTitleSynchronized: true` 和 `platformSessionTitle`；child、未知归属、绑定不一致、空标题或标题仓储失败均不改平台标题，原事件仍照常持久化和 SSE 发布但没有标记。本行为不新增 API、DTO、事件 wire name、数据库/Flyway 或 SDK。
