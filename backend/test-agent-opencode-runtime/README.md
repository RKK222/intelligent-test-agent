...
- OpenCode 内置 title agent 产生的既有 `session.updated`：仅当路由已确认当前 Run 的 root scope、远端 root ID 与平台 Session 绑定一致，且 `info.title` 有效时，才回写对应平台 Session 标题；同时兼容 `rawPayload.properties.info.title` 包装。child、未知归属、绑定不一致或空标题均不改平台标题；标题仓储失败仅记录告警，不能阻断原始 RunEvent 的持久化或 SSE 发布。本行为不新增 API、DTO、事件 wire name、数据库/Flyway 或 SDK。
