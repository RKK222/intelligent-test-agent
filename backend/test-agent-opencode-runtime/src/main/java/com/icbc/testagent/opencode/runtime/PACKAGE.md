...
- `run.RunEventSseRouteService`：按 Run 原始 routing decision 解析生产 opencode 进程和目标 Java，供 API 层把 `/runs/{runId}/events` 流式转发到生产 Java。
...
- `run.*` 测试必须覆盖 Run 创建、通用 agent binding 保存/复用、远端 session 懒创建/复用、事件持久化策略、RunEvent SSE 生产 Java 路由解析、终态快照/token 回写、active-run、Diff fallback、消息恢复、Redis 输出活跃/待处理 ask 状态和 stale active Run 收敛任务。
