...
- `process.*`：当前用户 opencode 进程分配、通用参数 session/config 路径读取、manager WebSocket 控制面网关、后端实例生命周期和超级管理员运行管理快照查询。
...
- `process.*` 测试必须覆盖用户进程分配、通用参数路径读取、manager 控制面命令路由、后端心跳注册和运行管理快照聚合。
