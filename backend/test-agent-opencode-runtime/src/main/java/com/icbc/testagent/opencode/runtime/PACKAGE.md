...
- `run.RunStorageModeSelector` / `run.RunTerminalProjectionService`：前者按已校验上下文和 userId 稳定哈希为新 Run 固定存储模式；后者从 Redis 物化状态生成双摘要、usage、Diff 与远端定位终态投影，并通过领域端口执行三语句关系型事务。
...
- `run.*` 测试必须覆盖 Run 创建、通用 agent binding 保存/复用、远端 session 懒创建/复用、事件持久化策略、Redis manifest 优先的 RunEvent SSE 生产 Java 路由、new/legacy scope 数据库访问边界和新模式 root/scope/dedup/pending 统一端口、终态快照/token 回写、确定性双摘要的净化/Unicode 截断/fallback、Redis active-run、Diff fallback、消息恢复、Redis 输出活跃/待处理 ask 状态和 stale active Run 收敛任务。
