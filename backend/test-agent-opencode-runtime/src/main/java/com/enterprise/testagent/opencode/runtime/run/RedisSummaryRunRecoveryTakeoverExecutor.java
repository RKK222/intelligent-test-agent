...
public class RedisSummaryRunRecoveryTakeoverExecutor implements RunRecoveryTakeoverExecutor {
    ...
    private boolean runtimeStateUnavailable(Throwable error) {
        ...
        while (current != null) {
            if (current instanceof com.enterprise.testagent.common.error.PlatformException platform
                    && (platform.errorCode() == com.enterprise.testagent.common.error.ErrorCode.RUNTIME_STATE_UNAVAILABLE
                    || platform.errorCode() == com.enterprise.testagent.common.error.ErrorCode.RUN_DETAILS_EXPIRED)) {
                return true;
            }
