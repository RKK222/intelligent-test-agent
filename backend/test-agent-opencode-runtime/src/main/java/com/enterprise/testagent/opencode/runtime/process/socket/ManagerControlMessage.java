...
public record ManagerControlMessage(
    ...
    /**
     * 兼容新增 processCreated 前的完整构造调用；旧载荷必须保留 null 三态。
     */
    public ManagerControlMessage(
            String type,
            String protocolVersion,
    ...
    /** 构造启动命令，并明确标识该命令是否用于恢复已存在的平台 binding。 */
    public static ManagerControlMessage command(
            String commandId,
            String command,
            int port,
        ...
        return new ManagerControlMessage(
                ...
                null,
                null,
                bindingRecovery ? Boolean.TRUE : null);
    }

    /**
     * 构造带统一认证号和期望 PID 的实例所有权校验停止命令。
     */
    public static ManagerControlMessage ownedStopCommand(
            String commandId,
            int port,
