...
- `run.Run`、`run.RunId`、`run.RunStatus`、`run.TokenUsage`、`run.RunRepository`：运行聚合和值对象、状态机、单次 token 消耗和值对象、持久化端口；`RunRepository.saveIfStatus` 用于按当前状态条件保存，防止终态事件与异步 transport error 竞态时旧状态覆盖新终态。
