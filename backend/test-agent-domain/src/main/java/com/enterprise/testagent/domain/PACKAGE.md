...
- `run.RunPersistenceAnchor`、`run.RunTerminalProjection`、`run.RunConversationSummary`、`run.RunDetailsLocator`、`run.RunSummaryPersistencePort`：普通 Run 的 PostgreSQL 无原文控制面锚点、终态双摘要和低频远端定位端口；Redis 摘要与 legacy Scheduled Run 共用 Session/clientRequestId 唯一键，legacy 额外携带 Scheduled dispatch attempt/租约/受理时间并通过 claim/mark fencing 恢复 anchor-only 崩溃窗口。锚点插入仅允许活动态，但查询允许同一行后续更新出的终态以恢复稳定 runId。`RunDetailsLocator.dispatchMessageId` 用于 Redis 详情过期后的目标轮因果恢复，对象约束禁止承载原始输入输出、reasoning、工具内容或事件 payload。
