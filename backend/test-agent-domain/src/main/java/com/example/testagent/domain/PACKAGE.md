# 包说明：com.example.testagent.domain

## 职责

纯领域模型包，表达 Workspace、Session、Run、RunEvent、ExecutionNode、RoutingDecision 等核心业务概念和状态规则。
...
- `session.Session`、`session.SessionId`、`session.SessionStatus`、`session.SessionRepository`：会话领域对象和值对象、持久化端口。
