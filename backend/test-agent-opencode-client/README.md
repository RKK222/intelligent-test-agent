...
- `GeneratedOpencodeSdkGateway`：唯一直接调用 generated SDK 的内部适配器；使用自定义 generated `ApiClient` WebClient 缓冲上限，避免 `/session/{sessionID}/message` 大量 tool/read/write parts 超过默认 256KB 后无法恢复历史消息。
