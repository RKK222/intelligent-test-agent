...
- OpenCode 运行时按 `公共共享 → 公共个人 → 应用共享 → 应用个人` 合并，后层覆盖前层但不改写任何个人 Git 分支。用户进程启动时注入稳定公共个人配置目录及应用/个人受信根；OpenCode 仅在个人 worktree 固定结构内把当前目录映射到同版本应用共享副本。公共或应用共享配置推送先进入持久化 rollout，所有服务器副本到达明确 commit 且目标进程空闲后执行全局 dispose；公共个人或应用个人定义保存只排队热加载当前用户进程，忙碌 Run 结束后执行。
