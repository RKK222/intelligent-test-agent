...
- 基于应用版本工作区副本创建个人 git worktree，根目录优先读取 `common_parameters.OPENCODE_PERSONAL_WORKTREE_ROOT`。OpenCode、编辑器和终端始终使用该个人 worktree；应用版本副本对普通成员只读。个人 worktree 的普通文件可以本地编辑、stage 和 commit；普通成员和应用管理员的 `spec/**` 只保留在个人分支（生成结果另由系统同步），不参与远程发布，服务端按规范化路径拒绝混选和 `./spec` 别名；`SUPER_ADMIN` 发布时显式豁免。前端“提交并推送”会先把全部选中文件提交到个人 HEAD，再将该角色允许发布的文件投影到应用 feature 分支。发布不会 merge 整个个人分支，完成后广播 `workspace.version.sync-requested`，在线用户手动刷新/同步，禁止自动覆盖脏工作树。旧同步接口仍沿用普通角色的强制投影规则，`force` 不绕过本地提交、`spec/**` 禁推和 feature 分支保护。
