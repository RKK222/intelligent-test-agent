...
- Agent 配置管理：公共级读取 `OPENCODE_PUBLIC_AGENT_GIT_URL`、`OPENCODE_PUBLIC_CONFIG_GIT_ROOT`、`OPENCODE_PUBLIC_CONFIG_WORKTREE_ROOT`，工作空间级使用 `{workspace.rootPath}/.opencode/agents/`；支持只读浏览、SUPER_ADMIN 写入、Git diff/stage/commit/publish、公共配置更新、worktree 模式和 `agent-config.public-sync-requested` 广播同步。
...
- `AgentConfigApplicationServiceTest` 覆盖公共 Git 地址未配置禁用、公共更新 clone 与广播、worktree 名称拼接日期、工作空间级复数 agents 目录读写。
