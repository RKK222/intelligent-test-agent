...
- Agent/Skill 配置管理：公共 Git 仍由 `SUPER_ADMIN` 独占；应用级 `.opencode/agents/**`、`.opencode/skills/**`（含 rules/templates）由 `APP_ADMIN` 管理，`SUPER_ADMIN` 继承该权限；普通成员只能读取。应用级文件树、读取和写入统一走目标服务器文件 WebSocket ticket，Git API 通过 `BackendJavaRouteResolver`/`BackendHttpForwarder` 路由到用户 opencode 绑定所在的 Java，避免跨服务器落到错误磁盘。创建应用技能包默认生成 OpenCode 可识别的 `skills/<name>/SKILL.md`、`rules/README.md`、`templates/README.md` 模板。
...
- 基于应用版本工作区副本创建个人 git worktree，根目录优先读取 `common_parameters.OPENCODE_PERSONAL_WORKTREE_ROOT`。OpenCode、编辑器和终端始终使用该个人 worktree；应用版本副本对普通成员只读。个人 worktree 的普通文件可以本地编辑、stage 和 commit，`spec/**` 只保留在个人分支（生成结果另由系统同步），不参与远程发布。发布入口会在服务端按规范化路径强制拒绝任何 `spec/**`，包括与可发布文件混选或使用 `./spec` 路径别名的请求；前端“提交并推送”会先把全部选中文件提交到个人 HEAD，再只将非 `spec/**` 文件投影到应用 feature 分支。发布不会 merge 整个个人分支，完成后广播 `workspace.version.sync-requested`，在线用户手动刷新/同步，禁止自动覆盖脏工作树。旧同步接口也复用该强制投影规则，`force` 不绕过本地提交、`spec/**` 禁推和 feature 分支保护。
...
- `ManagedWorkspaceApplicationServiceTest` 覆盖应用成员校验及 `FORBIDDEN` 加载上下文、托管逻辑路径、个人 worktree 创建、Git diff、个人 worktree 本地提交、从个人 `HEAD` 按白名单投影并推送 feature、spec 不随个人分支发布、应用副本只读 Git 操作、广播和失败阶段命令透传；同步接口覆盖仅使用已提交文件的兼容路径。
