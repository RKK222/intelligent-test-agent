...
- 工作区内文件单层列表、UTF-8 内容读写、普通文件同目录重命名、文件状态、普通文件删除和路径越权拦截。
...
- Agent/Skill 配置管理：公共级按当前部署模式解释 `OPENCODE_PUBLIC_AGENT_GIT_URL`，外部部署直接使用完整 URL，内部部署按应用版本库规则把该值视为 `host[:port]/path` 片段并按当前管理员统一认证号动态拼接 `ssh://{unifiedAuthId}@...`；远端分支查询、clone、fetch、pull、push 都使用该有效 URL。同时读取 `OPENCODE_PUBLIC_CONFIG_GIT_ROOT`、`OPENCODE_PUBLIC_CONFIG_DIR`、`OPENCODE_PUBLIC_CONFIG_WORKTREE_ROOT`，文件树根为 `{OPENCODE_PUBLIC_CONFIG_GIT_ROOT}/opencode/`，`skills/` 下直接放实际技能包；工作空间级文件树根为 `{workspace.rootPath}/.opencode/`，可维护 `agents/` 与 `skills/` 技能包。支持只读浏览、SUPER_ADMIN 写入、Git diff/stage/commit/publish、公共配置显式拉取、worktree 模式和 `agent-config.public-sync-requested` 广播同步。工作区 Git Diff 与 Agent 配置 Diff 共用 `GitWorkspaceService` 的 porcelain 解析和 diff 聚合；Git 写入发布共用 `GitPublishWorkflow` 统一执行 clean、fetch、pull --ff-only、merge、push、headCommit 和冲突后 merge abort。工作空间级 Agent Git diff 只扫描 `.opencode/` 下的 `agents/` 与 `skills/`，返回给 UI 时去掉 `.opencode/` 前缀，避免普通应用文件误进入 agents 分组。公共 Git 地址缺失或为 `UNCONFIGURED` 时，禁用原因会明确提示超级管理员到“系统管理 → 通用参数管理”配置对应参数。公共配置初始化/显式拉取会在 Git 根目录缺失或为空目录时 clone 到 `OPENCODE_PUBLIC_CONFIG_GIT_ROOT`，内部模式比较已有 origin 时忽略 `ssh://任意用户@` 前缀，避免不同管理员操作同一内部库时误判不一致，并校验 `OPENCODE_PUBLIC_CONFIG_DIR` 存在且非空；该目录应由公共配置 Git 仓库内容提供，不在 manager 启动时创建空目录。已有仓库的未提交修改不再阻断只读文件树，显式拉取默认拒绝覆盖，只有请求显式携带 `discardLocalChanges=true` 时才恢复已跟踪文件再拉取。公共 Agent 提交并推送入口先 fetch 远端，再提交当前工作区修改、merge `origin/{branch}` 并 push；即使本次没有新 commit 也会继续 merge 和 push，避免已有未推送本地提交被 UI 误报成功。merge 远端产生冲突时返回 `CONFLICT` 和安全 `conflictFiles`，保留 Git 原生 merge 现场，管理员可复用公共冲突接口逐个或批量解决、取消合并，全部解决后再次提交会先落 merge commit 再 push；远端拒绝 push 时归因为 `REMOTE_REJECTED` 并返回安全 `gitFailureHint`。公共 worktree 创建不再首次 clone，只允许在管理员选择的已初始化服务器上创建，并把 `worktreeId -> linuxServerId` 落库。公共 worktree 切换列表按 `scope=PUBLIC`、`linuxServerId` 和 `status=ACTIVE` 查询，并补充创建人 `userId/username`；后续 Agent/Skill 配置文件目录列表/读取/写入由 API 层按服务器归属签发文件 WebSocket ticket 执行，Git 操作仍按服务器归属路由。Git 命令失败会通过统一 Git 错误码返回安全归因和 `gitFailureHint`，worktree 合并冲突会返回安全的 `conflictFiles` 列表，便于前端提示认证、仓库、网络、分支或冲突文件问题。远端分支/目录只读查询最长等待 60 秒，公共配置分支加载、初始化和底层 Git 命令会输出带 traceId 的结构化日志，日志中的 Git URL 和命令会隐藏 SSH/HTTP 用户名或 token。
...
- `WorkspaceFileServiceTest` 覆盖 UTF-8 读写、普通文件同目录重命名、目标冲突、普通文件删除、目录删除拒绝、路径穿越拒绝、目录列表排序与上限、文件大小限制和 null 内容写入。
