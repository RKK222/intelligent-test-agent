...
- 基于应用版本工作区副本创建个人 git worktree，根目录优先读取 `common_parameters.OPENCODE_PERSONAL_WORKTREE_ROOT`。OpenCode、编辑器和终端始终使用该个人 worktree；应用版本副本对普通成员只读。个人 worktree 的普通文件可以本地编辑、stage 和 commit，`spec/**` 只保留在个人分支（生成结果另由系统同步），不参与远程发布。发布先要求白名单文件已在个人 worktree 本地提交，再以个人 `HEAD` 加相对路径为依据，把选中文件投影到应用 feature 分支 worktree，提交并推送 feature 分支；不会 merge 整个个人分支，因此 spec 或未选文件不会泄漏。发布完成后广播 `workspace.version.sync-requested`，在线用户手动刷新/同步，禁止自动覆盖脏工作树。旧同步接口也复用该投影规则，`force` 不绕过本地提交和 feature 分支保护。
