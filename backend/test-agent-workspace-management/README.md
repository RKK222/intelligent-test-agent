...
- `ManagedWorkspaceApplicationServiceTest` 覆盖应用成员校验、标准库分支校验、设置页初始版本工作区创建、应用版本工作区创建、通用参数根目录、代码库英文名路径片段、服务器副本记录、目标 commit、广播发布、`git pull`、运行态 Workspace 关联、最近使用记录、私人 worktree 新命名规则、Git diff 路径解码、单文件 discard、`yyyy年M月` 版本格式（`sanitizeVersionForBranchAndPath` 转 `yyyy-MM` 派生分支/路径）和非法版本格式拒绝。
