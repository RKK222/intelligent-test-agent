...
- `GitWorkspaceService`：封装 clone、worktree add、分支/origin/head/status、porcelain/diff、索引恢复到 HEAD、冲突 stage 1/2/3 读取、按操作人身份提交、push/pull/fetch、冲突文件列表、`merge --abort` 和按提交/路径白名单投影文件等 Git 原子命令；提交身份只通过命令级 `-c user.name/user.email` 注入，不写全局或共享仓库配置。
