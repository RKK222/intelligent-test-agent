...
- `GitWorkspaceService`：封装 clone、worktree add、同名分支已存在时复用分支挂载 worktree、当前分支/origin/head/status 查询、提交指定文件、push/force-with-lease、pull --ff-only、fetch 和 reset --hard 到指定 commit。
...
- `GitWorkspaceServiceTest` 覆盖 clone 分支、worktree 创建、同名分支复用、分支/origin/head/status 查询、提交、push、pull、fetch/reset 和临时 SSH key 清理。
