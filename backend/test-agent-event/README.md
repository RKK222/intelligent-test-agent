...
SSE polling 不得在 Reactor interval/event-loop 线程上直接执行阻塞式 Repository 查询；回放瞬时失败不能改变 Run 终态，只能等待后续轮询或客户端重连恢复。transient 事件不写入数据库、不设置 SSE `id`，不能作为 `Last-Event-ID` 恢复点。Redis bus 是实时增强通道，不替代 `run_events` replay、消息快照恢复或业务表补偿扫描。
