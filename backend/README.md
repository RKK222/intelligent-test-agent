...
export TEST_AGENT_DB_URL=jdbc:postgresql://<pg-host>:5432/<database>
export TEST_AGENT_DB_USERNAME=<username>
export TEST_AGENT_DB_PASSWORD=<password>
export TEST_AGENT_API_TOKEN=<api-token>
export TEST_AGENT_CORS_ALLOWED_ORIGINS=https://<frontend-origin>
...
```bash
export TEST_AGENT_REDIS_ENABLED=true
export TEST_AGENT_REDIS_HOST=<redis-host>
export TEST_AGENT_REDIS_PORT=6379
```

## 本地运行依赖

研发测试主路径使用外部 PostgreSQL、Redis 和 opencode server 地址。`deploy/local/docker-compose.yml` 只保留为个人离线开发备用入口：
...
`tools/dev-backend-run.sh` 是后续本地启动后端的统一入口，默认读取仓库根目录未跟踪的 `.env.local` 并启动 `local` profile；传入 `--profile test` 时读取 `.env.test` 并启动 `test` profile。`.env.local` 和 `.env.test` 已被 `.gitignore` 排除，真实数据库密码只允许写入这些本机文件。
