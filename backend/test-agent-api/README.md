...
- `POST /api/internal/platform/opencode-runtime/sessions/{sessionId}/side-question` 保留同步兼容路径；`.../side-question/runs` 与 agent-scoped 等价路径立即返回旁路 Run，并复用 RunEvent SSE 输出阶段、增量和终态。运行时在临时 fork 中按上下文规模决定是否 compact，流式路径固定使用 `build` agent，并以系统提示限制必要检查为只读、等待自然语言最终回答，返回后删除临时会话，不追加主会话历史。
