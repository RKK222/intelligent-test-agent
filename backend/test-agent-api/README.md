...
- 暴露配置管理接口，Controller 只委托 configuration-management 业务服务；应用与工作区接口统一校验 `APP_ADMIN`，`SUPER_ADMIN` 继承该能力。版本库类型下拉接口和新增代码库的 `repositoryType` DTO 仅做协议转换，旧 `standard` 兼容派生由业务服务处理。设置页创建应用工作空间接口会读取当前用户 READY opencode 进程的 Linux 服务器并委托 workspace-management 创建初始版本工作区，进度通过 `workspace-create-operations/{operationId}` HTTP 轮询查询。
...
- `AuthControllerRolesTest`、`ConfigurationManagementControllerTest` 覆盖认证响应 roles、`APP_ADMIN`/`SUPER_ADMIN` 鉴权、代码库英文名与版本库类型 DTO、版本库类型下拉接口、工作空间创建进度轮询和 SSH key 不回显私钥。
