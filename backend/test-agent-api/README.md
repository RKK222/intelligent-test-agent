...
- `GET /api/internal/platform/opencode-runtime/sessions` 是当前登录用户历史会话分页接口，支持 `page/size/q`，返回 `workspaceContext`（应用、应用工作空间模板和版本上下文）且按 `updatedAt desc` 排序；列表不校验当前应用成员资格，点击切换时再由 workspace-management recent 接口校验权限。Session 历史正文恢复主入口是 agent-scoped session tree messages，响应以可选 `historyRepresentation/replayAvailable/detailsAvailableUntil` 区分完整详情和摘要降级；内部平台 `GET /api/internal/platform/opencode-runtime/sessions/{sessionId}/messages` 支持 `refresh=false` 只读数据库快照，`SessionMessageResponse` 可选携带 `contentKind/summaryStatus/summaryVersion`，供前端反馈 messageId 映射和只读 transcript 使用。`RunResponse` 同样可选携带 `storageMode/clientRequestId/detailsAvailableUntil`；旧领域对象映射保持这些字段为空，专用新模式投影通过 DTO 重载显式接入。active-run API 供前端刷新后恢复 SSE。
...
- `POST /api/internal/platform/opencode-runtime/sessions/{sessionId}/side-question` 与 agent-scoped 等价路径提供宠物旁路问答；Controller 只做 DTO/traceId/认证主体适配，运行时在临时 fork 中按上下文规模决定是否 compact，使用 `plan` agent 的只读权限完成必要检查并等待自然语言最终回答，返回后删除临时会话，不追加主会话历史。
...
- `UserOpencodeBackendRoutingWebFilterTest` 覆盖用户 opencode 进程请求按已有 binding 所属服务器转发、内部路由头防循环、只读状态 GET 的降级，以及 agent/platform 两个 start-run 入口通过 context 只读路由时零 assignment 调用、远端/本地 body 可复读、过期或显式非法 token 统一 409 且不回显 token、请求体超限统一 400；无 token 的初始化、Run 和工作区操作继续覆盖兼容路由。
...
- `RunEventSseBackendRoutingWebFilterTest` 覆盖 RunEvent SSE 按 Run 生产 Java 路由、平台/agent URL、防循环头和目标缺失时本机处理；`RunEventSseRouteServiceTest` 固化 manifest 路由 0 次 routing/process Repository 查询；`BackendSseForwarderTest` 覆盖 SSE 流式转发保留 Authorization、`X-Trace-Id`、`Last-Event-ID`、query、`text/event-stream` 和 `X-Test-Agent-Backend-Routed`。
