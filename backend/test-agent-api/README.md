...
- RunEvent SSE 建连前由 `RunEventSseBackendRoutingWebFilter` 按 Run 原始归属定位生产 Java，目标不是当前 Java 时使用 `BackendSseForwarder` 流式转发 `text/event-stream` 并保留 Authorization、trace、Last-Event-ID 和 query；目标 Java 的 `RunController` 再委托 runtime 按固定 storageMode 建流：legacy 恢复 opencode projected messages 并合并 durable replay/live bus，Redis 新模式则先发完整物化 reset、再读 `runtimeVersion` 尾流。HTTP `GET /api/internal/agent/{agentId}/runs/{runId}/session-tree/messages` 返回同一当前 Run 子树 snapshot 并合并当前 Run durable 状态事件，`GET /api/internal/agent/{agentId}/sessions/{sessionId}/session-tree/messages` 返回 root 下跨 Run 已发现的历史子树并按 root session 合并 durable 状态事件，旧 `/api/runs/...` 与 `/api/sessions/...` 已作废。
