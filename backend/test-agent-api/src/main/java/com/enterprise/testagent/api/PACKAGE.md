...
- `web.platform.ReferenceRepositoryController`、`web.platform.ReferenceRepositoryDtos`：应用引用资产库列表、初始化、同步、受控分支切换、只读指针核验、状态和单层树内部入口；只负责 `APP_ADMIN`（含 `SUPER_ADMIN`）鉴权、分支请求、traceId 和阻塞任务调度。
