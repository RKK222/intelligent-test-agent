...
- `web.platform.WorkspaceController`、`web.platform.SessionController`、`web.platform.RunController`、`web.platform.TerminalController`：平台协议入口，旧 `/api/...` 与 `/api/internal/platform/...` URL 并行映射。
- `web.platform.PlatformOpencodeRuntimeController`：平台侧 opencode runtime 代理入口，只承载旧 `/api/...` 与 `/api/internal/platform/...` 路径。
- `web.agent.AgentOpencodeRuntimeController`：agent 侧 opencode 兼容代理入口，只承载 `/api/internal/agent/opencode/...` 路径。
...
- `backend/test-agent-api/src/test/java/com/example/testagent/api/web/platform`。
- `backend/test-agent-api/src/test/java/com/example/testagent/api/web/agent`。
- `backend/test-agent-api/src/test/java/com/example/testagent/api/web/common`。
- `backend/test-agent-api/src/test/java/com/example/testagent/api/config`。
- 平台 Controller 测试必须覆盖公开 `/api/...` 和内部 `/api/internal/platform/...` 路径；agent Controller 测试必须覆盖需要兼容的 `/api/internal/agent/opencode/...` 路径。
