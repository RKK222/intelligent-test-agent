...
- `web.platform.PlatformOpencodeRuntimeController`：平台侧 opencode runtime 代理入口，只承载旧 `/api/...` 与 `/api/internal/platform/...` 路径，并把可选用户主体和 question `remoteSessionId` 请求体原样交给业务层决定用户进程、固定节点 fallback 或远端 question session。
...
- `web.agent.AgentOpencodeRuntimeController`：agent 侧 opencode 兼容代理入口，承载 `/api/internal/agent/{agentId}/...` 路径并把 agentId、可选用户主体和 question `remoteSessionId` 请求体交给业务层选择 runtime。
