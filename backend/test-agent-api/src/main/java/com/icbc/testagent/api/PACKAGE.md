...
`RunController` 的 RunEvent SSE 入口只做目标 Java 上的协议合流：先通过当前 agent runtime 输出 projected messages snapshot，再输出 `test-agent-event` 提供的 durable replay/live bus/可选 Redis SSE；不直接访问 Repository 或 generated SDK。
