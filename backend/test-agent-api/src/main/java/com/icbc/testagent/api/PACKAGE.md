...
- `web.agent.AgentOpencodeRuntimeController`：agent 侧 opencode 兼容代理入口，承载 `/api/internal/agent/{agentId}/...` 路径并把 agentId 交给业务层选择 runtime。
...
- 平台 Controller 测试必须覆盖公开 `/api/...`、内部 `/api/internal/platform/...` 和 Run 相关 `/api/internal/agent/{agentId}/...` 路径；agent Controller 测试必须覆盖需要兼容的 `/api/internal/agent/opencode/...` 路径和 agentId 透传。
