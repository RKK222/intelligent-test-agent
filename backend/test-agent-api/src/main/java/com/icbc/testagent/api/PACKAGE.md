...
- `web.platform.RuntimeManagementController`：超级管理员运行管理入口，校验 `SUPER_ADMIN` 后把筛选、分页、命令参数和 traceId 交给 runtime 查询/命令服务；API 层不实现 opencode server 启动、停止或健康确认。
...
- `web.platform.AgentConfigController`：Agent 配置 HTTP 元数据、Git 操作和进度 ticket 入口；公共 worktree 列表只返回指定服务器 `ACTIVE/PUBLIC` 元数据和创建人字段，文件内容操作继续走平台文件 WebSocket。
...
- `web.platform.WorkspaceFileWebSocketHandler`：受控平台文件 WebSocket upgrade 入口，覆盖 workspace 文件、目录选择和 Agent 配置文件 RPC。
...
- Agent 配置 Controller 测试必须覆盖公共 worktree 列表的 `SUPER_ADMIN` 权限、`linuxServerId` 缺参校验和响应字段。
