...
- `ScheduledTaskMapper` / `ScheduledTaskMapper.xml` / `MyBatisScheduledTaskRepository`：替代旧 JDBC scheduler 仓储，持久化任务定义、运行记录、服务器亲和 USER_PLAN 查询及 `PENDING -> RUNNING/SKIPPED` 条件更新。
- `NightExecutionTaskMapper` / `NightExecutionTaskMapper.xml` / `MyBatisNightExecutionTaskRepository`：持久化夜间任务、owner/clientRequestId 幂等锁、任务状态 CAS、会话锁和全局时段容量；PostgreSQL 首次创建使用事务级 advisory lock，过期任务与占位保留 30 天后清理。
