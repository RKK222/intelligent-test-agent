...
- `mybatis.ReferenceRepositoryMapper` / `mybatis/ReferenceRepositoryMapper.xml`：引用资产总体状态与服务器副本的全部关系型 SQL，包含主键初始化、generation 推进、目标 upsert、离线 `DEFERRED`、租约认领/续期和带 fencing 条件的状态写回。
